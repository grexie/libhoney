...
HoneycombByteReadHandler::HoneycombByteReadHandler(const unsigned char* bytes,
                                       size_t size,
                                       HoneycombRefPtr<HoneycombBaseRefCounted> source)
    : bytes_(bytes), size_(size), offset_(0), source_(source) {}

size_t HoneycombByteReadHandler::Read(void* ptr, size_t size, size_t n) {
...
int HoneycombByteReadHandler::Seek(int64_t offset, int whence) {
  int rv = -1L;
  base::AutoLock lock_scope(lock_);
  switch (whence) {
    case SEEK_CUR:
...
int64_t HoneycombByteReadHandler::Tell() {
  base::AutoLock lock_scope(lock_);
  return offset_;
}

int HoneycombByteReadHandler::Eof() {
  base::AutoLock lock_scope(lock_);
