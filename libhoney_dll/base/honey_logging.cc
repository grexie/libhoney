...
LogMessage::~LogMessage() {
  ...
  honey_log(file_, line_, severity_, str_newline.c_str());
}
...
std::ostream& operator<<(std::ostream& out, const wchar_t* wstr) {
  ...
  if (length > 0) {
    honey_string_utf8_t str = {0};
    honey_string_wide_to_utf8(wstr, length, &str);
    out << str.str;
...
std::ostream& operator<<(std::ostream& out, const char16_t* wstr) {
  ...
  if (length > 0) {
    honey_string_utf8_t str = {0};
    honey_string_utf16_to_utf8(wstr, length, &str);
    out << str.str;
    honey_string_utf8_clear(&str);
  }
  return out;
