...
# This example utility uses the honey_json_builder class to create and update an
# index.json file in the same directory as this file. Example usage:
#
# Add files for macosx64 platform at the specified version:
# > python honey_json_builder_example.py add macosx64 3.2704.1416.g185cd6c 51.0.2704.47
#
# Add files for all platforms at the specified version:
# > python honey_json_builder_example.py add all 3.2704.1416.g185cd6c 51.0.2704.47
#
# See honey_json_builder.get_platforms() for the list of supported platforms.
...
def make_fake_file_info(platform, version, type):
  return {
      'name':
          honey_json_builder.get_file_name(version, platform, type) + '.tar.gz',
      'size':
          make_fake_size(),
      'lastModified':
...
if __name__ == '__main__':
  ...
  if len(sys.argv) < 5 or sys.argv[1] != 'add':
    sys.stderr.write(
        'Usage: %s add <platform> <honey_version> <chromium_version>\n' %
        sys.argv[0])
    sys.exit()
  ...
  elif sys.argv[2] in honey_json_builder.get_platforms():
    platforms = [sys.argv[2]]
  else:
    sys.stderr.write('Invalid platform: %s\n' % platform)
    sys.exit()

  # Requested Honeycomb version.
  honey_version = sys.argv[3]
  if not honey_json_builder.is_valid_version(honey_version):
    sys.stderr.write('Invalid Honeycomb version: %s\n' % honey_version)
    sys.exit()

  # Requested Chromium version.
  chromium_version = sys.argv[4]
  if not honey_json_builder.is_valid_chromium_version(chromium_version):
  ...
  # example purposes.
  builder = honey_json_builder(prettyprint=True, silent=False)

  # Map the Honeycomb version to the Chromium version to avoid a remote query.
  builder.set_chromium_version(honey_version, chromium_version)
  ...
  #   honey_json_builder.add_file() will return False and upload of the file
  #   should be skipped.
  new_files = []
  for platform in platforms:
    new_files.extend(create_fake_files(platform, honey_version))
