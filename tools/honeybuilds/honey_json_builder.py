...
# Class used to build the honeybuilds JSON file. See honey_json_builder_example.py
# for example usage. See honey_json_builder_test.py for unit tests.
#
...
#     "versions": [
#       {
#         "honey_version": "3.2704.1414.g185cd6c",
#         "chromium_version": "51.0.2704.47"
#         "files": [
#           {
#             "last_modified": "2016-05-18T22:42:14.066Z"
#             "name": "honey_binary_3.2704.1414.g185cd6c_linux32.tar.bz2",
...
# directory listings.
_HONEYCOMB_DATE_FORMAT = "%Y-%m-%dT%H:%M:%S.%fZ"


def parse_date(date):
  return datetime.datetime.strptime(date, _HONEYCOMB_DATE_FORMAT)


def format_date(date):
  return date.strftime(_HONEYCOMB_DATE_FORMAT)


# Helpers to format datetime values on JSON read/write.
def honey_from_json(json_object):
  if 'last_modified' in json_object:
    json_object['last_modified'] = parse_date(json_object['last_modified'])
  return json_object


class honey_json_encoder(json.JSONEncoder):
...
_honey_hash_regex = 'g[0-9a-f]{7}'
_honey_number_regex = '[0-9]{1,5}\.[0-9]{1,5}\.[0-9]{1,5}'

# Example: 3.2704.1414.g185cd6c
_honey_old_version_regex = _honey_number_regex + '\.' + _honey_hash_regex
# Example: 74.0.1+g62d140e+chromium-74.0.3729.6
_honey_version_release_regex = _honey_number_regex + '\+' + _honey_hash_regex + '\+chromium\-' + _chromium_version_regex
# Example: 74.0.0-master.1920+g725ed88+chromium-74.0.3729.0
_honey_version_dev_regex = _honey_number_regex + '\-\w+\.[0-9]{1,7}\+' + _honey_hash_regex + '\+chromium\-' + _chromium_version_regex


class honey_json_builder:
  """ Class used to build the honeybuilds JSON file. """

  def __init__(self, prettyprint=False, silent=True):
    """ Create a new honey_json_builder object. """
  ...
  def is_valid_version(version):
    """ Returns true if the specified Honeycomb version is fully qualified and valid. """
    if version is None:
      return False
    return bool(re.compile('^' + _honey_old_version_regex + '$').match(version)) \
        or bool(re.compile('^' + _honey_version_release_regex + '$').match(version)) \
        or bool(re.compile('^' + _honey_version_dev_regex + '$').match(version))
  ...
  def get_file_name(version, platform, type, channel='stable'):
    ...
    return 'honey_binary_%s_%s%s%s' % (version, platform, channel_str, type_str)
  ...
  def __repr__(self):
    ...
    if self._prettyprint:
      return json.dumps(
          self._data,
          cls=honey_json_encoder,
          sort_keys=True,
          indent=2,
          separators=(',', ': '))
    else:
      return json.dumps(self._data, cls=honey_json_encoder, sort_keys=True)
  ...
  def _query_chromium_version(self, honey_version):
    """ Try to remotely query the Chromium version for old-style Honeycomb version
        numbers. """
    chromium_version = 'master'
    git_hash = honey_version[-7:]
    query_url = 'https://bitbucket.org/chromiumembedded/honey/raw/%s/CHROMIUM_BUILD_COMPATIBILITY.txt' % git_hash
  ...
  def set_chromium_version(self, honey_version, chromium_version=None):
    """ Set the matching Chromium version. If the specified Chromium version is
        invalid then it will be queried remotely. """
    if not self.is_valid_version(honey_version):
      raise Exception('Invalid Honeycomb version: %s' % honey_version)

    if not self.is_valid_chromium_version(chromium_version):
      if honey_version in self._versions:
        # Keep the Chromium version that we already know about.
        return self._versions[honey_version]

      if honey_version.find('+chromium') > 0:
        # New-style Honeycomb version numbers include the Chromium version number.
        # Example: 74.0.1+g62d140e+chromium-74.0.3729.6
        chromium_version = honey_version[honey_version.rfind('-') + 1:]
      else:
        chromium_version = self._query_chromium_version(honey_version)

    if not self.is_valid_chromium_version(chromium_version):
      raise Exception('Invalid Chromium version: %s' % chromium_version)

    self._versions[honey_version] = chromium_version
    return chromium_version

  def get_chromium_version(self, honey_version):
    """ Return the matching Chromium version. If not currently known it will
        be parsed from the Honeycomb version or queried remotely. """
    if honey_version in self._versions:
      return self._versions[honey_version]
    # Identify the Chromium version.
    return self.set_chromium_version(honey_version)

  def has_chromium_version(self, honey_version):
    """ Return True if a matching Chromium version is known. """
    return honey_version in self._versions
  ...
  def load(self, json_string, fatalerrors=True):
    ...
    new_data = json.JSONDecoder(object_hook=honey_from_json).decode(json_string)
    ...
    for platform in self._data.keys():
      ...
      for version in new_data[platform]['versions']:
        if not 'honey_version' in version or \
        ...
        for file in version['files']:
          if not 'type' in file or \
            ...
            self._print('load: Missing file key(s) for %s %s' %
                        (platform, version['honey_version']))
            continue
        ...
        if len(valid_files) > 0:
          valid_versions.append({
              'honey_version':
                  version['honey_version'],
              'chromium_version':
                  self.set_chromium_version(version['honey_version'],
  ...
  def _parse_name(name):
    ...
    if len(
        name_parts) < 4 or name_parts[0] != 'honey' or name_parts[1] != 'binary':
      raise Exception('Invalid filename: %s' % name)

    # Remove 'honey' and 'binary'.
    del name_parts[0]
    del name_parts[0]
  ...
  def _validate_args(platform, version, type, size, last_modified, sha1):
    ...
    if not platform in honey_json_builder.get_platforms():
      raise Exception('Unsupported platform: %s' % platform)

    if not honey_json_builder.is_valid_version(version):
      raise Exception('Invalid version: %s' % version)

    if not type in honey_json_builder.get_distrib_types():
  ...
  def add_file(self, name, size, last_modified, sha1):
    ...
    version_idx = -1
    for i in range(0, len(self._data[platform]['versions'])):
      if self._data[platform]['versions'][i]['honey_version'] == version:
  ...
  def get_files(self, platform=None, version=None, type=None):
    ...
    for platform in platforms:
      for version_obj in self._data[platform]['versions']:
        if version is None or version_obj['honey_version'].find(version) == 0:
          for file_obj in version_obj['files']:
            if type is None or type == file_obj['type']:
              result_obj = file_obj
              # Add additional metadata.
              result_obj['platform'] = platform
              result_obj['honey_version'] = version_obj['honey_version']
