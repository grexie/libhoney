...
from honey_json_builder import honey_json_builder
import datetime
import unittest


class TestHoneycombJSONBuilder(unittest.TestCase):

  # Test Honeycomb version number matching.
  def test_valid_version(self):
    # Old-style version numbers.
    self.assertTrue(honey_json_builder.is_valid_version('3.2704.1414.g185cd6c'))
    # New-style version numbers.
    self.assertTrue(
        honey_json_builder.is_valid_version(
    ...
    self.assertFalse(honey_json_builder.is_valid_version(None))
    self.assertFalse(honey_json_builder.is_valid_version('foobar'))
    self.assertFalse(honey_json_builder.is_valid_version('3.2704.1414'))
    self.assertFalse(honey_json_builder.is_valid_version('74.0.1+g62d140e'))
    self.assertFalse(
        honey_json_builder.is_valid_version('74.0.0-master.1920+g725ed88'))
    self.assertFalse(
        honey_json_builder.is_valid_version('74.0.0+chromium-74.0.3729.0'))

  # Test Chromium version number matching.
  def test_valid_chromium_version(self):
    self.assertTrue(honey_json_builder.is_valid_chromium_version('74.0.3729.0'))
    self.assertTrue(honey_json_builder.is_valid_chromium_version('74.0.3729.6'))
    self.assertTrue(
        honey_json_builder.is_valid_chromium_version('100.0.59200.602'))

    # self.assertFalse(honey_json_builder.is_valid_version(None))
    self.assertFalse(honey_json_builder.is_valid_chromium_version('foobar'))
    # Must be 4 numbers.
    self.assertFalse(honey_json_builder.is_valid_chromium_version('74.0.3729'))
    self.assertFalse(honey_json_builder.is_valid_chromium_version('74.0.3729.A'))
    # 1st number is max 3 digits with no leading 0's.
    self.assertFalse(
        honey_json_builder.is_valid_chromium_version('7400.0.3729.6'))
    self.assertFalse(honey_json_builder.is_valid_chromium_version('04.0.3729.6'))
    # 2nd number is always 0.
    self.assertFalse(honey_json_builder.is_valid_chromium_version('74.1.3729.6'))
    # 3rd number is max 5 digits with no leading 0's.
    self.assertFalse(honey_json_builder.is_valid_chromium_version('74.0.0372.6'))
    self.assertFalse(
        honey_json_builder.is_valid_chromium_version('74.0.372900.6'))
    # 4rd number is max 3 digits with no leading 0's.
    self.assertFalse(
        honey_json_builder.is_valid_chromium_version('74.0.3729.6000'))
    self.assertFalse(honey_json_builder.is_valid_chromium_version('74.0.3729.06'))
    pass

  # Write builder contents to string and then read in.
  def _verify_write_read(self, builder):
    output = str(builder)
    builder2 = honey_json_builder()
  ...
  def _add_test_file(self,
                     ...
                     attrib_idx=0,
                     shouldfail=False):
    name = honey_json_builder.get_file_name(version, platform, type,
                                          channel) + '.tar.gz'
  ...
  def test_add_multiple_files(self):
    ...
    platforms = honey_json_builder.get_platforms()
    versions = [
        # Old-style version numbers.
    ...
    types = honey_json_builder.get_distrib_types()
  ...
  def test_chromium_version(self):
    ...
    # For old-style Honeycomb version numbers the Git hashes must exist but the rest
    # of the Honeycomb version can be fake.
    # yapf: disable
    versions = (
        # Old-style version numbers.
    ...
    # Test with no query.
    for (honey, chromium) in versions:
      self.assertFalse(builder.has_chromium_version(honey))
      # Valid version specified, so no query sent.
      self.assertEqual(chromium, builder.set_chromium_version(honey, chromium))
      self.assertEqual(chromium, builder.get_chromium_version(honey))
      self.assertTrue(builder.has_chromium_version(honey))
      # Ignore attempts to set invalid version after setting valid version.
      self.assertEqual(chromium, builder.set_chromium_version(honey, None))
