...
def make_libhoneycomb_dll_dylib_impl_parts(name, retval, args):
...
def make_libhoneycomb_dll_dylib_impl_func(func):
  name = func.get_capi_name()
  parts = func.get_capi_parts([], True)
  retval = parts['retval']
  args = parts['args']
  return make_libhoneycomb_dll_dylib_impl_parts(name, retval, args)


def make_libhoneycomb_dll_dylib_impl(header):
  filenames = []
  includes = [
  ...
  for func in header.get_funcs():
    declare, init, impl = make_libhoneycomb_dll_dylib_impl_func(func)
    ptr_declare += declare
    ptr_init += init
    ptr_impl += impl
  ...
  for other in OTHER_HEADERS:
    ...
    for func in funcs:
      declare, init, impl = make_libhoneycomb_dll_dylib_impl_parts(
...
def write_libhoneycomb_dll_dylib_impl(header, file):
  newcontents = make_libhoneycomb_dll_dylib_impl(header)
...
if __name__ == "__main__":
  ...
  sys.stdout.write(make_libhoneycomb_dll_dylib_impl(header))
