...
class honey_api_hash:
  """ Honeycomb API hash calculator """
  ...
  def __init__(self, headerdir, debugdir=None, verbose=False):
    ...
    self.platform_files = {
        # List of includes_win_capi from honey_paths2.gypi.
        "windows": [
            "internal/honey_app_win.h",
            "internal/honey_types_win.h",
        ],
        # List of includes_mac_capi from honey_paths2.gypi.
        "mac": [
            "internal/honey_types_mac.h",
        ],
        # List of includes_linux_capi from honey_paths2.gypi.
        "linux": [
            "internal/honey_types_linux.h",
        ]
    }

    self.included_files = []

    # List of include/ and include/internal/ files from honey_paths2.gypi.
    self.excluded_files = [
        # includes_common
        "honey_api_hash.h",
        "honey_base.h",
        "honey_config.h",
        "honey_version.h",
        "internal/honey_export.h",
        "internal/honey_ptr.h",
        "internal/honey_string_wrappers.h",
        "internal/honey_time_wrappers.h",
        "internal/honey_types_wrappers.h",
        # includes_win
        "honey_sandbox_win.h",
        "internal/honey_win.h",
        # includes_mac
        "honey_application_mac.h",
        "honey_sandbox_mac.h",
        "internal/honey_mac.h",
        # includes_linux
        "internal/honey_linux.h",
    ]
  ...
  def calculate(self):
    ...
    for filename in filenames:
      ...
      # Parse honey_string.h happens in special case: grab only defined HONEYCOMB_STRING_TYPE_xxx declaration
      content_objects = None
      if filename == "internal/honey_string.h":
  ...
  def __parse_string_type(self, content):
    """ Grab defined HONEYCOMB_STRING_TYPE_xxx """
    objects = []
    for m in re.finditer(
        "\n\s*?#\s*?define\s+?(HONEYCOMB_STRING_TYPE_\w+)\s+?.*?\n", content,
        flags=0):
      object = {
          "name": m.group(1),
          "text": m.group(0),
...
if __name__ == "__main__":
  ...
  disc = """
    This utility calculates Honeycomb API hash.
    """
  ...
  calc = honey_api_hash(options.cppheaderdir, options.debugdir, options.verbose)
