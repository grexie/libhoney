...
#include "libhoney/browser/net_service/browser_urlrequest_impl.h"
...
class AuthCallbackImpl : public HoneycombAuthCallback {
 public:
...
void RunCallbackOnIOThread(
    ...
    absl::optional<HoneycombBrowserURLRequest::RequestInfo> url_request_info,
    const net::AuthChallengeInfo& auth_info,
    const GURL& origin_url,
    HoneycombRefPtr<AuthCallbackImpl> callback_impl) {
  ...
  if (browser) {
    ...
    if (client) {
      HoneycombRefPtr<HoneycombRequestHandler> handler = client->GetRequestHandler();
      if (handler) {
...
LoginDelegate::LoginDelegate(const net::AuthChallengeInfo& auth_info,
  ...
  // May be nullptr for requests originating from HoneycombURLRequest.
  HoneycombRefPtr<HoneycombBrowserHostBase> browser;
  if (web_contents) {
    browser = HoneycombBrowserHostBase::GetBrowserForContents(web_contents);
  }

  // |callback| needs to be executed asynchronously.
  HONEYCOMB_POST_TASK(HONEYCOMB_UIT, base::BindOnce(&LoginDelegate::Start,
                                        weak_ptr_factory_.GetWeakPtr(), browser,
                                        auth_info, request_id, origin_url));
}

void LoginDelegate::Continue(const HoneycombString& username,
...
void LoginDelegate::Start(HoneycombRefPtr<HoneycombBrowserHostBase> browser,
                          const net::AuthChallengeInfo& auth_info,
                          const content::GlobalRequestID& request_id,
                          const GURL& origin_url) {
  HONEYCOMB_REQUIRE_UIT();

  auto url_request_info = HoneycombBrowserURLRequest::FromRequestID(request_id);

  if (browser || url_request_info) {
    // AuthCallbackImpl is bound to the current thread.
    HoneycombRefPtr<AuthCallbackImpl> callbackImpl =
    ...
    HONEYCOMB_POST_TASK(HONEYCOMB_IOT, base::BindOnce(&RunCallbackOnIOThread, browser,
