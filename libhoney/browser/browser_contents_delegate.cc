...
#include "libhoney/browser/browser_contents_delegate.h"
...
#include "libhoney/browser/native/cursor_util.h"
...
class HoneycombWidgetHostInterceptor
    : public blink::mojom::WidgetHostInterceptorForTesting,
      public content::RenderWidgetHostObserver {
 public:
  HoneycombWidgetHostInterceptor(HoneycombRefPtr<HoneycombBrowser> browser,
  ...
  HoneycombWidgetHostInterceptor(const HoneycombWidgetHostInterceptor&) = delete;
  HoneycombWidgetHostInterceptor& operator=(const HoneycombWidgetHostInterceptor&) = delete;
 ...
 private:
  HoneycombRefPtr<HoneycombBrowser> const browser_;
...
HoneycombBrowserContentsDelegate::HoneycombBrowserContentsDelegate(
    scoped_refptr<HoneycombBrowserInfo> browser_info)
    : browser_info_(browser_info) {
  DCHECK(browser_info_->browser());
}

void HoneycombBrowserContentsDelegate::ObserveWebContents(
...
void HoneycombBrowserContentsDelegate::AddObserver(Observer* observer) {
  observers_.AddObserver(observer);
}

void HoneycombBrowserContentsDelegate::RemoveObserver(Observer* observer) {
...
content::WebContents* HoneycombBrowserContentsDelegate::OpenURLFromTab(
  ...
  if (auto c = client()) {
    if (auto handler = c->GetRequestHandler()) {
      ...
      cancel = handler->OnOpenURLFromTab(
          ...
          static_cast<honey_window_open_disposition_t>(params.disposition),
          params.user_gesture);
    }
  }
...
void HoneycombBrowserContentsDelegate::LoadingStateChanged(
    content::WebContents* source,
...
void HoneycombBrowserContentsDelegate::UpdateTargetURL(content::WebContents* source,
                                                 const GURL& url) {
  if (auto c = client()) {
...
bool HoneycombBrowserContentsDelegate::DidAddMessageToConsole(
    content::WebContents* source,
  ...
  if (auto c = client()) {
    if (auto handler = c->GetDisplayHandler()) {
      ...
      honey_log_severity_t honey_level = LOGSEVERITY_DEBUG;
      switch (log_level) {
        case blink::mojom::ConsoleMessageLevel::kVerbose:
          honey_level = LOGSEVERITY_DEBUG;
          break;
        case blink::mojom::ConsoleMessageLevel::kInfo:
          honey_level = LOGSEVERITY_INFO;
          break;
        case blink::mojom::ConsoleMessageLevel::kWarning:
          honey_level = LOGSEVERITY_WARNING;
          break;
        case blink::mojom::ConsoleMessageLevel::kError:
          honey_level = LOGSEVERITY_ERROR;
          break;
      }

      return handler->OnConsoleMessage(browser(), honey_level, message, source_id,
                                       line_no);
    }
  }
...
void HoneycombBrowserContentsDelegate::EnterFullscreenModeForTab(
    content::RenderFrameHost* requesting_frame,
    const blink::mojom::FullscreenOptions& options) {
  OnFullscreenModeChange(/*fullscreen=*/true);
}

void HoneycombBrowserContentsDelegate::ExitFullscreenModeForTab(
    content::WebContents* web_contents) {
  OnFullscreenModeChange(/*fullscreen=*/false);
}

void HoneycombBrowserContentsDelegate::CanDownload(
    const GURL& url,
...
HoneycombBrowserContentsDelegate::PreHandleKeyboardEvent(
    content::WebContents* source,
    ...
    if (auto c = client()) {
      if (auto handler = c->GetKeyboardHandler()) {
        ...
        if (browser_util::GetHoneycombKeyEvent(event, honey_event)) {
          honey_event.focus_on_editable_field = focus_on_editable_field_;

          auto event_handle = delegate->GetEventHandle(event);
          bool is_keyboard_shortcut = false;
          bool result = handler->OnPreKeyEvent(
              browser(), honey_event, event_handle, &is_keyboard_shortcut);
          if (result) {
...
bool HoneycombBrowserContentsDelegate::HandleKeyboardEvent(
    content::WebContents* source,
    const content::NativeWebKeyboardEvent& event) {
    ...
    if (auto c = client()) {
      if (auto handler = c->GetKeyboardHandler()) {
        ...
        if (browser_util::GetHoneycombKeyEvent(event, honey_event)) {
          ...
          auto event_handle = delegate->GetEventHandle(event);
          if (handler->OnKeyEvent(browser(), honey_event, event_handle)) {
            return true;
          }
        }
      }
...
void HoneycombBrowserContentsDelegate::RenderFrameCreated(
...
void HoneycombBrowserContentsDelegate::RenderFrameHostChanged(
...
void HoneycombBrowserContentsDelegate::RenderFrameHostStateChanged(
...
void HoneycombBrowserContentsDelegate::RenderFrameDeleted(
    content::RenderFrameHost* render_frame_host) {
  const auto frame_id =
...
void HoneycombBrowserContentsDelegate::RenderWidgetCreated(
    content::RenderWidgetHost* render_widget_host) {
  new HoneycombWidgetHostInterceptor(browser(), render_widget_host);
}

void HoneycombBrowserContentsDelegate::RenderViewReady() {
...
void HoneycombBrowserContentsDelegate::PrimaryMainFrameRenderProcessGone(
    base::TerminationStatus status) {
  honey_termination_status_t ts = TS_ABNORMAL_TERMINATION;
...
void HoneycombBrowserContentsDelegate::OnFrameFocused(
    content::RenderFrameHost* render_frame_host) {
  HoneycombRefPtr<HoneycombFrameHostImpl> frame = static_cast<HoneycombFrameHostImpl*>(
      browser_info_->GetFrameForHost(render_frame_host).get());
  if (!frame || frame->IsFocused()) {
    return;
  }

  HoneycombRefPtr<HoneycombFrameHostImpl> previous_frame = focused_frame_;
  if (frame->IsMain()) {
    focused_frame_ = nullptr;
  } else {
...
void HoneycombBrowserContentsDelegate::PrimaryMainDocumentElementAvailable() {
  has_document_ = true;
...
void HoneycombBrowserContentsDelegate::LoadProgressChanged(double progress) {
  if (auto c = client()) {
    if (auto handler = c->GetDisplayHandler()) {
...
void HoneycombBrowserContentsDelegate::DidStopLoading() {
...
void HoneycombBrowserContentsDelegate::DidFinishNavigation(
...
void HoneycombBrowserContentsDelegate::DidFailLoad(
    content::RenderFrameHost* render_frame_host,
...
void HoneycombBrowserContentsDelegate::DidFinishLoad(
    content::RenderFrameHost* render_frame_host,
    const GURL& validated_url) {
  auto frame = browser_info_->GetFrameForHost(render_frame_host);
  frame->RefreshAttributes();
...
void HoneycombBrowserContentsDelegate::TitleWasSet(content::NavigationEntry* entry) {
...
void HoneycombBrowserContentsDelegate::DidUpdateFaviconURL(
    content::RenderFrameHost* render_frame_host,
    const std::vector<blink::mojom::FaviconURLPtr>& candidates) {
  if (auto c = client()) {
    if (auto handler = c->GetDisplayHandler()) {
      std::vector<HoneycombString> icon_urls;
...
void HoneycombBrowserContentsDelegate::OnWebContentsFocused(
    content::RenderWidgetHost* render_widget_host) {
  if (auto c = client()) {
...
void HoneycombBrowserContentsDelegate::OnFocusChangedInPage(
...
void HoneycombBrowserContentsDelegate::WebContentsDestroyed() {
...
void HoneycombBrowserContentsDelegate::Observe(
    int type,
...
bool HoneycombBrowserContentsDelegate::OnSetFocus(honey_focus_source_t source) {
...
HoneycombRefPtr<HoneycombClient> HoneycombBrowserContentsDelegate::client() const {
  if (auto b = browser()) {
...
HoneycombRefPtr<HoneycombBrowser> HoneycombBrowserContentsDelegate::browser() const {
  return browser_info_->browser();
}

HoneycombBrowserPlatformDelegate* HoneycombBrowserContentsDelegate::platform_delegate()
    const {
...
void HoneycombBrowserContentsDelegate::OnAddressChange(const GURL& url) {
  if (auto c = client()) {
    if (auto handler = c->GetDisplayHandler()) {
...
void HoneycombBrowserContentsDelegate::OnLoadStart(
  ...
  if (auto c = client()) {
    if (auto handler = c->GetLoadHandler()) {
      ...
      handler->OnLoadStart(browser(), frame,
                           static_cast<honey_transition_type_t>(transition_type));
    }
  }
}

void HoneycombBrowserContentsDelegate::OnLoadEnd(HoneycombRefPtr<HoneycombFrame> frame,
                                           const GURL& url,
...
void HoneycombBrowserContentsDelegate::OnLoadError(HoneycombRefPtr<HoneycombFrame> frame,
                                             const GURL& url,
                                             int error_code) {
  if (auto c = client()) {
    if (auto handler = c->GetLoadHandler()) {
      ...
      handler->OnLoadError(browser(), frame,
                           static_cast<honey_errorcode_t>(error_code),
                           net::ErrorToShortString(error_code), url.spec());
    }
  }
}

void HoneycombBrowserContentsDelegate::OnTitleChange(const std::u16string& title) {
  if (auto c = client()) {
    if (auto handler = c->GetDisplayHandler()) {
...
void HoneycombBrowserContentsDelegate::OnFullscreenModeChange(bool fullscreen) {
...
void HoneycombBrowserContentsDelegate::OnStateChanged(State state_changed) {
  for (auto& observer : observers_) {
