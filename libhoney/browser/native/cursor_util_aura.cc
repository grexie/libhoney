// Copyright 2023 The Honeycomb Authors. Portions copyright
...
display::Display GetDisplay(HoneycombRefPtr<HoneycombBrowser> browser) {
  if (auto web_contents =
          static_cast<HoneycombBrowserHostBase*>(browser.get())->GetWebContents()) {
...
inline honey_cursor_handle_t GetCursorHandleImpl(CursorType* cursor) {
...
inline honey_cursor_handle_t GetCursorHandleImpl(CursorType* cursor) {
  return static_cast<honey_cursor_handle_t>(cursor->xcursor());
}
#elif BUILDFLAG(IS_OZONE)
using CursorType = ui::BitmapCursor;
inline honey_cursor_handle_t GetCursorHandleImpl(CursorType* cursor) {
  return static_cast<honey_cursor_handle_t>(cursor->platform_data());
}
#else
