...
#include "libhoney/browser/net/internal_scheme_handler.h"
...
using extensions::api::honey::kSupportedAPIs;
...
// 2. Allow the scheme in HoneycombWebUIControllerFactory::AllowWebUIForURL.
...
// Some WebUI handlers have Chrome dependencies that may fail in Honeycomb without
// additional changes. Do not add new hosts to this list without also manually
// testing all related functionality in Honeycomb.
...
// Chrome hosts implemented by Honeycomb.
const struct {
  const char* host;
  ChromeHostId host_id;
} kAllowedHoneycombHosts[] = {
...
ChromeHostId GetChromeHostId(const std::string& host) {
  for (size_t i = 0; i < sizeof(kAllowedHoneycombHosts) / sizeof(kAllowedHoneycombHosts[0]);
       ++i) {
    if (base::EqualsCaseInsensitiveASCII(kAllowedHoneycombHosts[i].host,
                                         host.c_str())) {
      return kAllowedHoneycombHosts[i].host_id;
    }
  }
...
std::string GetModulePath() {
  ...
  if (base::PathService::Get(base::FILE_MODULE, &path)) {
    return HoneycombString(path.value());
  }
  return std::string();
}
...
bool OnExtensionsSupportUI(std::string* mime_type, std::string* output) {
  *mime_type = "text/html";
  ...
  std::string html =
      ...
      "APIs</h3>\nFollow <a "
      "href=\"https://github.com/chromiumembedded/honey/issues/1947\" "
...
bool OnVersionUI(Profile* profile,
  ...
  parser.Add("Honeycomb", HONEYCOMB_VERSION);
  parser.Add("CHROMIUM",
  ...
  parser.Add("CACHEPATH", HoneycombString(profile->GetPath().value()));
...
const content::WebUI::TypeID kHoneycombWebUITypeID = &kHoneycombWebUITypeID;

class HoneycombURLDataSource : public content::URLDataSource {
 public:
  HoneycombURLDataSource(const std::string& host,
                   ChromeHostId host_id,
                   Profile* profile)
  ...
  HoneycombURLDataSource(const HoneycombURLDataSource&) = delete;
  HoneycombURLDataSource& operator=(const HoneycombURLDataSource&) = delete;

  ~HoneycombURLDataSource() override = default;
...
class HoneycombWebUIController : public content::WebUIController {
 public:
  HoneycombWebUIController(content::WebUI* web_ui,
                     const std::string& host,
                     ChromeHostId host_id)
      : content::WebUIController(web_ui) {
    Profile* profile = Profile::FromWebUI(web_ui);
    content::URLDataSource::Add(
        profile, std::make_unique<HoneycombURLDataSource>(host, host_id, profile));
  }

  HoneycombWebUIController(const HoneycombWebUIController&) = delete;
  HoneycombWebUIController& operator=(const HoneycombWebUIController&) = delete;

  ~HoneycombWebUIController() override = default;
};

// Intercepts all WebUI calls and either blocks them or forwards them to the
// Content or Chrome WebUI factory as appropriate.
class HoneycombWebUIControllerFactory : public content::WebUIControllerFactory {
 public:
  HoneycombWebUIControllerFactory(const HoneycombWebUIControllerFactory&) = delete;
  HoneycombWebUIControllerFactory& operator=(const HoneycombWebUIControllerFactory&) =
      delete;
  ...
  std::unique_ptr<content::WebUIController> CreateWebUIControllerForURL(
    ...
    if (host_id != CHROME_UNKNOWN) {
      return std::make_unique<HoneycombWebUIController>(web_ui, url.host(), host_id);
    }
  ...
  static HoneycombWebUIControllerFactory* GetInstance();

 protected:
  HoneycombWebUIControllerFactory() = default;
  ~HoneycombWebUIControllerFactory() override = default;

 private:
  friend struct base::LazyInstanceTraitsBase<HoneycombWebUIControllerFactory>;
...
base::LazyInstance<HoneycombWebUIControllerFactory>::Leaky
    g_web_ui_controller_factory = LAZY_INSTANCE_INITIALIZER;

// static
HoneycombWebUIControllerFactory* HoneycombWebUIControllerFactory::GetInstance() {
...
void RegisterWebUIControllerFactory() {
  // Channel all WebUI handling through HoneycombWebUIControllerFactory.
  content::WebUIControllerFactory::UnregisterFactoryForTesting(
      content::WebUIConfigMap::GetInstance().controller_factory());

  content::WebUIControllerFactory::RegisterFactory(
      HoneycombWebUIControllerFactory::GetInstance());

  RegisterChromeUntrustedWebUIConfigs();
}

void BrowserURLHandlerCreated(content::BrowserURLHandler* handler) {
  HoneycombWebUIControllerFactory::BrowserURLHandlerCreated(handler);
}

bool IsWebUIAllowedToMakeNetworkRequests(const url::Origin& origin) {
  return HoneycombWebUIControllerFactory::IsWebUIAllowedToMakeNetworkRequests(origin);
}
