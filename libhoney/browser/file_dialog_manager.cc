...
#include "libhoney/browser/file_dialog_manager.h"
...
class HoneycombFileDialogCallbackImpl : public HoneycombFileDialogCallback {
 public:
  using CallbackType = HoneycombFileDialogManager::RunFileChooserCallback;

  explicit HoneycombFileDialogCallbackImpl(CallbackType callback)
      : callback_(std::move(callback)) {}

  ~HoneycombFileDialogCallbackImpl() override {
    if (!callback_.is_null()) {
      ...
      if (HONEYCOMB_CURRENTLY_ON_UIT()) {
        ...
        HONEYCOMB_POST_TASK(HONEYCOMB_UIT,
                      base::BindOnce(&HoneycombFileDialogCallbackImpl::CancelNow,
                                     std::move(callback_)));
      }
    }
  }
  ...
  void Continue(const std::vector<HoneycombString>& file_paths) override {
    if (HONEYCOMB_CURRENTLY_ON_UIT()) {
      if (!callback_.is_null()) {
        ...
        if (!file_paths.empty()) {
          std::vector<HoneycombString>::const_iterator it = file_paths.begin();
      ...
      HONEYCOMB_POST_TASK(HONEYCOMB_UIT,
                    base::BindOnce(&HoneycombFileDialogCallbackImpl::Continue, this,
                                   file_paths));
    }
  }
  ...
  void Cancel() override {
    if (HONEYCOMB_CURRENTLY_ON_UIT()) {
      ...
      HONEYCOMB_POST_TASK(HONEYCOMB_UIT,
                    base::BindOnce(&HoneycombFileDialogCallbackImpl::Cancel, this));
    }
  }
 ...
 private:
  ...
  IMPLEMENT_REFCOUNTING(HoneycombFileDialogCallbackImpl);
};

void RunFileDialogDismissed(HoneycombRefPtr<HoneycombRunFileDialogCallback> callback,
                            const std::vector<base::FilePath>& file_paths) {
  std::vector<HoneycombString> paths;
...
class HoneycombFileSelectListener : public content::FileSelectListener {
 public:
  using CallbackType = HoneycombFileDialogManager::RunFileChooserCallback;

  explicit HoneycombFileSelectListener(CallbackType callback)
      : callback_(std::move(callback)) {}

 private:
  ~HoneycombFileSelectListener() override = default;
...
class HoneycombSelectFileDialogListener : public ui::SelectFileDialog::Listener {
 public:
  HoneycombSelectFileDialogListener(ui::SelectFileDialog::Listener* listener,
                              void* params,
                              base::OnceClosure callback)
      : listener_(listener), params_(params), callback_(std::move(callback)) {}

  HoneycombSelectFileDialogListener(const HoneycombSelectFileDialogListener&) = delete;
  HoneycombSelectFileDialogListener& operator=(const HoneycombSelectFileDialogListener&) =
      delete;
 ...
 private:
  ~HoneycombSelectFileDialogListener() override = default;
...
HoneycombFileDialogManager::HoneycombFileDialogManager(HoneycombBrowserHostBase* browser)
    : browser_(browser) {}

HoneycombFileDialogManager::~HoneycombFileDialogManager() = default;

void HoneycombFileDialogManager::Destroy() {
  if (dialog_listener_) {
...
void HoneycombFileDialogManager::RunFileDialog(
  ...
  if (!accept_filters.empty()) {
    std::vector<HoneycombString>::const_iterator it = accept_filters.begin();
...
void HoneycombFileDialogManager::RunFileChooser(
  ...
  // Make sure we get native files in HoneycombFileSelectListener.
  ...
  // dialogs by passing the |run_from_honey=true| flag. FileSelectHelper uses
  // ui::SelectFileDialog::Create internally and that call will be intercepted
  // by HoneycombSelectFileDialogFactory, resulting in call to RunSelectFile below.
  // See related comments on HoneycombSelectFileDialogFactory.
  FileSelectHelper::RunFileChooser(
      browser_->GetWebContents()->GetPrimaryMainFrame(),
      base::MakeRefCounted<HoneycombFileSelectListener>(std::move(callback)),
      new_params, /*run_from_honey=*/true);
}

void HoneycombFileDialogManager::RunSelectFile(
  ...
  auto callback =
      base::BindOnce(&HoneycombFileDialogManager::SelectFileDoneByDelegateCallback,
  ...
  dialog_listener_ = new HoneycombSelectFileDialogListener(
      listener, params,
      base::BindOnce(&HoneycombFileDialogManager::SelectFileDoneByListenerCallback,
                     weak_ptr_factory_.GetWeakPtr(),
                     /*listener_destroyed=*/false));

  // This call will not be intercepted by HoneycombSelectFileDialogFactory due to the
  // |run_from_honey=true| flag.
  // See related comments on HoneycombSelectFileDialogFactory.
...
void HoneycombFileDialogManager::SelectFileListenerDestroyed(
    ui::SelectFileDialog::Listener* listener) {
  HONEYCOMB_REQUIRE_UIT();
  DCHECK(listener);
...
HoneycombFileDialogManager::RunFileChooserCallback
HoneycombFileDialogManager::MaybeRunDelegate(
    const blink::mojom::FileChooserParams& params,
    RunFileChooserCallback callback) {
  if (auto client = browser_->client()) {
    if (auto handler = browser_->client()->GetDialogHandler()) {
      ...
      HoneycombRefPtr<HoneycombFileDialogCallbackImpl> callbackImpl(
          new HoneycombFileDialogCallbackImpl(std::move(callback)));
      const bool handled = handler->OnFileDialog(
          browser_, static_cast<honey_file_dialog_mode_t>(mode), params.title,
...
void HoneycombFileDialogManager::SelectFileDoneByDelegateCallback(
    ui::SelectFileDialog::Listener* listener,
    void* params,
...
void HoneycombFileDialogManager::SelectFileDoneByListenerCallback(
    bool listener_destroyed) {
  HONEYCOMB_REQUIRE_UIT();

  // Avoid re-entrancy of this method. HoneycombSelectFileDialogListener callbacks to
  ...
  // HoneycombSelectFileDialogListener::Destroy(). Similarly, the below call to
  // Cancel() may trigger another execution from
  // HoneycombSelectFileDialogListener::Destroy().
  if (!dialog_listener_) {
    return;
  }
