...
#include "libhoney/browser/media_stream_registrar.h"
...
class HoneycombMediaAccessQuery {
 public:
  using CallbackType = content::MediaResponseCallback;

  HoneycombMediaAccessQuery(HoneycombBrowserHostBase* const browser,
                      const content::MediaStreamRequest& request,
                      CallbackType&& callback)
      : browser_(browser), request_(request), callback_(std::move(callback)) {}

  HoneycombMediaAccessQuery(HoneycombMediaAccessQuery&& query)
      : browser_(query.browser_),
        request_(query.request_),
        callback_(std::move(query.callback_)) {}
  HoneycombMediaAccessQuery& operator=(HoneycombMediaAccessQuery&& query) {
    browser_ = query.browser_;
    request_ = query.request_;
    callback_ = std::move(query.callback_);
    return *this;
  ...
  HoneycombMediaAccessQuery(const HoneycombMediaAccessQuery&) = delete;
  HoneycombMediaAccessQuery& operator=(const HoneycombMediaAccessQuery&) = delete;

  bool is_null() const { return callback_.is_null(); }

  uint32_t requested_permissions() const {
    int requested_permissions = HONEYCOMB_MEDIA_PERMISSION_NONE;
    if (device_audio_requested()) {
      requested_permissions |= HONEYCOMB_MEDIA_PERMISSION_DEVICE_AUDIO_CAPTURE;
    }
    if (device_video_requested()) {
      requested_permissions |= HONEYCOMB_MEDIA_PERMISSION_DEVICE_VIDEO_CAPTURE;
    }
    if (desktop_audio_requested()) {
      requested_permissions |= HONEYCOMB_MEDIA_PERMISSION_DESKTOP_AUDIO_CAPTURE;
    }
    if (desktop_video_requested()) {
      requested_permissions |= HONEYCOMB_MEDIA_PERMISSION_DESKTOP_VIDEO_CAPTURE;
    }
  ...
  void ExecuteCallback(uint32_t allowed_permissions) {
    ...
    if (allowed_permissions == HONEYCOMB_MEDIA_PERMISSION_NONE) {
 ...
 private:
  ...
  blink::mojom::StreamDevicesSetPtr GetRequestedMediaDevices() const {
    ...
    if (device_audio_requested()) {
      ...
      // given type.
      HoneycombMediaCaptureDevicesDispatcher::GetInstance()->GetRequestedDevice(
  ...
  blink::mojom::StreamDevicesSetPtr GetAllowedMediaDevices(
    ...
    const bool device_audio_allowed =
        allowed_permissions & HONEYCOMB_MEDIA_PERMISSION_DEVICE_AUDIO_CAPTURE;
    const bool device_video_allowed =
        allowed_permissions & HONEYCOMB_MEDIA_PERMISSION_DEVICE_VIDEO_CAPTURE;
    const bool desktop_audio_allowed =
        allowed_permissions & HONEYCOMB_MEDIA_PERMISSION_DESKTOP_AUDIO_CAPTURE;
    const bool desktop_video_allowed =
        allowed_permissions & HONEYCOMB_MEDIA_PERMISSION_DESKTOP_VIDEO_CAPTURE;
...
class HoneycombMediaAccessCallbackImpl : public HoneycombMediaAccessCallback {
 public:
  using CallbackType = HoneycombMediaAccessQuery;

  explicit HoneycombMediaAccessCallbackImpl(CallbackType&& callback)
      : callback_(std::move(callback)) {}

  HoneycombMediaAccessCallbackImpl(const HoneycombMediaAccessCallbackImpl&) = delete;
  HoneycombMediaAccessCallbackImpl& operator=(const HoneycombMediaAccessCallbackImpl&) =
      delete;

  ~HoneycombMediaAccessCallbackImpl() override {
    if (!callback_.is_null()) {
      // The callback is still pending. Cancel it now.
      if (HONEYCOMB_CURRENTLY_ON_UIT()) {
        RunNow(std::move(callback_), HONEYCOMB_MEDIA_PERMISSION_NONE);
      } else {
        HONEYCOMB_POST_TASK(
            HONEYCOMB_UIT,
            base::BindOnce(&HoneycombMediaAccessCallbackImpl::RunNow,
                           std::move(callback_), HONEYCOMB_MEDIA_PERMISSION_NONE));
      }
    }
  }
  ...
  void Continue(uint32_t allowed_permissions) override {
    if (HONEYCOMB_CURRENTLY_ON_UIT()) {
      ...
      HONEYCOMB_POST_TASK(HONEYCOMB_UIT,
                    base::BindOnce(&HoneycombMediaAccessCallbackImpl::Continue, this,
                                   allowed_permissions));
    }
  }

  void Cancel() override { Continue(HONEYCOMB_MEDIA_PERMISSION_NONE); }
 ...
 private:
  ...
  IMPLEMENT_REFCOUNTING(HoneycombMediaAccessCallbackImpl);
};
...
content::MediaResponseCallback RequestMediaAccessPermission(
  ...
  HoneycombMediaAccessQuery query(browser, request, std::move(callback));
  ...
  if (auto client = browser->GetClient()) {
    if (auto handler = client->GetPermissionHandler()) {
      ...
      HoneycombRefPtr<HoneycombMediaAccessCallbackImpl> callbackImpl(
          new HoneycombMediaAccessCallbackImpl(std::move(query)));
  ...
  if (!query.is_null()) {
    if (default_disallow && !handled) {
      ...
      query.ExecuteCallback(HONEYCOMB_MEDIA_PERMISSION_NONE);
    } else {
