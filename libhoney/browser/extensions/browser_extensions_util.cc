...
HoneycombRefPtr<HoneycombBrowserHostBase> GetOwnerBrowserForGlobalId(
  ...
  if (HONEYCOMB_CURRENTLY_ON_UIT()) {
    ...
    scoped_refptr<HoneycombBrowserInfo> info =
        HoneycombBrowserInfoManager::GetInstance()->GetBrowserInfo(global_id,
                                                             is_guest_view);
    if (info.get()) {
      HoneycombRefPtr<HoneycombBrowserHostBase> browser = info->browser();
      if (!browser.get()) {
...
HoneycombRefPtr<HoneycombBrowserHostBase> GetOwnerBrowserForHost(
  ...
  HoneycombRefPtr<HoneycombBrowserHostBase> browser =
      HoneycombBrowserHostBase::GetBrowserForHost(host);
  if (!browser.get() && ExtensionsEnabled()) {
    ...
    if (owner) {
      browser = HoneycombBrowserHostBase::GetBrowserForContents(owner);
...
HoneycombRefPtr<AlloyBrowserHostImpl> GetBrowserForTabId(
  ...
  for (const auto& browser_info :
    ...
    HoneycombRefPtr<AlloyBrowserHostImpl> current_browser =
    ...
    if (current_browser && current_browser->GetIdentifier() == tab_id) {
      ...
      if (HoneycombBrowserContext::FromBrowserContext(
              current_browser->GetBrowserContext()) == honey_browser_context) {
