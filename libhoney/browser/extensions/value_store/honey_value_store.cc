...
#include "libhoney/browser/extensions/value_store/honey_value_store.h"
...
const char kGenericErrorMessage[] = "HoneycombValueStore configured to error";
...
HoneycombValueStore::HoneycombValueStore() = default;
HoneycombValueStore::~HoneycombValueStore() = default;

void HoneycombValueStore::set_status_code(StatusCode status_code) {
  status_ = ValueStore::Status(status_code, kGenericErrorMessage);
}

size_t HoneycombValueStore::GetBytesInUse(const std::string& key) {
  // Let SettingsStorageQuotaEnforcer implement this.
  DCHECK(false) << "Not implemented";
  return 0;
}

size_t HoneycombValueStore::GetBytesInUse(const std::vector<std::string>& keys) {
  // Let SettingsStorageQuotaEnforcer implement this.
  DCHECK(false) << "Not implemented";
  return 0;
}

size_t HoneycombValueStore::GetBytesInUse() {
  // Let SettingsStorageQuotaEnforcer implement this.
  DCHECK(false) << "Not implemented";
  return 0;
}

ValueStore::ReadResult HoneycombValueStore::Get(const std::string& key) {
  return Get(std::vector<std::string>(1, key));
}

ValueStore::ReadResult HoneycombValueStore::Get(
...
ValueStore::WriteResult HoneycombValueStore::Set(WriteOptions options,
...
ValueStore::WriteResult HoneycombValueStore::Remove(const std::string& key) {
  return Remove(std::vector<std::string>(1, key));
}

ValueStore::WriteResult HoneycombValueStore::Remove(
    const std::vector<std::string>& keys) {
  write_count_++;
  if (!status_.ok()) {
    return WriteResult(CreateStatusCopy(status_));
...
ValueStore::WriteResult HoneycombValueStore::Clear() {
