...
#include "libhoney/browser/alloy/alloy_browser_context.h"
...
#include "libhoney/browser/media_access_query.h"
#include "libhoney/browser/osr/osr_util.h"
...
#include "libhoney/common/net/url_util.h"
...
class ShowDevToolsHelper {
 public:
  ShowDevToolsHelper(HoneycombRefPtr<AlloyBrowserHostImpl> browser,
...
// static
HoneycombRefPtr<AlloyBrowserHostImpl> AlloyBrowserHostImpl::Create(
  ...
  scoped_refptr<HoneycombBrowserInfo> info =
      HoneycombBrowserInfoManager::GetInstance()->CreateBrowserInfo(
  ...
  auto request_context_impl =
      static_cast<HoneycombRequestContextImpl*>(create_params.request_context.get());

  HoneycombRefPtr<HoneycombExtension> honey_extension;
  if (create_params.extension) {
    auto honey_browser_context = request_context_impl->GetBrowserContext();
    honey_extension =
        honey_browser_context->GetExtension(create_params.extension->id());
    CHECK(honey_extension);
  }

  auto platform_delegate_ptr = platform_delegate.get();

  HoneycombRefPtr<AlloyBrowserHostImpl> browser = CreateInternal(
...
// static
HoneycombRefPtr<AlloyBrowserHostImpl> AlloyBrowserHostImpl::CreateInternal(
    const HoneycombBrowserSettings& settings,
    HoneycombRefPtr<HoneycombClient> client,
    content::WebContents* web_contents,
    bool own_web_contents,
    scoped_refptr<HoneycombBrowserInfo> browser_info,
    HoneycombRefPtr<AlloyBrowserHostImpl> opener,
  ...
  HoneycombRefPtr<AlloyBrowserHostImpl> browser = new AlloyBrowserHostImpl(
  ...
  if (opener && opener->platform_delegate_) {
    ...
    // result in a call to HoneycombBrowserViewDelegate::OnPopupBrowserViewCreated().
  ...
  // HoneycombBrowserViewDelegate::OnBrowserCreated().
...
// static
HoneycombRefPtr<AlloyBrowserHostImpl> AlloyBrowserHostImpl::GetBrowserForHost(
    const content::RenderViewHost* host) {
  REQUIRE_ALLOY_RUNTIME();
  auto browser = HoneycombBrowserHostBase::GetBrowserForHost(host);
...
// static
HoneycombRefPtr<AlloyBrowserHostImpl> AlloyBrowserHostImpl::GetBrowserForContents(
    const content::WebContents* contents) {
  REQUIRE_ALLOY_RUNTIME();
  auto browser = HoneycombBrowserHostBase::GetBrowserForContents(contents);
  return static_cast<AlloyBrowserHostImpl*>(browser.get());
}

// static
HoneycombRefPtr<AlloyBrowserHostImpl> AlloyBrowserHostImpl::GetBrowserForGlobalId(
    const content::GlobalRenderFrameHostId& global_id) {
  REQUIRE_ALLOY_RUNTIME();
  auto browser = HoneycombBrowserHostBase::GetBrowserForGlobalId(global_id);
  return static_cast<AlloyBrowserHostImpl*>(browser.get());
}
...
void AlloyBrowserHostImpl::CloseBrowser(bool force_close) {
  if (HONEYCOMB_CURRENTLY_ON_UIT()) {
    ...
    HONEYCOMB_POST_TASK(HONEYCOMB_UIT, base::BindOnce(&AlloyBrowserHostImpl::CloseBrowser,
                                          this, force_close));
  }
}
...
HoneycombWindowHandle AlloyBrowserHostImpl::GetWindowHandle() {
  if (is_views_hosted_ && HONEYCOMB_CURRENTLY_ON_UIT()) {
...
HoneycombWindowHandle AlloyBrowserHostImpl::GetOpenerWindowHandle() {
  return opener_;
}

void AlloyBrowserHostImpl::Find(const HoneycombString& searchText,
                                bool forward,
                                bool matchCase,
                                bool findNext) {
  if (!HONEYCOMB_CURRENTLY_ON_UIT()) {
...
void AlloyBrowserHostImpl::StopFinding(bool clearSelection) {
  if (!HONEYCOMB_CURRENTLY_ON_UIT()) {
    HONEYCOMB_POST_TASK(HONEYCOMB_UIT, base::BindOnce(&AlloyBrowserHostImpl::StopFinding,
                                          this, clearSelection));
    return;
  }
...
void AlloyBrowserHostImpl::ShowDevTools(const HoneycombWindowInfo& windowInfo,
                                        HoneycombRefPtr<HoneycombClient> client,
                                        const HoneycombBrowserSettings& settings,
                                        const HoneycombPoint& inspect_element_at) {
  if (!HONEYCOMB_CURRENTLY_ON_UIT()) {
    ...
    HONEYCOMB_POST_TASK(HONEYCOMB_UIT, base::BindOnce(ShowDevToolsWithHelper, helper));
    return;
  }
...
HoneycombRefPtr<HoneycombExtension> AlloyBrowserHostImpl::GetExtension() {
  return extension_;
}
...
void AlloyBrowserHostImpl::WasHidden(bool hidden) {
  ...
  if (!HONEYCOMB_CURRENTLY_ON_UIT()) {
    HONEYCOMB_POST_TASK(HONEYCOMB_UIT,
                  base::BindOnce(&HoneycombBrowserHost::WasHidden, this, hidden));
    return;
  }
...
void AlloyBrowserHostImpl::Invalidate(PaintElementType type) {
  ...
  if (!HONEYCOMB_CURRENTLY_ON_UIT()) {
    HONEYCOMB_POST_TASK(
        HONEYCOMB_UIT, base::BindOnce(&AlloyBrowserHostImpl::Invalidate, this, type));
    return;
  }
...
void AlloyBrowserHostImpl::SendTouchEvent(const HoneycombTouchEvent& event) {
  if (!IsWindowless()) {
    DCHECK(false) << "Window rendering is not disabled";
    return;
  }
  ...
  if (!HONEYCOMB_CURRENTLY_ON_UIT()) {
    HONEYCOMB_POST_TASK(HONEYCOMB_UIT, base::BindOnce(&AlloyBrowserHostImpl::SendTouchEvent,
                                          this, event));
    return;
  }
...
bool AlloyBrowserHostImpl::IsVisible() const {
  ...
  return HoneycombBrowserHostBase::IsVisible();
}
...
void AlloyBrowserHostImpl::DestroyBrowser() {
  ...
  // HoneycombBrowserViewDelegate::OnBrowserDestroyed().
  ...
  HoneycombBrowserHostBase::DestroyBrowser();
}
...
void AlloyBrowserHostImpl::OnSetFocus(honey_focus_source_t source) {
  if (!HONEYCOMB_CURRENTLY_ON_UIT()) {
    HONEYCOMB_POST_TASK(HONEYCOMB_UIT, base::BindOnce(&AlloyBrowserHostImpl::OnSetFocus,
                                          this, source));
    return;
  }
...
void AlloyBrowserHostImpl::FindReply(content::WebContents* web_contents,
                                     ...
                                     bool final_update) {
  auto alloy_delegate =
      static_cast<HoneycombBrowserPlatformDelegateAlloy*>(platform_delegate());
  if (alloy_delegate->HandleFindReply(request_id, number_of_matches,
    ...
    if (client_) {
      if (auto handler = client_->GetFindHandler()) {
        ...
        HoneycombRect rect(details.selection_rect().x(), details.selection_rect().y(),
...
void AlloyBrowserHostImpl::ImeCommitText(const HoneycombString& text,
                                         const HoneycombRange& replacement_range,
...
void AlloyBrowserHostImpl::DragTargetDragOver(
  ...
  if (!HONEYCOMB_CURRENTLY_ON_UIT()) {
    HONEYCOMB_POST_TASK(
        HONEYCOMB_UIT, base::BindOnce(&AlloyBrowserHostImpl::DragTargetDragOver, this,
                                event, allowed_ops));
    return;
  }
...
void AlloyBrowserHostImpl::DragTargetDrop(const HoneycombMouseEvent& event) {
  if (!IsWindowless()) {
    DCHECK(false) << "Window rendering is not disabled";
    return;
  }
  ...
  if (!HONEYCOMB_CURRENTLY_ON_UIT()) {
    HONEYCOMB_POST_TASK(HONEYCOMB_UIT, base::BindOnce(&AlloyBrowserHostImpl::DragTargetDrop,
                                          this, event));
    return;
  }
...
void AlloyBrowserHostImpl::DragSourceEndedAt(
    int x,
    int y,
    HoneycombBrowserHost::DragOperationsMask op) {
  if (!IsWindowless()) {
    DCHECK(false) << "Window rendering is not disabled";
    return;
  }
...
void AlloyBrowserHostImpl::SetAudioMuted(bool mute) {
  if (!HONEYCOMB_CURRENTLY_ON_UIT()) {
    HONEYCOMB_POST_TASK(HONEYCOMB_UIT, base::BindOnce(&AlloyBrowserHostImpl::SetAudioMuted,
                                          this, mute));
    return;
  }
  if (!web_contents()) {
...
void AlloyBrowserHostImpl::CloseContents(content::WebContents* source) {
  ...
  if (client_.get() && (IsWindowless() || !window_destroyed_)) {
    HoneycombRefPtr<HoneycombLifeSpanHandler> handler = client_->GetLifeSpanHandler();
    if (handler.get()) {
  ...
  if (close_browser) {
    ...
    if (!IsWindowless() && !window_destroyed_) {
      ...
      // destroyed.
      HoneycombRefPtr<AlloyBrowserHostImpl> browser(this);
...
bool AlloyBrowserHostImpl::TakeFocus(content::WebContents* source,
                                     bool reverse) {
  if (client_.get()) {
    HoneycombRefPtr<HoneycombFocusHandler> handler = client_->GetFocusHandler();
    if (handler.get()) {
...
bool AlloyBrowserHostImpl::CanDragEnter(content::WebContents* source,
  ...
  HoneycombRefPtr<HoneycombDragHandler> handler;
  if (client_) {
    handler = client_->GetDragHandler();
  }
  if (handler) {
    HoneycombRefPtr<HoneycombDragDataImpl> drag_data(new HoneycombDragDataImpl(data));
    drag_data->SetReadOnly(true);
    if (handler->OnDragEnter(
            this, drag_data.get(),
            static_cast<HoneycombDragHandler::DragOperationsMask>(mask))) {
      return false;
    }
  }
  return true;
...
void AlloyBrowserHostImpl::GetCustomWebContentsView(
  ...
  HoneycombBrowserInfoManager::GetInstance()->GetCustomWebContentsView(
      target_url,
...
void AlloyBrowserHostImpl::WebContentsCreated(
  ...
  HoneycombBrowserInfoManager::GetInstance()->WebContentsCreated(
      target_url,
      frame_util::MakeGlobalId(opener_render_process_id,
                               opener_render_frame_id),
  ...
  scoped_refptr<HoneycombBrowserInfo> info =
      HoneycombBrowserInfoManager::GetInstance()->CreatePopupBrowserInfo(
  ...
  HoneycombRefPtr<HoneycombRequestContextImpl> request_context = opener->request_context();
  ...
  HoneycombRefPtr<AlloyBrowserHostImpl> browser =
      CreateInternal(settings, client, new_contents, /*own_web_contents=*/false,
                     ...
                     std::move(platform_delegate), /*honey_extension=*/nullptr);
}

content::JavaScriptDialogManager*
AlloyBrowserHostImpl::GetJavaScriptDialogManager(content::WebContents* source) {
  if (!javascript_dialog_manager_) {
    javascript_dialog_manager_.reset(new HoneycombJavaScriptDialogManager(this));
  }
...
void AlloyBrowserHostImpl::RunFileChooser(
  ...
  // This will eventually call into HoneycombFileDialogManager.
...
bool AlloyBrowserHostImpl::ShowContextMenu(
  ...
  if (!menu_manager_.get() && platform_delegate_) {
    menu_manager_.reset(
        new HoneycombMenuManager(this, platform_delegate_->CreateMenuRunner()));
  }
...
void AlloyBrowserHostImpl::ResizeDueToAutoResize(content::WebContents* source,
  ...
  if (client_) {
    HoneycombRefPtr<HoneycombDisplayHandler> handler = client_->GetDisplayHandler();
    if (handler && handler->OnAutoResize(
                       this, HoneycombSize(new_size.width(), new_size.height()))) {
      return;
    }
  }
...
void AlloyBrowserHostImpl::DidFinishNavigation(
  ...
  if (web_contents()) {
    ...
    if (honey_browser_context) {
      honey_browser_context->AddVisitedURLs(
...
void AlloyBrowserHostImpl::StartAudioCapturer() {
  ...
  HoneycombRefPtr<HoneycombAudioHandler> audio_handler = client_->GetAudioHandler();
  if (!audio_handler.get()) {
    return;
  }

  HoneycombAudioParameters params;
  params.channel_layout = HONEYCOMB_CHANNEL_LAYOUT_STEREO;
  ...
  audio_capturer_.reset(new HoneycombAudioCapturer(params, this, audio_handler));
}
...
AlloyBrowserHostImpl::AlloyBrowserHostImpl(
    ...
    std::unique_ptr<HoneycombBrowserPlatformDelegate> platform_delegate,
    HoneycombRefPtr<HoneycombExtension> extension)
    : HoneycombBrowserHostBase(settings,
                         client,
