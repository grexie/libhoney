...
#include "libhoney/browser/download_manager_delegate.h"
#include "libhoney/browser/extensions/extension_system.h"
#include "libhoney/browser/prefs/browser_prefs.h"
#include "libhoney/browser/ssl_host_state_delegate.h"
...
class HoneycombVisitedLinkListener : public visitedlink::VisitedLinkWriter::Listener {
 public:
  HoneycombVisitedLinkListener() { DCHECK(listener_map_.empty()); }

  HoneycombVisitedLinkListener(const HoneycombVisitedLinkListener&) = delete;
  HoneycombVisitedLinkListener& operator=(const HoneycombVisitedLinkListener&) = delete;
...
AlloyBrowserContext::AlloyBrowserContext(
    ...
    : HoneycombBrowserContext(settings) {}
...
void AlloyBrowserContext::Initialize() {
  HoneycombBrowserContext::Initialize();
  ...
  if (extensions_enabled) {
    ...
    // depend on it.
    extension_system_ = static_cast<extensions::HoneycombExtensionSystem*>(
  ...
  visitedlink_listener_ = new HoneycombVisitedLinkListener;
...
void AlloyBrowserContext::Shutdown() {
  HoneycombBrowserContext::Shutdown();
...
void AlloyBrowserContext::RemoveHoneycombRequestContext(
  ...
  HoneycombBrowserContext::RemoveHoneycombRequestContext(context);
}
...
void AlloyBrowserContext::LoadExtension(
  ...
  if (manifest && manifest->GetSize() > 0) {
    HoneycombDictionaryValueImpl* value_impl =
        static_cast<HoneycombDictionaryValueImpl*>(manifest.get());
...
bool AlloyBrowserContext::GetExtensions(std::vector<HoneycombString>& extension_ids) {
  if (!extensions::ExtensionsEnabled()) {
    return false;
  }

  extensions::HoneycombExtensionSystem::ExtensionMap extension_map =
      extension_system()->GetExtensions();
  extensions::HoneycombExtensionSystem::ExtensionMap::const_iterator it =
      extension_map.begin();
...
HoneycombRefPtr<HoneycombExtension> AlloyBrowserContext::GetExtension(
...
bool AlloyBrowserContext::UnloadExtension(const HoneycombString& extension_id) {
...
AlloyBrowserContext::GetDownloadManagerDelegate() {
  if (!download_manager_delegate_) {
    download_manager_delegate_.reset(
        new HoneycombDownloadManagerDelegate(GetDownloadManager()));
  }
...
content::SSLHostStateDelegate* AlloyBrowserContext::GetSSLHostStateDelegate() {
  if (!ssl_host_state_delegate_) {
    ssl_host_state_delegate_.reset(new HoneycombSSLHostStateDelegate());
  }
