...
#ifndef HONEYCOMB_LIBHONEY_BROWSER_BROWSER_INFO_MANAGER_H_
#define HONEYCOMB_LIBHONEY_BROWSER_BROWSER_INFO_MANAGER_H_
#pragma once
...
class HoneycombBrowserInfoManager : public content::RenderProcessHostObserver {
 public:
  HoneycombBrowserInfoManager();

  HoneycombBrowserInfoManager(const HoneycombBrowserInfoManager&) = delete;
  HoneycombBrowserInfoManager& operator=(const HoneycombBrowserInfoManager&) = delete;

  ~HoneycombBrowserInfoManager() override;

  // Returns this singleton instance of this class.
  static HoneycombBrowserInfoManager* GetInstance();

  // Called immediately before a new HoneycombBrowserHost implementation is created
  // directly. In this case |is_popup| will be true only for DevTools browsers.
  scoped_refptr<HoneycombBrowserInfo> CreateBrowserInfo(
      bool is_popup,
      bool is_windowless,
  ...
  scoped_refptr<HoneycombBrowserInfo> CreatePopupBrowserInfo(
  ...
  // Called from HoneycombBrowserManager::GetNewBrowserInfo for delivering
  ...
  // Called from HoneycombBrowserHostBase::DestroyBrowser() when a browser is
  // destroyed.
  void RemoveBrowserInfo(scoped_refptr<HoneycombBrowserInfo> browser_info);

  // Called from HoneycombContext::FinishShutdownOnUIThread() to destroy all browsers.
  void DestroyAllBrowsers();

  // Returns the HoneycombBrowserInfo matching the specified ID or nullptr if no
  // match is found. It is allowed to add new callers of this method but
  // consider using HoneycombBrowserHostBase::GetBrowserForGlobalId() or
  ...
  scoped_refptr<HoneycombBrowserInfo> GetBrowserInfo(
      const content::GlobalRenderFrameHostId& global_id,
      bool* is_guest_view = nullptr);

  // Returns all existing HoneycombBrowserInfo objects.
  using BrowserInfoList = std::list<scoped_refptr<HoneycombBrowserInfo>>;
 ...
 private:
  ...
  //   HoneycombBrowserHostBase::MaybeAllowNavigation implementation.
  ...
  //   Creates the HoneycombBrowserHost representation for the popup.
  // - AddWebContents (UIT) (chrome runtime only):
  //   Creates the Browser or tab representation for the popup.
  // - HoneycombBrowserManager::GetNewBrowserInfo (IOT)
  ...
  scoped_refptr<HoneycombBrowserInfo> GetBrowserInfoInternal(
      const content::GlobalRenderFrameHostId& global_id,
      bool* is_guest_view);
...
#endif  // HONEYCOMB_LIBHONEY_BROWSER_BROWSER_INFO_H_
