...
#include "libhoney/browser/chrome/chrome_browser_delegate.h"
...
std::unique_ptr<content::WebContents> ChromeBrowserDelegate::AddWebContents(
  ...
  if (HoneycombBrowserInfoManager::GetInstance()->AddWebContents(
          new_contents.get())) {
...
void ChromeBrowserDelegate::SetAsDelegate(content::WebContents* web_contents,
  ...
  auto platform_delegate = HoneycombBrowserPlatformDelegate::Create(create_params_);
  CHECK(platform_delegate);

  auto browser_info = HoneycombBrowserInfoManager::GetInstance()->CreateBrowserInfo(
    ...
    if (auto client = browser->GetClient()) {
      if (auto handler = client->GetCommandHandler()) {
        return handler->OnChromeCommand(
            ...
            static_cast<honey_window_open_disposition_t>(disposition));
      }
    }
  }
  return false;
...
bool ChromeBrowserDelegate::IsPageActionIconVisible(
  ...
  static_assert(static_cast<int>(HONEYCOMB_CPAIT_MAX_VALUE) ==
  ...
  if (auto client = create_params_.client) {
    if (auto handler = client->GetCommandHandler()) {
      return handler->IsChromePageActionIconVisible(
          static_cast<honey_chrome_page_action_icon_type_t>(icon_type));
    }
  }
  return true;
}
...
bool ChromeBrowserDelegate::IsToolbarButtonVisible(
  ...
  static_assert(static_cast<int>(HONEYCOMB_CTBT_MAX_VALUE) ==
  ...
  if (auto client = create_params_.client) {
    if (auto handler = client->GetCommandHandler()) {
      return handler->IsChromeToolbarButtonVisible(
          static_cast<honey_chrome_toolbar_button_type_t>(button_type));
    }
  }
  return true;
}
...
bool ChromeBrowserDelegate::SupportsFramelessPictureInPicture() const {
  ...
  if (opener_host_) {
    if (auto chrome_browser_view = opener_host_->chrome_browser_view()) {
      if (auto honey_delegate = chrome_browser_view->honey_delegate()) {
        frameless_pip_ = honey_delegate->UseFramelessWindowForPictureInPicture(
            chrome_browser_view->honey_browser_view());
      }
    }
  }
...
void ChromeBrowserDelegate::WindowFullscreenStateChanged() {
  ...
  // notified asynchronously via HoneycombNativeWidgetMac callbacks.
#if !BUILDFLAG(IS_MAC)
  if (auto browser = ChromeBrowserHostImpl::GetBrowserForBrowser(browser_)) {
    if (auto chrome_browser_view = browser->chrome_browser_view()) {
      auto* honey_window = chrome_browser_view->honey_browser_view()->honey_window();
      if (auto* delegate = honey_window->delegate()) {
        // Give the HoneycombWindowDelegate a chance to handle the event.
        delegate->OnWindowFullscreenTransition(honey_window,
                                               /*is_completed=*/true);
      }
    }
  }
...
void ChromeBrowserDelegate::CreateBrowser(
  ...
  // Check if chrome and Honeycomb are using the same browser context.
  // TODO(chrome-runtime): Verify if/when this might occur.
  auto chrome_browser_context =
      HoneycombBrowserContext::FromBrowserContext(browser_->create_params().profile);
  ...
  HoneycombRefPtr<ChromeBrowserHostImpl> browser_host =
...
HoneycombBrowserContentsDelegate* ChromeBrowserDelegate::GetDelegateForWebContents(
    content::WebContents* web_contents) {
  auto browser_host =
      ChromeBrowserHostImpl::GetBrowserForContents(web_contents);
  if (browser_host) {
...
std::unique_ptr<BrowserDelegate> BrowserDelegate::Create(
    Browser* browser,
    scoped_refptr<CreateParams> honey_params,
    const Browser* opener) {
