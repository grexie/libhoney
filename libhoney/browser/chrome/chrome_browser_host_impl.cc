...
#include "libhoney/browser/chrome/views/chrome_browser_view.h"

// static
HoneycombRefPtr<ChromeBrowserHostImpl> ChromeBrowserHostImpl::Create(
  ...
  if (url.is_empty()) {
    ...
    // the current Honeycomb behavior of not navigating at all. Use a special URL that
...
// static
HoneycombRefPtr<ChromeBrowserHostImpl> ChromeBrowserHostImpl::GetBrowserForHost(
...
// static
HoneycombRefPtr<ChromeBrowserHostImpl> ChromeBrowserHostImpl::GetBrowserForContents(
...
// static
HoneycombRefPtr<ChromeBrowserHostImpl> ChromeBrowserHostImpl::GetBrowserForGlobalId(
...
// static
HoneycombRefPtr<ChromeBrowserHostImpl> ChromeBrowserHostImpl::GetBrowserForBrowser(
...
void ChromeBrowserHostImpl::AddNewContents(
  ...
  HoneycombBrowserCreateParams params;
...
void ChromeBrowserHostImpl::OnWebContentsDestroyed(
  ...
  // The exception is if the browser still exists at HoneycombShutdown, in which
  // case DestroyBrowser() will be called first via
  // HoneycombBrowserInfoManager::DestroyAllBrowsers().
...
void ChromeBrowserHostImpl::OnSetFocus(honey_focus_source_t source) {
  if (!HONEYCOMB_CURRENTLY_ON_UIT()) {
    HONEYCOMB_POST_TASK(HONEYCOMB_UIT, base::BindOnce(&ChromeBrowserHostImpl::OnSetFocus,
                                          this, source));
    return;
  }
...
void ChromeBrowserHostImpl::CloseBrowser(bool force_close) {
  ...
  HONEYCOMB_POST_TASK(HONEYCOMB_UIT, base::BindOnce(&ChromeBrowserHostImpl::DoCloseBrowser,
                                        this, force_close));
}
...
HoneycombWindowHandle ChromeBrowserHostImpl::GetWindowHandle() {
  if (HONEYCOMB_CURRENTLY_ON_UIT()) {
...
HoneycombWindowHandle ChromeBrowserHostImpl::GetOpenerWindowHandle() {
  NOTIMPLEMENTED();
  return kNullWindowHandle;
}

void ChromeBrowserHostImpl::Find(const HoneycombString& searchText,
                                 bool forward,
                                 bool matchCase,
                                 bool findNext) {
  NOTIMPLEMENTED();
...
void ChromeBrowserHostImpl::ShowDevTools(const HoneycombWindowInfo& windowInfo,
...
void ChromeBrowserHostImpl::SendTouchEvent(const HoneycombTouchEvent& event) {
  NOTIMPLEMENTED();
}
...
void ChromeBrowserHostImpl::ImeCommitText(const HoneycombString& text,
                                          const HoneycombRange& replacement_range,
...
void ChromeBrowserHostImpl::DragTargetDragOver(const HoneycombMouseEvent& event,
                                               DragOperationsMask allowed_ops) {
  NOTIMPLEMENTED();
}
...
void ChromeBrowserHostImpl::DragTargetDrop(const HoneycombMouseEvent& event) {
  NOTIMPLEMENTED();
}
...
HoneycombRefPtr<HoneycombExtension> ChromeBrowserHostImpl::GetExtension() {
  return nullptr;
}
...
bool ChromeBrowserHostImpl::Navigate(const content::OpenURLParams& params) {
  ...
  if (GetCurrentTabIndex() == TabStripModel::kNoTab) {
    ...
    return HoneycombBrowserHostBase::Navigate(params);
  }
  ...
  if (browser_) {
    ...
    // 1. It doesn't trigger a call to HoneycombRequestHandler::OnOpenURLFromTab, and
    // 2. It navigates in this HoneycombBrowserHost's WebContents instead of
...
Browser* ChromeBrowserHostImpl::CreateBrowser(
  ...
  HoneycombRefPtr<HoneycombRequestContextImpl> request_context_impl =
  ...
  CHECK(honey_browser_context);
  auto profile = honey_browser_context->AsProfile();
  CHECK(profile);

  Browser::CreateParams chrome_params =
      Browser::CreateParams(profile, /*user_gesture=*/false);

  // Pass |params| to honey::BrowserDelegate::Create from the Browser constructor.
  chrome_params.honey_params = base::MakeRefCounted<DelegateCreateParams>(params);
  ...
  if (params.browser_view) {
    ...
    auto view_impl =
        static_cast<HoneycombBrowserViewImpl*>(params.browser_view.get());
...
void ChromeBrowserHostImpl::Attach(content::WebContents* web_contents,
  ...
  if (opener && opener->platform_delegate_) {
    ...
    // HoneycombBrowserHost::GetWindowHandle() will return kNullWindowHandle in
...
void ChromeBrowserHostImpl::SetBrowser(Browser* browser) {
  HONEYCOMB_REQUIRE_UIT();
  browser_ = browser;
  static_cast<HoneycombBrowserPlatformDelegateChrome*>(platform_delegate_.get())
