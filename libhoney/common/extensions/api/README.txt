...
1. Add libhoney/common/extensions/api/<api>.idl or .json file which defines the
   API.
2. Add <api>.idl or .json to the 'schema_sources' list in
   libhoney/common/extensions/api/BUILD.gn. Serialization code will be
   generated based on this list in step 4.
3. Add libhoney/browser/extensions/api/<api>/<api>_api.[h|cc] class implementation
   files and associated entries to the 'libhoney_static' target in BUILD.gn.
4. Run the honey_create_projects script and build to generate the
   honey/libhoney/common/extensions/api/<api>.h file and other serialization code
   required by the extensions system.
5. Add an entry in the libhoney/common/extensions/api/_*_features.json files if
   necessary [1].
6. Add an entry in the libhoney/common/extensions/api/*_manifest_overlay.json
...
1. Register the API in libhoney/browser/extensions/chrome_api_registration.cc
