...
honey_dom_node_type_t GetNodeType(const blink::WebNode& node) {
...
std::unique_ptr<HoneycombObserverRegistration>
RegisterExecutionContextLifecycleStateObserver(
  ...
  class Observer : public blink::GarbageCollected<Observer>,
   ...
   private:
    HoneycombExecutionContextLifecycleStateObserver* observer_;
  };

  class Registration : public HoneycombObserverRegistration {
   public:
...
struct HoneycombScriptForbiddenScope::Impl {
  blink::ScriptForbiddenScope scope_;
};

HoneycombScriptForbiddenScope::HoneycombScriptForbiddenScope() : impl_(new Impl()) {}

HoneycombScriptForbiddenScope::~HoneycombScriptForbiddenScope() {}
