...
TEST(ZipReaderTest, Read) {
  ...
  HoneycombRefPtr<HoneycombStreamReader> stream(
      HoneycombStreamReader::CreateForData(g_test_zip, sizeof(g_test_zip) - 1));
...
// Test HoneycombZipArchive object.
TEST(ZipReaderTest, ReadArchive) {
  ...
  HoneycombRefPtr<HoneycombZipArchive> archive(new HoneycombZipArchive());

  ASSERT_EQ(archive->Load(stream, HoneycombString(), false), (size_t)5);
  ...
  HoneycombRefPtr<HoneycombZipArchive::File> file;
  ...
  // Test stream reading.
  HoneycombRefPtr<HoneycombStreamReader> reader(file->GetStreamReader());
