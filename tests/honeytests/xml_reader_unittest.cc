...
TEST(XmlReaderTest, Read) {
  ...
  HoneycombRefPtr<HoneycombStreamReader> stream(
      HoneycombStreamReader::CreateForData(g_test_xml, sizeof(g_test_xml) - 1));
  ASSERT_TRUE(stream.get() != nullptr);

  // Create the XML reader.
  HoneycombRefPtr<HoneycombXmlReader> reader(HoneycombXmlReader::Create(
...
TEST(XmlReaderTest, ReadError) {
  ...
  // Create the stream reader.
  HoneycombRefPtr<HoneycombStreamReader> stream(
      HoneycombStreamReader::CreateForData(test_str, sizeof(test_str) - 1));
  ASSERT_TRUE(stream.get() != nullptr);
...
TEST(XmlReaderTest, ObjectLoad) {
  ...
  // Create the XML reader.
  HoneycombRefPtr<HoneycombXmlObject> object(new HoneycombXmlObject("object"));
  ...
  HoneycombRefPtr<HoneycombXmlObject> obj(object->FindChild("ns:obj"));
  ASSERT_TRUE(obj.get());
  ASSERT_TRUE(obj->HasChildren());
  ASSERT_EQ(obj->GetChildCount(), (size_t)4);

  HoneycombRefPtr<HoneycombXmlObject> obj_child(obj->FindChild("ns:objC"));
  ...
  HoneycombXmlObject::ObjectVector obj_children;
  ASSERT_EQ(obj->GetChildren(obj_children), (size_t)4);
  ASSERT_EQ(obj_children.size(), (size_t)4);

  HoneycombXmlObject::ObjectVector::const_iterator it = obj_children.begin();
  for (int ct = 0; it != obj_children.end(); ++it, ++ct) {
    ...
    if (ct == 0) {
      ...
      HoneycombXmlObject::AttributeMap attribs;
...
TEST(XmlReaderTest, ObjectLoadError) {
  // Test start/end tag mismatch error.
  {
    ...
    // Create the stream reader.
    HoneycombRefPtr<HoneycombStreamReader> stream(
        HoneycombStreamReader::CreateForData(error_xml, sizeof(error_xml) - 1));
    ASSERT_TRUE(stream.get() != nullptr);
