...
// Verify Set/Get methods for HoneycombRequest, HoneycombPostData and HoneycombPostDataElement.
TEST(RequestTest, SetGet) {
  // HoneycombRequest CreateRequest
  ...
  HoneycombString url = "https://tests.com/run.html";
  HoneycombString method = "POST";
  HoneycombRequest::HeaderMap setHeaders, getHeaders;
  setHeaders.insert(std::make_pair("HeaderA", "ValueA"));
  setHeaders.insert(std::make_pair("HeaderB", "ValueB"));

  // HoneycombPostData CreatePostData
  HoneycombRefPtr<HoneycombPostData> postData(HoneycombPostData::Create());
  EXPECT_TRUE(postData.get() != nullptr);

  // HoneycombPostDataElement CreatePostDataElement
  HoneycombRefPtr<HoneycombPostDataElement> element1(HoneycombPostDataElement::Create());
  EXPECT_TRUE(element1.get() != nullptr);
  HoneycombRefPtr<HoneycombPostDataElement> element2(HoneycombPostDataElement::Create());
  EXPECT_TRUE(element2.get() != nullptr);

  // HoneycombPostDataElement SetToFile
  HoneycombString file = "c:\\path\\to\\file.ext";
  element1->SetToFile(file);
  EXPECT_EQ(PDE_TYPE_FILE, element1->GetType());
  EXPECT_EQ(file, element1->GetFile());

  // HoneycombPostDataElement SetToBytes
  ...
  // HoneycombRequest SetURL
  request->SetURL(url);
  EXPECT_EQ(url, request->GetURL());

  // HoneycombRequest SetMethod
  request->SetMethod(method);
  EXPECT_EQ(method, request->GetMethod());

  // HoneycombRequest SetReferrer
  HoneycombString referrer = "https://tests.com/referrer.html";
  HoneycombRequest::ReferrerPolicy policy = REFERRER_POLICY_ORIGIN;
...
TEST(RequestTest, SetGetHeaderByName) {
  ...
  HoneycombRequest::HeaderMap headers, expectedHeaders;
...
void CreateRequest(HoneycombRefPtr<HoneycombRequest>& request) {
  request = HoneycombRequest::Create();
  EXPECT_TRUE(request.get() != nullptr);
  ...
  HoneycombRequest::HeaderMap headers;
...
class RequestSendRecvTestHandler : public TestHandler {
 ...
 private:
  void TestRequest(HoneycombRefPtr<HoneycombRequest> request) {
  ...
  void TestResponse(HoneycombRefPtr<HoneycombResponse> response) {
...
TEST(RequestTest, SendRecv) {
  HoneycombRefPtr<RequestSendRecvTestHandler> handler =
...
static struct TypeExpected {
  ...
  honey_resource_type_t resource_type;
  int expected_count;
} g_type_expected[] = {
...
class TypeExpectations {
 public:
  ...
  bool GotRequest(HoneycombRefPtr<HoneycombRequest> request) {
    ...
    honey_transition_type_t transition_type = request->GetTransitionType();
    honey_resource_type_t resource_type = request->GetResourceType();
 ...
 public:
  ...
  HoneycombRefPtr<HoneycombResourceHandler> GetResourceHandler(
    ...
    if (get_expectations_.GotRequest(request) &&
      ...
      HoneycombPostTask(TID_UI, base::BindOnce(&TypeTestHandler::DestroyTest, this));
    }
...
TEST(RequestTest, ResourceAndTransitionType) {
  HoneycombRefPtr<TypeTestHandler> handler = new TypeTestHandler();
  handler->ExecuteTest();
  ReleaseAndWaitForDestructor(handler);
}
