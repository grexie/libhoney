...
class CorsBrowserTest : public client::ClientAppBrowser::Delegate {
 public:
  ...
  void OnContextInitialized(HoneycombRefPtr<client::ClientAppBrowser> app) override {
    if (IsChromeRuntimeEnabled()) {
      ...
      bool result = HoneycombRequestContext::GetGlobalContext()->SetPreference(
...
struct Resource {
  ...
  void Init(HandlerType request_handler,
    ...
    path = request_path;
    response_data = data;
    response = HoneycombResponse::Create();
  ...
  virtual bool VerifyRequest(HoneycombRefPtr<HoneycombRequest> request) const {
    return true;
  }
};
...
struct TestSetup {
  ...
  Resource* GetResource(HoneycombRefPtr<HoneycombRequest> request) const {
...
class CorsTestHandler : public RoutingTestHandler {
 public:
  ...
  void OnLoadError(HoneycombRefPtr<HoneycombBrowser> browser,
    ...
    const honey_errorcode_t expected_error = resource->response->GetError();
 ...
 protected:
  ...
  void StartServer(base::OnceClosure next_step) {
    if (!HoneycombCurrentlyOn(TID_UI)) {
      HoneycombPostTask(TID_UI, base::BindOnce(&CorsTestHandler::StartServer, this,
                                         std::move(next_step)));
      return;
    }
  ...
  void ClearCookies() {
    ...
    test_request::GetAllCookies(
        HoneycombCookieManager::GetGlobalManager(nullptr), /*delete_cookies=*/true,
...
TEST(CorsTest, BasicServer) {
  ...
  HoneycombRefPtr<CorsTestHandler> handler = new CorsTestHandler(&setup);
  handler->ExecuteTest();
...
struct CookieTestSetup : TestSetup {
  ...
  bool VerifyClearedCookies(
    ...
    const std::string& cookie = HoneycombString(&cookies[0].name).ToString() + "=" +
                                HoneycombString(&cookies[0].value).ToString();
...
struct CookieResource : Resource {
  ...
  bool VerifyRequest(HoneycombRefPtr<HoneycombRequest> request) const override {
