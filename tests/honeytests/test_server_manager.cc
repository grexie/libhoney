...
#include "tests/honeytests/test_server_manager.h"
...
class ObserverRegistration : public HoneycombRegistration {
 public:
  ...
  static void InitializeRegistration(
      HoneycombRefPtr<ObserverRegistration> registration,
      Manager::DoneCallback callback) {
    if (!HoneycombCurrentlyOn(TID_UI)) {
      HoneycombPostTask(TID_UI, base::BindOnce(InitializeRegistration, registration,
                                         std::move(callback)));
      return;
    }
...
void Manager::Start(StartDoneCallback callback, bool https_server) {
  ...
  if (!HoneycombCurrentlyOn(TID_UI)) {
    HoneycombPostTask(TID_UI, base::BindOnce(Manager::Start, std::move(callback),
                                       https_server));
    return;
  }
...
void Manager::Stop(DoneCallback callback, bool https_server) {
  EXPECT_FALSE(callback.is_null());
  if (!HoneycombCurrentlyOn(TID_UI)) {
    HoneycombPostTask(TID_UI, base::BindOnce(Manager::Stop, std::move(callback),
                                       https_server));
    return;
  }
...
// static
HoneycombRefPtr<HoneycombRegistration> Manager::AddObserver(Observer* observer,
...
// static
HoneycombRefPtr<HoneycombRegistration> Manager::AddObserverAndStart(
    Observer* observer,
...
void Manager::OnTestServerRequest(HoneycombRefPtr<HoneycombRequest> request,
