...
class LifeSpanTestHandler : public RoutingTestHandler {
 public:
  ...
  bool OnBeforeUnloadDialog(HoneycombRefPtr<HoneycombBrowser> browser,
    ...
    if (executing_delay_close_) {
      callback->Continue(true, HoneycombString());
      return true;
    }
    ...
    callback->Continue(settings_.accept_before_unload_dialog, HoneycombString());
    return true;
  }
...
TEST(LifeSpanTest, DoCloseAllow) {
  ...
  HoneycombRefPtr<LifeSpanTestHandler> handler = new LifeSpanTestHandler(settings);
  handler->ExecuteTest();
