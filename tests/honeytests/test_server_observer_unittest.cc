...
class TestServerObserver : public test_server::ObserverHelper {
 public:
  ...
  void OnInitialized(const std::string& server_origin) override {
    ...
    config.request_ = HoneycombRequest::Create();
  ...
  void OnRequestResponse(const test_request::State& state) {
    ...
    HoneycombPostTask(TID_UI, base::BindOnce(&TestServerObserver::Shutdown,
                                       weak_ptr_factory_.GetWeakPtr()));
  }
...
void CreateObserverOnUIThread(TestState* state,
  ...
  if (!HoneycombCurrentlyOn(TID_UI)) {
    HoneycombPostTask(TID_UI, base::BindOnce(CreateObserverOnUIThread,
...
void SignalIfDone(HoneycombRefPtr<HoneycombWaitableEvent> event,
                  size_t* count,
                  size_t total) {
  if (++(*count) == total) {
    event->Signal();
...
void Wait(HoneycombRefPtr<HoneycombWaitableEvent> event) {
