...
class MultiQueryManager : public HoneycombMessageRouterBrowserSide::Handler {
 public:
  enum TestType {
  ...
  void OnNotify(HoneycombRefPtr<HoneycombBrowser> browser,
 ...
 private:
  ...
  class NotifyTask : public HoneycombTask {
   public:
  ...
  void OnQueryCompleted(TestType type) {
    ...
    if (auto_complete_count_ + manual_complete_count_ == total_count) {
      running_ = false;
      HoneycombPostTask(TID_UI, new NotifyTask(weak_ptr_factory_.GetWeakPtr(), true));
    }
  }
...
#define MULTI_QUERY_SINGLE_FRAME_TYPE_TEST(name, type, synchronous) \
  TEST(MessageRouterTest, name) {                                   \
    HoneycombRefPtr<MultiQuerySingleFrameTestHandler> handler =           \
...
class MultiQueryMultiHandlerTestHandler : public SingleLoadTestHandler,
 ...
 public:
  ...
  void OnManualQueriesCompleted(MultiQueryManager* manager) override {
    ...
    HoneycombRefPtr<HoneycombMessageRouterBrowserSide> router = GetRouter();
...
TEST(MessageRouterTest, MultiQueryMultiHandler) {
  HoneycombRefPtr<MultiQueryMultiHandlerTestHandler> handler =
...
class MultiQueryManagerMap : public HoneycombMessageRouterBrowserSide::Handler,
                             public MultiQueryManager::Observer {
 public:
  class Observer {
   public:
  ...
  MultiQueryManager* GetManager(HoneycombRefPtr<HoneycombBrowser> browser,
                                HoneycombRefPtr<HoneycombFrame> frame) const {
    const int browser_id = browser->GetIdentifier();
    // Always use the same ID for the main frame.
    const int64_t frame_id = frame->IsMain() ? -1 : frame->GetIdentifier();
...
class MultiQueryMultiFrameTestHandler : public SingleLoadTestHandler,
 ...
 public:
  ...
  void OnMapManualQueriesCompleted(MultiQueryManagerMap* map) override {
    ...
    if (manager_map_.HasAutoQueries()) {
      HoneycombRefPtr<HoneycombFrame> frame = GetBrowser()->GetMainFrame();
...
TEST(MessageRouterTest, MultiQueryMultiFrameSync) {
  HoneycombRefPtr<MultiQueryMultiFrameTestHandler> handler =
...
TEST(MessageRouterTest, MultiQueryMultiBrowserSameOriginSync) {
  HoneycombRefPtr<MultiQueryMultiBrowserTestHandler> handler =
...
TEST(MessageRouterTest, MultiQueryMultiNavigateSameOriginSync) {
  HoneycombRefPtr<MultiQueryMultiNavigateTestHandler> handler =
