...
const HoneycombAudioHandler::ChannelLayout kChannelLayout = HONEYCOMB_CHANNEL_LAYOUT_STEREO;
...
class AudioTestHandler : public TestHandler, public HoneycombAudioHandler {
 public:
  AudioTestHandler() {}
  ...
  HoneycombRefPtr<HoneycombAudioHandler> GetAudioHandler() override { return this; }
...
// HoneycombBrowserHostImpl::OnAudioStateChangedand and
// HoneycombBrowserHostImpl::OnRecentlyAudibleTimerFired). The timeouts in kTestHtml
...
TEST(AudioOutputTest, AudioOutputTest) {
  HoneycombRefPtr<AudioOutputTestHandler> handler = new AudioOutputTestHandler();
  handler->ExecuteTest();
...
TEST(AudioOutputTest, AudioCloseBrowserTest) {
  HoneycombRefPtr<AudioCloseBrowserTest> handler = new AudioCloseBrowserTest();
  handler->ExecuteTest();
  ReleaseAndWaitForDestructor(handler);
}
...
TEST(AudioOutputTest, AudioTogglePlaybackTest) {
  HoneycombRefPtr<AudioTogglePlaybackTest> handler = new AudioTogglePlaybackTest();
  handler->ExecuteTest();
  ReleaseAndWaitForDestructor(handler);
}
