...
TEST(TimeTest, Now) {
  ...
  honey_basetime_t now = honey_basetime_now();
  EXPECT_NE(now.val, 0);
}
...
TEST(TimeTest, TimeToBaseTime) {
  ...
  ASSERT_FALSE(honey_time_to_basetime(nullptr, nullptr));
  ASSERT_FALSE(honey_time_to_basetime(&date, nullptr));
  ASSERT_FALSE(honey_time_to_basetime(nullptr, &basetime));

  ASSERT_TRUE(honey_time_to_basetime(&date, &basetime));
  ASSERT_EQ(basetime.val, 12625826828009000);
}

TEST(TimeTest, BaseTimeToTime) {
  honey_basetime_t basetime{12625826828009000};

  // Choosing some date which should be always representable on any platform.
  honey_time_t date{};

  ASSERT_TRUE(honey_time_from_basetime(basetime, &date) != 0);
...
TEST(TimeTest, InvalidTimeToBaseTime) {
  ...
  honey_time_t date{};
  date.year = 90000;
  honey_basetime_t basetime{999999999};

  ASSERT_FALSE(honey_time_to_basetime(&date, &basetime) != 0);
...
TEST(TimeTest, InvalidBaseTimeToTime) {
  // Unrepresentable.
  honey_basetime_t basetime{std::numeric_limits<int64_t>::max()};
  honey_time_t date{};
  date.year = 999999999;

  ASSERT_FALSE(honey_time_from_basetime(basetime, &date) != 0);
