...
class DownloadSchemeHandler : public HoneycombResourceHandler {
 public:
 ...
 private:
  ...
  size_t offset_;
  HoneycombRefPtr<HoneycombResourceReadCallback> read_callback_;
...
class DownloadSchemeHandlerFactory : public HoneycombSchemeHandlerFactory {
 public:
...
class DownloadTestHandler : public TestHandler {
 public:
  ...
  void RunTestContinue(HoneycombRefPtr<HoneycombRequestContext> request_context) {
    ...
    if (test_mode_ == NAVIGATED || test_mode_ == PENDING) {
      delay_callback_vendor = base::BindRepeating(
          [](HoneycombRefPtr<DownloadTestHandler> self) {
            return base::BindOnce(&DownloadTestHandler::OnDelayCallback, self);
          },
          HoneycombRefPtr<DownloadTestHandler>(this));
    }
    ...
    if (request_context) {
      ...
      HoneycombRegisterSchemeHandlerFactory("https", kTestDomain, scheme_factory);
    }
  ...
  void OnDelayCallback(base::OnceClosure callback) {
    if (!HoneycombCurrentlyOn(TID_UI)) {
      HoneycombPostTask(TID_UI, base::BindOnce(&DownloadTestHandler::OnDelayCallback,
  ...
  void OnBeforeDownload(
    ...
    EXPECT_EQ(HONEYCOMB_DOWNLOAD_INTERRUPT_REASON_NONE,
  ...
  void DestroyTest() override {
    ...
    if (request_context_) {
      ...
      HoneycombRegisterSchemeHandlerFactory("https", kTestDomain, nullptr);
    }
 ...
 private:
  void SendClick(HoneycombRefPtr<HoneycombBrowser> browser, uint32_t modifiers) {
    EXPECT_TRUE(is_clicked());
  ...
  HoneycombRefPtr<HoneycombDownloadItemCallback> download_item_callback_;

  std::string download_url_;
  HoneycombScopedTempDir temp_dir_;
...
// There will be no resulting HoneycombDownloadHandler callbacks.
