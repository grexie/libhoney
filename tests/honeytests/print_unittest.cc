...
// Verify Set/Get methods for HoneycombPrintSettings.
TEST(PrintTest, SettingsSetGet) {
  ...
  HoneycombPrintSettings::PageRangeList page_ranges;
  page_ranges.push_back(HoneycombRange(1, 3));
  page_ranges.push_back(HoneycombRange(5, 6));
  settings->SetPageRanges(page_ranges);
  EXPECT_EQ(page_ranges.size(), settings->GetPageRangesCount());
  HoneycombPrintSettings::PageRangeList page_ranges2;
  ...
  HoneycombPrintSettings::ColorModel color_model = COLOR_MODEL_CMYK;
  ...
  HoneycombPrintSettings::DuplexMode duplex_mode = DUPLEX_MODE_SIMPLEX;
