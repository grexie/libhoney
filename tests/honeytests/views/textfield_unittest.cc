...
void RunTextfieldContents(HoneycombRefPtr<HoneycombWindow> window) {
  HoneycombRefPtr<HoneycombTextfield> textfield = HoneycombTextfield::CreateTextfield(nullptr);
  ...
  EXPECT_EQ(HoneycombRange(0, 0), textfield->GetSelectedRange());
  ...
  // Test select range.
  EXPECT_FALSE(textfield->HasSelection());
  EXPECT_EQ(HoneycombRange(static_cast<uint32_t>(cursor_pos),
                     static_cast<uint32_t>(cursor_pos)),
            textfield->GetSelectedRange());
  textfield->SelectRange(HoneycombRange(0, static_cast<uint32_t>(cursor_pos)));
  EXPECT_TRUE(textfield->HasSelection());
  EXPECT_EQ(HoneycombRange(0, static_cast<uint32_t>(cursor_pos)),
            textfield->GetSelectedRange());
  ...
  EXPECT_TRUE(textfield->IsCommandEnabled(HONEYCOMB_TFC_SELECT_ALL));
  textfield->ExecuteCommand(HONEYCOMB_TFC_SELECT_ALL);
  EXPECT_TRUE(textfield->HasSelection());
  EXPECT_EQ(HoneycombRange(0, static_cast<uint32_t>(cursor_pos)),
            textfield->GetSelectedRange());
  EXPECT_EQ(cursor_pos, textfield->GetCursorPosition());
...
void TextfieldContentsImpl(HoneycombRefPtr<HoneycombWaitableEvent> event) {
...
void RunTextfieldStyle(HoneycombRefPtr<HoneycombWindow> window) {
  HoneycombRefPtr<HoneycombTextfield> textfield = HoneycombTextfield::CreateTextfield(nullptr);
  EXPECT_TRUE(textfield.get());
  ...
  textfield->SetText(kText);
  textfield->ApplyTextColor(color, HoneycombRange(0, 5));
  textfield->ApplyTextStyle(HONEYCOMB_TEXT_STYLE_BOLD, true, HoneycombRange(0, 5));
...
void TextfieldStyleImpl(HoneycombRefPtr<HoneycombWaitableEvent> event) {
  auto config = std::make_unique<TestWindowDelegate::Config>();
...
class TestTextfieldDelegate : public HoneycombTextfieldDelegate {
 public:
  ...
  bool OnKeyEvent(HoneycombRefPtr<HoneycombTextfield> textfield,
    ...
    if (event.type == KEYEVENT_RAWKEYDOWN &&
      ...
      HoneycombPostTask(TID_UI, base::BindOnce(&TestTextfieldDelegate::FinishTest,
                                         this, textfield));
      return true;
    }
 ...
 private:
  void FinishTest(HoneycombRefPtr<HoneycombTextfield> textfield) {
...
void RunTextfieldKeyEvent(HoneycombRefPtr<HoneycombWindow> window) {
  HoneycombRefPtr<HoneycombTextfield> textfield =
      HoneycombTextfield::CreateTextfield(new TestTextfieldDelegate());
...
void TextfieldKeyEventImpl(HoneycombRefPtr<HoneycombWaitableEvent> event) {
  auto config = std::make_unique<TestWindowDelegate::Config>();
...
// Test Textfield input and events. This is primarily to exercise exposed Honeycomb
