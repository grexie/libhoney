...
// static
void TestWindowDelegate::RunTest(HoneycombRefPtr<HoneycombWaitableEvent> event,
                                 std::unique_ptr<Config> config) {
  HoneycombSize window_size{config->window_size, config->window_size};
...
void TestWindowDelegate::OnWindowCreated(HoneycombRefPtr<HoneycombWindow> window) {
  ...
  if (config_->initial_show_state == HONEYCOMB_SHOW_STATE_NORMAL) {
    ...
    HoneycombRect client_bounds = window->GetBounds();
    if (!config_->window_origin.IsEmpty()) {
...
void TestWindowDelegate::OnWindowDestroyed(HoneycombRefPtr<HoneycombWindow> window) {
...
bool TestWindowDelegate::IsFrameless(HoneycombRefPtr<HoneycombWindow> window) {
  return config_->frameless;
}

HoneycombRect TestWindowDelegate::GetInitialBounds(HoneycombRefPtr<HoneycombWindow> window) {
  got_get_initial_bounds_ = true;
  if (!config_->window_origin.IsEmpty()) {
    return HoneycombRect(config_->window_origin.x, config_->window_origin.y,
...
honey_show_state_t TestWindowDelegate::GetInitialShowState(
    HoneycombRefPtr<HoneycombWindow> window) {
  return config_->initial_show_state;
}

HoneycombSize TestWindowDelegate::GetPreferredSize(HoneycombRefPtr<HoneycombView> view) {
  got_get_preferred_size_ = true;
  return window_size_;
}

bool TestWindowDelegate::OnAccelerator(HoneycombRefPtr<HoneycombWindow> window,
                                       int command_id) {
...
bool TestWindowDelegate::OnKeyEvent(HoneycombRefPtr<HoneycombWindow> window,
...
TestWindowDelegate::TestWindowDelegate(HoneycombRefPtr<HoneycombWaitableEvent> event,
                                       std::unique_ptr<Config> config,
...
TestWindowDelegate::~TestWindowDelegate() {
  ...
  HoneycombPostTask(TID_UI, base::BindOnce(SignalEvent, event_));
}
...
void TestWindowDelegate::OnCloseWindow() {
  ...
  HoneycombRefPtr<HoneycombWindow> window = window_;
  window->Close();
