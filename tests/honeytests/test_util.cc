...
void TestMapEqual(const HoneycombRequest::HeaderMap& map1,
                  const HoneycombRequest::HeaderMap& map2,
                  bool allowExtras) {
  if (!allowExtras) {
  ...
  HoneycombRequest::HeaderMap::const_iterator it1, it2;
...
void TestMapNoDuplicates(const HoneycombRequest::HeaderMap& map) {
  HoneycombRequest::HeaderMap::const_iterator it1 = map.begin();
  for (; it1 != map.end(); ++it1) {
    HoneycombRequest::HeaderMap::const_iterator it2 = it1;
...
void TestPostDataElementEqual(HoneycombRefPtr<HoneycombPostDataElement> elem1,
                              HoneycombRefPtr<HoneycombPostDataElement> elem2) {
...
void TestPostDataEqual(HoneycombRefPtr<HoneycombPostData> postData1,
                       HoneycombRefPtr<HoneycombPostData> postData2) {
  EXPECT_TRUE(postData1.get());
  EXPECT_TRUE(postData2.get());

  EXPECT_EQ(postData1->GetElementCount(), postData2->GetElementCount());

  HoneycombPostData::ElementVector ev1, ev2;
  postData1->GetElements(ev1);
  postData1->GetElements(ev2);
  ASSERT_EQ(ev1.size(), ev2.size());

  HoneycombPostData::ElementVector::const_iterator it1 = ev1.begin();
  HoneycombPostData::ElementVector::const_iterator it2 = ev2.begin();
  for (; it1 != ev1.end() && it2 != ev2.end(); ++it1, ++it2) {
    TestPostDataElementEqual((*it1), (*it2));
  }
}

void TestRequestEqual(HoneycombRefPtr<HoneycombRequest> request1,
                      HoneycombRefPtr<HoneycombRequest> request2,
                      bool allowExtras) {
  ...
  HoneycombRequest::HeaderMap headers1, headers2;
  request1->GetHeaderMap(headers1);
  request2->GetHeaderMap(headers2);
  TestMapEqual(headers1, headers2, allowExtras);

  HoneycombRefPtr<HoneycombPostData> postData1 = request1->GetPostData();
  HoneycombRefPtr<HoneycombPostData> postData2 = request2->GetPostData();
...
void TestResponseEqual(HoneycombRefPtr<HoneycombResponse> response1,
                       HoneycombRefPtr<HoneycombResponse> response2,
                       bool allowExtras) {
...
void TestBinaryEqual(HoneycombRefPtr<HoneycombBinaryValue> val1,
                     HoneycombRefPtr<HoneycombBinaryValue> val2) {
...
void TestDictionaryEqual(HoneycombRefPtr<HoneycombDictionaryValue> val1,
                         HoneycombRefPtr<HoneycombDictionaryValue> val2) {
  EXPECT_TRUE(val1.get());
  EXPECT_TRUE(val2.get());
  ...
  HoneycombDictionaryValue::KeyList keys;
  EXPECT_TRUE(val1->GetKeys(keys));

  HoneycombDictionaryValue::KeyList::const_iterator it = keys.begin();
  for (; it != keys.end(); ++it) {
    HoneycombString key = *it;
    EXPECT_TRUE(val2->HasKey(key));
    HoneycombValueType type = val1->GetType(key);
...
void TestListEqual(HoneycombRefPtr<HoneycombListValue> val1, HoneycombRefPtr<HoneycombListValue> val2) {
  EXPECT_TRUE(val1.get());
  EXPECT_TRUE(val2.get());
  ...
  for (size_t i = 0; i < size; ++i) {
    HoneycombValueType type = val1->GetType(i);
...
void TestProcessMessageEqual(HoneycombRefPtr<HoneycombProcessMessage> val1,
                             HoneycombRefPtr<HoneycombProcessMessage> val2) {
  EXPECT_TRUE(val1.get());
  EXPECT_TRUE(val2.get());
...
void TestStringVectorEqual(const std::vector<HoneycombString>& val1,
                           const std::vector<HoneycombString>& val2) {
...
void SendMouseClickEvent(HoneycombRefPtr<HoneycombBrowser> browser,
                         const HoneycombMouseEvent& mouse_event,
                         honey_mouse_button_type_t mouse_button_type) {
  auto host = browser->GetHost();
  HoneycombPostDelayedTask(TID_UI,
                     base::BindOnce(&HoneycombBrowserHost::SendMouseClickEvent, host,
...
void GrantPopupPermission(HoneycombRefPtr<HoneycombRequestContext> request_context,
  ...
  if (IsChromeRuntimeEnabled()) {
    ...
    if (test_website_setting) {
      ...
      value->SetInt(HONEYCOMB_CONTENT_SETTING_VALUE_ALLOW);
      request_context->SetWebsiteSetting(
          parent_url, parent_url, HONEYCOMB_CONTENT_SETTING_TYPE_POPUPS, value);
    } else {
...
void CreateTestRequestContext(TestRequestContextMode mode,
  ...
  if (mode == TEST_RC_MODE_CUSTOM_WITH_HANDLER) {
    ...
    if (!cache_path.empty()) {
      HoneycombString(&settings.cache_path) = cache_path;
    }
    HoneycombRequestContext::CreateContext(settings, rc_handler);
  } else {
    HoneycombRequestContext::CreateContext(HoneycombRequestContext::GetGlobalContext(),
                                     rc_handler);
  }
}

HoneycombTime HoneycombTimeFrom(HoneycombBaseTime value) {
  HoneycombTime time;
  honey_time_from_basetime(value, &time);
  return time;
}

HoneycombBaseTime HoneycombBaseTimeFrom(const HoneycombTime& value) {
  honey_basetime_t temp;
  honey_time_to_basetime(&value, &temp);
  return HoneycombBaseTime(temp);
}
