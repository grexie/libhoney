...
#ifndef HONEYCOMB_TESTS_HONEYCOMBTESTS_TEST_UTIL_H_
#define HONEYCOMB_TESTS_HONEYCOMBTESTS_TEST_UTIL_H_
#pragma once
...
// Test that HoneycombRequest::HeaderMap objects are equal. Multiple values with the
...
// Test that the HoneycombRequest::HeaderMap object contains no duplicate entries.
void TestMapNoDuplicates(const HoneycombRequest::HeaderMap& map);

// Test that HoneycombPostDataElement objects are equal
...
// Test that HoneycombResponse objects are equal
// If |allowExtras| is true then additional header fields will be allowed in
// |response2|.
...
// Test if two HoneycombString vectors are equal.
...
// Returns true if the old HoneycombResourceHandler API should be tested.
...
void SendMouseClickEvent(HoneycombRefPtr<HoneycombBrowser> browser,
                         ...
                         honey_mouse_button_type_t mouse_button_type = MBT_LEFT);
...
// Create a HoneycombRequestContext object matching the specified |mode|. |cache_path|
...
// valid combinations.
using RCInitCallback = base::OnceCallback<void(HoneycombRefPtr<HoneycombRequestContext>)>;
...
#define RC_TEST_SINGLE(test_case_name, test_name, test_class, rc_mode,   \
  ...
  TEST(test_case_name, test_name) {                                      \
    HoneycombScopedTempDir scoped_temp_dir;                                    \
    ...
    HoneycombRefPtr<test_class> handler = new test_class(rc_mode, cache_path); \
    handler->ExecuteTest();                                              \
...
#define RC_TEST_BASE(test_case_name, test_name, test_class, test_mode, \
  ...
  TEST(test_case_name, test_name) {                                    \
    ...
    HoneycombRefPtr<test_class> handler =                                    \
...
#endif  // HONEYCOMB_TESTS_HONEYCOMBTESTS_TEST_UTIL_H_
