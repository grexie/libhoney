...
class ReadHandler : public HoneycombReadHandler {
 public:
...
class ReadTestHandler : public RoutingTestHandler {
 public:
  ...
  HoneycombRefPtr<HoneycombResourceHandler> GetResourceHandler(
    ...
    HoneycombRefPtr<ReadHandler> handler = new ReadHandler(may_block_);
    handler->CreateContent();
    expected_result_ = handler->GetExpectedResult();

    HoneycombRefPtr<HoneycombStreamReader> stream =
        HoneycombStreamReader::CreateForHandler(handler.get());
...
TEST(StreamResourceHandlerTest, ReadWillBlock) {
  HoneycombRefPtr<ReadTestHandler> handler = new ReadTestHandler(true);
  handler->ExecuteTest();
  ReleaseAndWaitForDestructor(handler);
}

TEST(StreamResourceHandlerTest, ReadWontBlock) {
  HoneycombRefPtr<ReadTestHandler> handler = new ReadTestHandler(false);
  handler->ExecuteTest();
  ReleaseAndWaitForDestructor(handler);
}
