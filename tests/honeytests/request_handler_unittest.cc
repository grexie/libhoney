      ...
      if (delay_loaded) {
        ...
        if (test_type_ == NNTT_DELAYED_RENDERER) {
          ...
          HoneycombRefPtr<HoneycombProcessMessage> message =
              HoneycombProcessMessage::Create(kNetNotifyMsg);
    ...
    class TestVisitor : public HoneycombCookieVisitor {
     public:
      ...
      ~TestVisitor() override {
        // Destroy the test.
        HoneycombPostTask(TID_UI, base::BindOnce(&NetNotifyTestHandler::DestroyTest,
                                           handler_));
      }
...
void RunNetNotifyTest(NetNotifyTestType test_type,
  ...
  std::vector<HoneycombRefPtr<NetNotifyTestHandler>> handlers;
  for (size_t i = 0U; i < count; ++i) {
    HoneycombRefPtr<NetNotifyTestHandler> handler =
