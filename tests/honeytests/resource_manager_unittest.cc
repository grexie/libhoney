...
HoneycombRefPtr<HoneycombResourceHandler> CreateContentsResourceHandler(
...
class ResourceManagerTestHandler : public RoutingTestHandler {
 public:
  struct State {
    State() : manager_(new HoneycombResourceManager()), expected_message_ct_(0) {}

    HoneycombRefPtr<HoneycombResourceManager> manager_;
  ...
  bool OnQuery(HoneycombRefPtr<HoneycombBrowser> browser,
    ...
    HoneycombPostTask(TID_UI, base::BindOnce(&ResourceManagerTestHandler::Continue,
                                       this, browser));
 ...
 private:
  void Continue(HoneycombRefPtr<HoneycombBrowser> browser) {
...
TEST(ResourceManagerTest, NoProviders) {
  ...
  HoneycombRefPtr<ResourceManagerTestHandler> handler =
...
class TestProvider : public HoneycombResourceManager::Provider {
 public:
  struct State {
  ...
  void OnRequestCanceled(
      scoped_refptr<HoneycombResourceManager::Request> request) override {
    HONEYCOMB_REQUIRE_IO_THREAD();
...
class SimpleTestProvider : public TestProvider {
 public:
  ...
  SimpleTestProvider(State* state, Mode mode, HoneycombResourceManager* manager)
      : TestProvider(state), mode_(mode), manager_(manager) {}

  SimpleTestProvider(State* state,
                     Mode mode,
                     HoneycombResourceManager* manager,
 ...
 private:
  Mode mode_;
  HoneycombResourceManager* manager_;  // Weak reference.
...
TEST(ResourceManagerTest, DirectoryProvider) {
  ...
  HoneycombScopedTempDir scoped_dir;
  ...
  EXPECT_TRUE(HoneycombCreateDirectory(sub_dir));
...
TEST(ResourceManagerTest, ArchiveProvider) {
  ...
  EXPECT_TRUE(HoneycombCreateDirectory(file_dir));
  ...
  EXPECT_TRUE(HoneycombZipDirectory(file_dir, archive_path, false));
...
class OneShotProvider : public HoneycombResourceManager::Provider {
 public:
  ...
  bool OnRequest(scoped_refptr<HoneycombResourceManager::Request> request) override {
    ...
    request->Continue(new HoneycombStreamResourceHandler("text/html", stream));
    return true;
  }
...
class EchoProvider : public HoneycombResourceManager::Provider {
 public:
  ...
  bool OnRequest(scoped_refptr<HoneycombResourceManager::Request> request) override {
    ...
    HoneycombRefPtr<HoneycombStreamResourceHandler> handler(
        new HoneycombStreamResourceHandler("text/html", stream));
    HoneycombPostDelayedTask(TID_IO,
                       base::BindOnce(&HoneycombResourceManager::Request::Continue,
                                      request, handler),
                       delay);
