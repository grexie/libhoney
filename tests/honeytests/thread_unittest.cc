...
class ThreadTest : public base::RefCountedThreadSafe<ThreadTest> {
 public:
  ...
  void CreateTestThread() {
    ...
    thread_ = HoneycombThread::CreateThread("test_thread");
    EXPECT_TRUE(thread_.get());
  ...
  void PostOnTestThreadAndCallback(
      ...
      HoneycombRefPtr<HoneycombTaskRunner> callback_task_runner,
      base::OnceClosure callback) {
    EXPECT_TRUE(thread_.get());
    thread_task_runner_->PostTask(HoneycombCreateClosureTask(base::BindOnce(
        &ThreadTest::ExecuteOnTestThread, this, std::move(test_task),
        callback_task_runner, std::move(callback))));
  }

  HoneycombRefPtr<HoneycombTaskRunner> owner_task_runner() const {
    return owner_task_runner_;
  }
  HoneycombRefPtr<HoneycombTaskRunner> thread_task_runner() const {
 ...
 private:
  ...
  void ExecuteOnTestThread(base::OnceClosure test_task,
    ...
    callback_task_runner->PostTask(HoneycombCreateClosureTask(std::move(callback)));
  }

  HoneycombRefPtr<HoneycombTaskRunner> owner_task_runner_;

  HoneycombRefPtr<HoneycombThread> thread_;
...
class BrowserThreadTestHandler : public TestHandler {
 public:
  explicit BrowserThreadTestHandler(HoneycombThreadId owner_thread_id)
  ...
  void RunThreadTestOnOwnerThread() {
    if (!HoneycombCurrentlyOn(owner_thread_id_)) {
  ...
  void DoneOnOwnerThread() {
    ...
    HoneycombPostTask(owner_thread_id_,
                base::BindOnce(
                    &BrowserThreadTestHandler::DestroyTestOnOwnerThread, this));
  }

  void DestroyTestOnOwnerThread() {
    EXPECT_TRUE(HoneycombCurrentlyOn(owner_thread_id_));
 ...
 private:
  ...
  const HoneycombThreadId owner_thread_id_;
...
TEST(ThreadTest, CreateFromBrowserUIThread) {
  HoneycombRefPtr<BrowserThreadTestHandler> handler =
 ...
 public:
  ...
  void OnLoadingStateChange(HoneycombRefPtr<HoneycombBrowser> browser,
    ...
    if (!isLoading) {
      ...
      HoneycombRefPtr<HoneycombProcessMessage> msg =
          HoneycombProcessMessage::Create(kRenderThreadTestMsg);
...
class RenderThreadRendererTest : public ClientAppRenderer::Delegate {
 ...
 private:
  void Done() {
    // Let the call stack unwind before destroying |thread_test_|.
    HoneycombPostTask(TID_RENDERER,
...
TEST(ThreadTest, CreateFromRenderThread) {
  HoneycombRefPtr<RenderThreadTestHandler> handler = new RenderThreadTestHandler();
  handler->ExecuteTest();
  ReleaseAndWaitForDestructor(handler);
}
