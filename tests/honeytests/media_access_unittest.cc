...
class TestSetup {
 public:
  ...
  // then never calling HoneycombMediaAccessCallback::Continue.
...
class MediaAccessTestHandler : public TestHandler, public HoneycombPermissionHandler {
 public:
  ...
  void OnLoadEnd(HoneycombRefPtr<HoneycombBrowser> browser,
    ...
    if (test_setup_->deny_with_navigation) {
      ...
      HoneycombExecuteJavaScriptWithUserGestureForTests(frame, "runTest()");
    }
  }
  ...
  bool OnRequestMediaAccessPermission(
    ...
    if (test_setup_->continue_async) {
      HoneycombPostTask(TID_UI, base::BindOnce(&HoneycombMediaAccessCallback::Continue,
                                         callback, response_));
    } else {
 ...
 private:
  bool want_audio_device() const {
    return request_ & HONEYCOMB_MEDIA_PERMISSION_DEVICE_AUDIO_CAPTURE;
  }
  bool want_video_device() const {
    return request_ & HONEYCOMB_MEDIA_PERMISSION_DEVICE_VIDEO_CAPTURE;
  }
  bool want_audio_desktop() const {
    return request_ & HONEYCOMB_MEDIA_PERMISSION_DESKTOP_AUDIO_CAPTURE;
  }
  bool want_video_desktop() const {
    return request_ & HONEYCOMB_MEDIA_PERMISSION_DESKTOP_VIDEO_CAPTURE;
  }

  bool got_audio_device() const {
    return response_ & HONEYCOMB_MEDIA_PERMISSION_DEVICE_AUDIO_CAPTURE;
  }
  bool got_video_device() const {
    return response_ & HONEYCOMB_MEDIA_PERMISSION_DEVICE_VIDEO_CAPTURE;
  }
  bool got_audio_desktop() const {
    return response_ & HONEYCOMB_MEDIA_PERMISSION_DESKTOP_AUDIO_CAPTURE;
  }
  bool got_video_desktop() const {
    return response_ & HONEYCOMB_MEDIA_PERMISSION_DESKTOP_VIDEO_CAPTURE;
  }
  ...
  HoneycombRefPtr<HoneycombMediaAccessCallback> callback_;
...
TEST(MediaAccessTest, DeviceFailureWhenReturningFalse) {
  ...
  HoneycombRefPtr<MediaAccessTestHandler> handler =
      new MediaAccessTestHandler(&test_setup,
                                 HONEYCOMB_MEDIA_PERMISSION_DEVICE_AUDIO_CAPTURE |
                                     HONEYCOMB_MEDIA_PERMISSION_DEVICE_VIDEO_CAPTURE,
                                 HONEYCOMB_MEDIA_PERMISSION_NONE);
  handler->ExecuteTest();
...
TEST(MediaAccessTest, DeviceFailureWhenRequestingAudioButReturningVideo) {
  ...
  HoneycombRefPtr<MediaAccessTestHandler> handler = new MediaAccessTestHandler(
      &test_setup, HONEYCOMB_MEDIA_PERMISSION_DEVICE_AUDIO_CAPTURE,
      HONEYCOMB_MEDIA_PERMISSION_DEVICE_VIDEO_CAPTURE);
  handler->ExecuteTest();
  ReleaseAndWaitForDestructor(handler);
...
TEST(MediaAccessTest, DeviceFailureWhenRequestingVideoButReturningAudio) {
  ...
  HoneycombRefPtr<MediaAccessTestHandler> handler = new MediaAccessTestHandler(
      &test_setup, HONEYCOMB_MEDIA_PERMISSION_DEVICE_VIDEO_CAPTURE,
      HONEYCOMB_MEDIA_PERMISSION_DEVICE_AUDIO_CAPTURE);
  handler->ExecuteTest();
  ReleaseAndWaitForDestructor(handler);
...
TEST(MediaAccessTest, DeviceFailureWhenReturningScreenCapture1) {
  ...
  HoneycombRefPtr<MediaAccessTestHandler> handler =
      new MediaAccessTestHandler(&test_setup,
                                 HONEYCOMB_MEDIA_PERMISSION_DEVICE_AUDIO_CAPTURE |
                                     HONEYCOMB_MEDIA_PERMISSION_DEVICE_VIDEO_CAPTURE,
                                 HONEYCOMB_MEDIA_PERMISSION_DESKTOP_AUDIO_CAPTURE);
  handler->ExecuteTest();
  ReleaseAndWaitForDestructor(handler);
...
TEST(MediaAccessTest, DeviceFailureWhenReturningScreenCapture2) {
  ...
  HoneycombRefPtr<MediaAccessTestHandler> handler =
      new MediaAccessTestHandler(&test_setup,
                                 HONEYCOMB_MEDIA_PERMISSION_DEVICE_AUDIO_CAPTURE |
                                     HONEYCOMB_MEDIA_PERMISSION_DEVICE_VIDEO_CAPTURE,
                                 HONEYCOMB_MEDIA_PERMISSION_DESKTOP_VIDEO_CAPTURE);
  handler->ExecuteTest();
  ReleaseAndWaitForDestructor(handler);
...
TEST(MediaAccessTest, DesktopFailureWhenRequestingVideoButReturningAudio) {
  ...
  HoneycombRefPtr<MediaAccessTestHandler> handler = new MediaAccessTestHandler(
      &test_setup, HONEYCOMB_MEDIA_PERMISSION_DESKTOP_VIDEO_CAPTURE,
      HONEYCOMB_MEDIA_PERMISSION_DESKTOP_AUDIO_CAPTURE);
  handler->ExecuteTest();
  ReleaseAndWaitForDestructor(handler);
