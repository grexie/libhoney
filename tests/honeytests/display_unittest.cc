...
class TitleTestHandler : public TestHandler {
 ...
 private:
  void NextIfReady(HoneycombRefPtr<HoneycombBrowser> browser) {
...
TEST(DisplayTest, Title) {
  HoneycombRefPtr<TitleTestHandler> handler = new TitleTestHandler();
  handler->ExecuteTest();
...
class AutoResizeTestHandler : public RoutingTestHandler {
 public:
  ...
  void OnAfterCreated(HoneycombRefPtr<HoneycombBrowser> browser) override {
    ...
    browser->GetHost()->SetAutoResizeEnabled(true, HoneycombSize(10, 10),
                                             HoneycombSize(500, 500));
  }
  ...
  bool OnAutoResize(HoneycombRefPtr<HoneycombBrowser> browser,
    ...
    if (new_size.width == 1064 && new_size.height == 576) {
      ...
      browser->GetHost()->SetAutoResizeEnabled(false, HoneycombSize(), HoneycombSize());
...
TEST(DisplayTest, AutoResize) {
  HoneycombRefPtr<AutoResizeTestHandler> handler = new AutoResizeTestHandler();
  handler->ExecuteTest();
  ReleaseAndWaitForDestructor(handler);
}
...
class ConsoleTestHandler : public TestHandler {
 public:
  struct TestConfig {
    ...
    explicit TestConfig(honey_log_severity_t message_level)
        : level(message_level),
 ...
 private:
  std::string CreateResourceContent(const HoneycombString& message,
                                    const HoneycombString& function,
                                    int line) {
...
TEST(DisplayTest, OnConsoleMessageDebug) {
  ...
  HoneycombRefPtr<ConsoleTestHandler> handler = new ConsoleTestHandler(config);
  handler->ExecuteTest();
  ReleaseAndWaitForDestructor(handler);
}
...
TEST(DisplayTest, LoadingProgress) {
  HoneycombRefPtr<LoadingProgressTestHandler> handler =
