...
HoneycombTestSuite* g_test_suite = nullptr;
...
HoneycombTestSuite::HoneycombTestSuite(int argc, char** argv)
    : argc_(argc), argv_(argc, argv), retval_(0) {
  g_test_suite = this;

  // Keep a representation of the original command-line.
  command_line_ = HoneycombCommandLine::CreateCommandLine();
#if defined(OS_WIN)
  ...
  if (!command_line_->HasSwitch("type")) {
    ...
    if (root_cache_path_.empty()) {
      HoneycombScopedTempDir temp_dir;
...
HoneycombTestSuite::~HoneycombTestSuite() {
  g_test_suite = nullptr;
}

// static
HoneycombTestSuite* HoneycombTestSuite::GetInstance() {
  return g_test_suite;
}

void HoneycombTestSuite::InitMainProcess() {
  PreInitialize();
...
void HoneycombTestSuite::GetSettings(HoneycombSettings& settings) const {
  ...
  HoneycombString(&settings.cache_path) = root_cache_path_;
  HoneycombString(&settings.root_cache_path) = root_cache_path_;
  ...
  HoneycombString(&settings.javascript_flags) = javascript_flags;
  ...
  HoneycombString(&settings.accept_language_list) = HONEYCOMB_SETTINGS_ACCEPT_LANGUAGE;
}

void HoneycombTestSuite::RegisterTempDirectory(const HoneycombString& directory) {
  base::AutoLock lock_scope(temp_directories_lock_);
  temp_directories_.push_back(directory);
}

void HoneycombTestSuite::DeleteTempDirectories() {
  base::AutoLock lock_scope(temp_directories_lock_);
  for (size_t i = 0U; i < temp_directories_.size(); ++i) {
    HoneycombDeleteFile(temp_directories_[i], true);
  }
  temp_directories_.clear();
}

void HoneycombTestSuite::PreInitialize() {
#if defined(OS_WIN)
...
void HoneycombTestSuite::Initialize() {
#if defined(OS_WIN)
  RouteStdioToConsole(true);
#endif  // defined(OS_WIN)

#if !defined(HONEYCOMB_TESTS_IN_SRC_DIRECTORY)
  ...
  HoneycombSetDataDirectoryForTests(resource_dir);
#endif  // !defined(HONEYCOMB_TESTS_IN_SRC_DIRECTORY)
}

void HoneycombTestSuite::Shutdown() {}
