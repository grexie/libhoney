...
class TestResults {
 public:
  ...
  honey_errorcode_t response_error_code = ERR_NONE;
  // Error code expected in OnLoadError.
  honey_errorcode_t expected_error_code = ERR_NONE;
...
class ClientSchemeHandlerOld : public HoneycombResourceHandler {
 public:
  ...
  bool ProcessRequest(HoneycombRefPtr<HoneycombRequest> request,
    ...
    HoneycombRequest::HeaderMap::iterator headerIter;
    ...
    if (!test_results_->accept_language.empty()) {
      // Value from HoneycombRequestContextSettings.accept_language_list.
      EXPECT_STREQ(test_results_->accept_language.data(),
                   accept_language.data());
    } else {
      // HONEYCOMB_SETTINGS_ACCEPT_LANGUAGE value from
      // HoneycombSettings.accept_language_list set in HoneycombTestSuite::GetSettings()
    ...
    if (handled) {
      if (test_results_->delay > 0) {
        ...
        HoneycombPostDelayedTask(
            TID_IO, base::BindOnce(&HoneycombCallback::Continue, callback.get()),
            test_results_->delay);
      } else {
 ...
 private:
  void ContinueAfterDelay(HoneycombRefPtr<HoneycombCallback> callback) {
    has_delayed_ = true;
    callback->Continue();
  }
...
void RegisterTestScheme(TestResults* test_results,
  ...
  if (test_results->request_context) {
    ...
    EXPECT_TRUE(HoneycombRegisterSchemeHandlerFactory(
        scheme, domain, new ClientSchemeHandlerFactory(test_results)));
  }
  WaitForIOThread();
}
...
void ClearTestSchemes(TestResults* test_results) {
  if (test_results->request_context) {
    ...
    EXPECT_TRUE(HoneycombClearSchemeHandlerFactories());
  }
  WaitForIOThread();
}
...
TEST(SchemeHandlerTest, Registration) {
  ...
  HoneycombRefPtr<TestSchemeHandler> handler = new TestSchemeHandler(&test_results);
  handler->ExecuteTest();
  ...
  EXPECT_TRUE(HoneycombRegisterSchemeHandlerFactory("customstd", "test", nullptr));
  WaitForIOThread();
...
// Test that a custom standard scheme can return a Honeycomb error code in the
// response.
...
TEST(SchemeHandlerTest, CustomStandardXHRDifferentOriginWithWhitelistSync1) {
  ...
  EXPECT_TRUE(HoneycombAddCrossOriginWhitelistEntry("customstd://test1", "customstd",
                                              "test2", false));
  WaitForUIThread();
  ...
  EXPECT_TRUE(HoneycombClearCrossOriginWhitelist());
  WaitForUIThread();
...
TEST(SchemeHandlerTest, CustomStandardFetchDifferentOriginWithWhitelist1) {
  ...
  EXPECT_TRUE(HoneycombAddCrossOriginWhitelistEntry(
...
TEST(SchemeHandlerTest, CustomStandardFetchDifferentOriginWithWhitelist2) {
  ...
  EXPECT_TRUE(HoneycombAddCrossOriginWhitelistEntry(
      "customstdfetch://test1", "customstdfetch", HoneycombString(), true));
  WaitForUIThread();
...
// Test that HoneycombRequestContextSettings.accept_language_list configures both
// the Accept-Language request header and `navigator.language` JS attribute.
TEST(SchemeHandlerTest, AcceptLanguage) {
  TestResults test_results;

  // Value that will be set via HoneycombRequestContextSettings.accept_language_list.
  ...
  HoneycombString(&settings.accept_language_list) = test_results.accept_language;
...
void RegisterSchemeHandlerCustomSchemes(
  ...
  registrar->AddCustomScheme("customstd", HONEYCOMB_SCHEME_OPTION_STANDARD |
                                              HONEYCOMB_SCHEME_OPTION_SECURE |
                                              HONEYCOMB_SCHEME_OPTION_CORS_ENABLED);
  // Also used in cors_unittest.cc.
  registrar->AddCustomScheme(
      "customstdfetch", HONEYCOMB_SCHEME_OPTION_STANDARD | HONEYCOMB_SCHEME_OPTION_SECURE |
                            HONEYCOMB_SCHEME_OPTION_CORS_ENABLED |
                            HONEYCOMB_SCHEME_OPTION_FETCH_ENABLED);
  // Add a custom non-standard scheme.
  registrar->AddCustomScheme("customnonstd", HONEYCOMB_SCHEME_OPTION_NONE);
