...
#include "tests/honeytests/test_server_observer.h"
...
typedef std::vector<HoneycombCookie> CookieVector;

class TestCompletionCallback : public HoneycombCompletionCallback {
 public:
  explicit TestCompletionCallback(HoneycombRefPtr<HoneycombWaitableEvent> event)
      : event_(event) {}
...
class TestSetCookieCallback : public HoneycombSetCookieCallback {
 public:
...
class TestDeleteCookiesCallback : public HoneycombDeleteCookiesCallback {
 public:
...
class TestVisitor : public HoneycombCookieVisitor {
 public:
...
// Set the cookies.
void SetCookies(HoneycombRefPtr<HoneycombCookieManager> manager,
...
// Delete the cookie.
void DeleteCookies(HoneycombRefPtr<HoneycombCookieManager> manager,
                   const HoneycombString& url,
...
void CreateCookie(HoneycombRefPtr<HoneycombCookieManager> manager,
                  HoneycombCookie& cookie,
                  bool withDomain,
                  bool sessionCookie,
                  HoneycombRefPtr<HoneycombWaitableEvent> event) {
  HoneycombString(&cookie.name).FromASCII("my_cookie");
  HoneycombString(&cookie.value).FromASCII("My Value");
  if (withDomain) {
    HoneycombString(&cookie.domain).FromASCII(kTestDomain);
  }
  HoneycombString(&cookie.path).FromASCII(kTestPath);
  if (!sessionCookie) {
    cookie.has_expires = true;
    // Must choose the expiry date dynamically due to the
    // "ClampCookieExpiryTo400Days" feature enabled in M104.
    honey_time_t expiry_time;
    EXPECT_TRUE(honey_time_from_timet(GetExpiryDate(), &expiry_time));
    EXPECT_TRUE(honey_time_to_basetime(&expiry_time, &cookie.expires));
  }
...
void VisitUrlCookies(HoneycombRefPtr<HoneycombCookieManager> manager,
                     const HoneycombString& url,
...
void VisitUrlCookies(HoneycombRefPtr<HoneycombCookieManager> manager,
  ...
  VisitUrlCookies(manager, url, includeHttpOnly, cookies, deleteCookies,
                  base::BindOnce(&HoneycombWaitableEvent::Signal, event));
  event->Wait();
}

// Visit all cookies. Execute |callback| on completion.
void VisitAllCookies(HoneycombRefPtr<HoneycombCookieManager> manager,
                     CookieVector& cookies,
                     bool deleteCookies,
                     base::OnceClosure callback) {
...
void GetCookie(HoneycombRefPtr<HoneycombCookieManager> manager,
  ...
  const HoneycombCookie& cookie_read = cookies[0];
  EXPECT_EQ(HoneycombString(&cookie_read.name), "my_cookie");
  EXPECT_EQ(HoneycombString(&cookie_read.value), "My Value");
  if (withDomain) {
    EXPECT_EQ(HoneycombString(&cookie_read.domain), ".www.test.com");
  } else {
    EXPECT_EQ(HoneycombString(&cookie_read.domain), kTestDomain);
  }
  EXPECT_EQ(HoneycombString(&cookie_read.path), kTestPath);
...
void VerifyNoCookies(HoneycombRefPtr<HoneycombCookieManager> manager,
                     HoneycombRefPtr<HoneycombWaitableEvent> event,
                     bool withUrl) {
  CookieVector cookies;
...
void DeleteAllCookies(HoneycombRefPtr<HoneycombCookieManager> manager,
                      HoneycombRefPtr<HoneycombWaitableEvent> event) {
  DeleteCookies(manager, HoneycombString(), HoneycombString(), kIgnoreNumDeleted, event);
}

void TestDomainCookie(HoneycombRefPtr<HoneycombCookieManager> manager,
                      HoneycombRefPtr<HoneycombWaitableEvent> event) {
  HoneycombCookie cookie;
...
void TestHostCookie(HoneycombRefPtr<HoneycombCookieManager> manager,
                    HoneycombRefPtr<HoneycombWaitableEvent> event) {
  HoneycombCookie cookie;
...
void TestInvalidCookie(HoneycombRefPtr<HoneycombCookieManager> manager,
                       HoneycombRefPtr<HoneycombWaitableEvent> event) {
  CookieVector cookies;

  HoneycombCookie cookie;
  const char* kUrl = "https://www.xyz.com";
  HoneycombString(&cookie.name).FromASCII("invalid1");
  HoneycombString(&cookie.value).FromASCII("invalid1");
  HoneycombString(&cookie.domain).FromASCII(".zyx.com");  // domain mismatch
...
void TestMultipleCookies(HoneycombRefPtr<HoneycombCookieManager> manager,
                         HoneycombRefPtr<HoneycombWaitableEvent> event) {
  std::stringstream ss;
  int i;
  ...
  for (i = 0; i < kNumCookies; i++) {
    ...
    ss << "my_cookie" << i;
    HoneycombString(&cookie.name).FromASCII(ss.str().c_str());
    ss.str("");
    ss << "My Value " << i;
    HoneycombString(&cookie.value).FromASCII(ss.str().c_str());
    ss.str("");
  ...
  for (i = 0; it != cookies.end(); ++it, ++i) {
    const HoneycombCookie& cookie = *it;

    ss << "my_cookie" << i;
    EXPECT_EQ(HoneycombString(&cookie.name), ss.str());
    ss.str("");
    ss << "My Value " << i;
    EXPECT_EQ(HoneycombString(&cookie.value), ss.str());
    ss.str("");
  }
  ...
  DeleteCookies(manager, kTestUrl, HoneycombString("my_cookie1"), 1, event);
  ...
  EXPECT_EQ(HoneycombString(&cookies[0].name), "my_cookie0");
  EXPECT_EQ(HoneycombString(&cookies[1].name), "my_cookie2");
  EXPECT_EQ(HoneycombString(&cookies[2].name), "my_cookie3");

  cookies.clear();

  // Delete the rest of the cookies.
  DeleteCookies(manager, kTestUrl, HoneycombString(), 3, event);
...
void TestAllCookies(HoneycombRefPtr<HoneycombCookieManager> manager,
                    HoneycombRefPtr<HoneycombWaitableEvent> event) {
  CookieVector cookies;
  ...
  HoneycombCookie cookie1;
  const char* kUrl1 = "https://www.foo.com";
  HoneycombString(&cookie1.name).FromASCII("my_cookie1");
  HoneycombString(&cookie1.value).FromASCII("My Value 1");

  cookies.push_back(cookie1);
  SetCookies(manager, kUrl1, cookies, true, event);
  cookies.clear();

  HoneycombCookie cookie2;
  const char* kUrl2 = "https://www.bar.com";
  HoneycombString(&cookie2.name).FromASCII("my_cookie2");
  HoneycombString(&cookie2.value).FromASCII("My Value 2");
  ...
  EXPECT_EQ(HoneycombString(&cookies[0].name), "my_cookie1");
  EXPECT_EQ(HoneycombString(&cookies[0].value), "My Value 1");
  EXPECT_EQ(HoneycombString(&cookies[0].domain), "www.foo.com");
  EXPECT_EQ(HoneycombString(&cookies[1].name), "my_cookie2");
  EXPECT_EQ(HoneycombString(&cookies[1].value), "My Value 2");
  EXPECT_EQ(HoneycombString(&cookies[1].domain), "www.bar.com");
  cookies.clear();
  ...
  EXPECT_EQ(HoneycombString(&cookies[0].name), "my_cookie2");
  EXPECT_EQ(HoneycombString(&cookies[0].value), "My Value 2");
  EXPECT_EQ(HoneycombString(&cookies[0].domain), "www.bar.com");
  cookies.clear();
...
TEST(CookieTest, BasicInvalidCookie) {
  ...
  HoneycombRefPtr<HoneycombCookieManager> manager =
      HoneycombCookieManager::GetGlobalManager(new TestCompletionCallback(event));
  event->Wait();
...
class CookieTestJSHandler : public TestHandler {
 public:
  ...
  // Go to the next URL.
  void LoadNextURL(HoneycombRefPtr<HoneycombFrame> frame) {
    if (!HoneycombCurrentlyOn(TID_UI)) {
      HoneycombPostTask(TID_UI, base::BindOnce(&CookieTestJSHandler::LoadNextURL,
                                         this, frame));
      return;
    }
  ...
  void VerifyCookie(HoneycombRefPtr<HoneycombCookieManager> manager,
  ...
  void VerifyCookieComplete(const std::string& name,
                            ...
                            TrackCallback* callback,
                            base::OnceClosure continue_callback) {
    if (cookies_.size() == 1U && HoneycombString(&cookies_[0].name) == name &&
        HoneycombString(&cookies_[0].value) == value) {
      callback->yes();
    }
  ...
  HoneycombRefPtr<HoneycombCookieManager> manager_;
...
TEST(CookieTest, GetCookieManagerJS) {
  HoneycombRefPtr<CookieTestJSHandler> handler = new CookieTestJSHandler();
  handler->ExecuteTest();
...
class CompletionCallback : public HoneycombCompletionCallback {
 public:
...
class CookieTestSchemeHandler : public TestHandler {
 public:
  class SchemeHandler : public HoneycombResourceHandler {
   public:
    ...
    bool Open(HoneycombRefPtr<HoneycombRequest> request,
      ...
      EXPECT_FALSE(HoneycombCurrentlyOn(TID_UI) || HoneycombCurrentlyOn(TID_IO));
      ...
      if (url == handler_->url1_) {
        ...
        HoneycombRequest::HeaderMap::iterator it = headerMap.find("Cookie");
  ...
  class SchemeHandlerFactory : public HoneycombSchemeHandlerFactory {
   public:
    explicit SchemeHandlerFactory(CookieTestSchemeHandler* handler)
        : handler_(handler) {}

    HoneycombRefPtr<HoneycombResourceHandler> Create(
  ...
  void RunTest() override {
    if (use_global_) {
      request_context_ = HoneycombRequestContext::GetGlobalContext();
    } else {
      // Create the request context that will use an in-memory cache.
      HoneycombRequestContextSettings settings;
      ...
      if (scheme_ == kCustomCookieScheme || block_cookies_) {
        if (!block_cookies_) {
          HoneycombString(&settings.cookieable_schemes_list) = kCustomCookieScheme;
        } else {
          settings.cookieable_schemes_exclude_defaults = true;
        }
      }

      request_context_ = HoneycombRequestContext::CreateContext(settings, nullptr);
    }
  ...
  // Go to the next URL.
  void LoadNextURL(HoneycombRefPtr<HoneycombFrame> frame, const std::string& url) {
    if (!HoneycombCurrentlyOn(TID_UI)) {
      HoneycombPostTask(TID_UI, base::BindOnce(&CookieTestSchemeHandler::LoadNextURL,
                                         this, frame, url));
      return;
    }
  ...
  void CompleteTest(HoneycombRefPtr<HoneycombBrowser> browser) {
    if (!HoneycombCurrentlyOn(TID_UI)) {
      HoneycombPostTask(TID_UI, base::BindOnce(&CookieTestSchemeHandler::CompleteTest,
                                         this, browser));
      return;
    }
...
TEST(CookieTest, GetCookieManagerHttpGlobal) {
  HoneycombRefPtr<CookieTestSchemeHandler> handler =
...
std::string GetHeaderValue(const HoneycombServer::HeaderMap& header_map,
...
class CookieAccessSchemeHandler : public HoneycombResourceHandler {
 public:
 ...
 private:
  static void TestCookie(const HoneycombCookie& cookie,
                         TrackCallback& got_cookie_js,
                         TrackCallback& got_cookie_net) {
    const std::string& cookie_name = HoneycombString(&cookie.name);
    const std::string& cookie_val = HoneycombString(&cookie.value);
...
class CookieAccessSchemeHandlerFactory : public HoneycombSchemeHandlerFactory,
 ...
 public:
  ...
  void Shutdown(base::OnceClosure complete_callback) {
    if (!HoneycombCurrentlyOn(TID_IO)) {
      HoneycombPostTask(TID_IO, base::BindOnce(std::move(complete_callback)));
      return;
    }
...
class CookieAccessServerHandler : public test_server::ObserverHelper,
 ...
 public:
  ...
  bool OnTestServerRequest(HoneycombRefPtr<HoneycombRequest> request,
...
class CookieAccessTestHandler : public RoutingTestHandler,
                                public HoneycombCookieAccessFilter {
 public:
  enum TestMode {
    ALLOW = 0,
    BLOCK_READ = 1 << 0,
    ...
    // Block all cookies using HoneycombRequestContextSettings. Can only be used with
  ...
  void RunTest() override {
    if (use_global_) {
      context_ = HoneycombRequestContext::GetGlobalContext();
    } else {
      // Create the request context that will use an in-memory cache.
      HoneycombRequestContextSettings settings;
      ...
      context_ = HoneycombRequestContext::CreateContext(settings, nullptr);
    }
  ...
  bool CanSaveCookie(HoneycombRefPtr<HoneycombBrowser> browser,
    ...
    EXPECT_STREQ("name_net", HoneycombString(&cookie.name).ToString().c_str());
    EXPECT_STREQ("value_net", HoneycombString(&cookie.value).ToString().c_str());
 ...
 private:
  void AddResponses(CookieAccessResponseHandler* handler) {
    ...
    // the cookies via JS.
    {
      data1_.response = HoneycombResponse::Create();
    ...
    {
      data2_.response = HoneycombResponse::Create();
    ...
    class TestVisitor : public HoneycombCookieVisitor {
     public:
      ...
      bool Visit(const HoneycombCookie& cookie,
                 ...
                 int total,
                 bool& deleteCookie) override {
        const std::string& name = HoneycombString(&cookie.name);
        const std::string& value = HoneycombString(&cookie.value);
  ...
  const std::string scheme_;
  const bool use_global_;
  HoneycombRefPtr<HoneycombRequestContext> context_;
  HoneycombRefPtr<HoneycombCookieManager> cookie_manager_;

  CookieAccessServerHandler* server_handler_ = nullptr;
  HoneycombRefPtr<CookieAccessSchemeHandlerFactory> scheme_factory_;
...
#define ACCESS_TEST(name, test_mode, backend_mode, custom_scheme, use_global) \
  TEST(CookieTest, Access##name) {                                            \
    HoneycombRefPtr<CookieAccessTestHandler> handler = new CookieAccessTestHandler( \
...
TEST(CookieTest, RestartGlobal) {
  HoneycombRefPtr<CookieRestartTestHandler> handler =
...
void RegisterCookieCustomSchemes(HoneycombRawPtr<HoneycombSchemeRegistrar> registrar) {
  // Used by GetCookieManagerCustom* tests.
  registrar->AddCustomScheme(
      kCustomCookieScheme,
      HONEYCOMB_SCHEME_OPTION_STANDARD | HONEYCOMB_SCHEME_OPTION_CORS_ENABLED);
}
