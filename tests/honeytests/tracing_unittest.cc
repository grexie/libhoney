...
// Use the Honeycomb version of the TRACE_* macros instead of the Chromium version.
#undef USING_CHROMIUM_INCLUDES
#include "include/base/honey_trace_event.h"
...
const char kTraceTestCategory[] = "honey.client";

class TracingTestHandler : public HoneycombEndTracingCallback,
                           public HoneycombCompletionCallback {
 public:
  TracingTestHandler(TracingTestType type, const char* trace_type)
      : trace_type_(trace_type), type_(type) {
    completion_event_ = HoneycombWaitableEvent::CreateWaitableEvent(true, false);
  }
  ...
  void BeginTracing() {
    ...
    HoneycombBeginTracing(kTraceTestCategory, this);
  }
  ...
  void TestTracing() {
    ...
    HoneycombEndTracing(HoneycombString(), this);
  }
  ...
  void ReadTracingFile(const std::string& file_path) {
    ...
    EXPECT_TRUE(HoneycombDeleteFile(file_path, false));
 ...
 private:
  ...
  HoneycombRefPtr<HoneycombWaitableEvent> completion_event_;
...
#define TRACING_TEST(name, test_type)                  \
  TEST(TracingTest, name) {                            \
    HoneycombRefPtr<TracingTestHandler> handler =            \
...
TEST(TracingTest, NowFromSystemTraceTime) {
  int64_t val = HoneycombNowFromSystemTraceTime();
  EXPECT_NE(val, 0);
}
