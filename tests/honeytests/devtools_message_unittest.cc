...
class DevToolsMessageTestHandler : public TestHandler {
 ...
 private:
  ...
  class TestMessageObserver : public HoneycombDevToolsMessageObserver {
   public:
  ...
  void ExecuteMethod(const std::string& method,
    ...
    if (message_id % 2 == 0) {
      ...
      method_execute_ct_++;
      HoneycombRefPtr<HoneycombDictionaryValue> dict;
      if (!params.empty()) {
        HoneycombRefPtr<HoneycombValue> value =
            HoneycombParseJSON(params.data(), params.size(), JSON_PARSER_RFC);
  ...
  void OnMethodResult(const MethodResult& result) {
    ...
    HoneycombPostTask(TID_UI, std::move(pending_result_next_));
  ...
  void OnEvent(const Event& event) {
    ...
    // Continue asynchronously to allow the callstack to unwind.
    HoneycombPostTask(TID_UI, std::move(pending_event_next_));
  ...
  HoneycombRefPtr<HoneycombRegistration> registration_;
...
// - HoneycombDevToolsMessageObserver registration and life span.
// - SendDevToolsMessage/ExecuteDevToolsMethod calls.
// - HoneycombDevToolsMessageObserver callbacks for method results and events.
TEST(DevToolsMessageTest, Messages) {
  HoneycombRefPtr<DevToolsMessageTestHandler> handler =
