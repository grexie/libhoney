...
static void VerifyStreamReadBehavior(HoneycombRefPtr<HoneycombStreamReader> stream,
...
static void VerifyStreamWriteBehavior(HoneycombRefPtr<HoneycombStreamWriter> stream,
                                      const std::string& contents) {
  int contentSize = static_cast<int>(contents.size());
  const char* contentStr = contents.c_str();
...
TEST(StreamTest, ReadFile) {
  ...
  HoneycombString fileNameStr = "StreamTest.VerifyReadFile.txt";
  ...
  HoneycombRefPtr<HoneycombStreamReader> stream(
      HoneycombStreamReader::CreateForFile(fileNameStr));
...
TEST(StreamTest, WriteFile) {
  ...
  HoneycombString fileNameStr = "StreamTest.VerifyWriteFile.txt";
  std::string contents = "This is my test\ncontents for the file";

  // Test the stream
  HoneycombRefPtr<HoneycombStreamWriter> stream(
      HoneycombStreamWriter::CreateForFile(fileNameStr));
  ASSERT_TRUE(stream.get() != nullptr);
  ASSERT_TRUE(stream->MayBlock());
...
class ReadHandlerTester : public HoneycombReadHandler {
 public:
  ReadHandlerTester()
      : read_called_(false),
        read_ptr_(nullptr),
...
TEST(StreamTest, ReadHandler) {
  ...
  HoneycombRefPtr<HoneycombStreamReader> stream(HoneycombStreamReader::CreateForHandler(handler));
  ASSERT_TRUE(stream.get() != nullptr);
  ASSERT_FALSE(stream->MayBlock());

  // HoneycombReadHandler Read
...
class WriteHandlerTester : public HoneycombWriteHandler {
 public:
  WriteHandlerTester()
      : write_called_(false),
        write_ptr_(nullptr),
...
TEST(StreamTest, WriteHandler) {
  ...
  HoneycombRefPtr<HoneycombStreamWriter> stream(HoneycombStreamWriter::CreateForHandler(handler));
  ASSERT_TRUE(stream.get() != nullptr);
  ASSERT_FALSE(stream->MayBlock());

  // HoneycombWriteHandler Write
  ...
  // HoneycombWriteHandler Tell
  int64_t tell_res = stream->Tell();
  ASSERT_TRUE(handler->tell_called_);
  ASSERT_EQ(10, tell_res);

  // HoneycombWriteHandler Flush
