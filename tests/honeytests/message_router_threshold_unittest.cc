...
constexpr auto kCharSize = sizeof(HoneycombString::char_type);

enum class TestType { SUCCESS, FAILURE };

HoneycombString GenerateResponse(size_t count, char ch) {
  return std::string(count, ch);
}

HoneycombString GenerateResponse(size_t count, wchar_t ch) {
...
using CharTestHandlerPtr = HoneycombRefPtr<CharTestHandler>;

using WCharTestHandler = ThresholdTestHandler<wchar_t>;
using WCharTestHandlerPtr = HoneycombRefPtr<WCharTestHandler>;
