...
#if defined(OS_WIN)
const HoneycombRect kExpandedSelectRect(462, 42, 81, 334);
#elif defined(OS_MAC)
const HoneycombRect kExpandedSelectRect(462, 42, 75, 334);
#elif defined(OS_LINUX)
const HoneycombRect kExpandedSelectRect(462, 42, 79, 334);
#else
...
constexpr uint32_t kAllTouchHandleFlags =
    (HONEYCOMB_THS_FLAG_ENABLED | HONEYCOMB_THS_FLAG_ORIENTATION | HONEYCOMB_THS_FLAG_ORIGIN |
     HONEYCOMB_THS_FLAG_ALPHA);
...
enum OSRTestType {
  ...
  // subsequently after last element HoneycombFocusHandler::OnTakeFocus should be
  ...
  // HONEYCOMB_POINTER_TYPE_PEN is mapped to pen pointer event
  OSR_TEST_PEN,
    ...
    switch (test_type_) {
      case OSR_TEST_TOOLTIP:
        if (StartTest()) {
          ...
          const HoneycombRect& expected_rect = GetElementBounds("LI10");
...
#error "Unsupported platform"
      ...
      case OSR_TEST_CURSOR:
        if (StartTest()) {
          ...
          HoneycombMouseEvent mouse_event;
          const HoneycombRect& expected_rect = GetElementBounds("LI02");
          mouse_event.x = MiddleX(expected_rect);
          mouse_event.y = MiddleY(expected_rect);
          browser->GetHost()->SendMouseMoveEvent(mouse_event, false);
        }
      ...
      case OSR_TEST_MOUSE_MOVE:
        if (StartTest()) {
          HoneycombMouseEvent mouse_event;
          const HoneycombRect& expected_rect = GetElementBounds("LI03");
          mouse_event.x = MiddleX(expected_rect);
          mouse_event.y = MiddleY(expected_rect);
          mouse_event.modifiers = 0;
          browser->GetHost()->SendMouseMoveEvent(mouse_event, false);
      ...
      case OSR_TEST_CONTEXT_MENU:
        if (StartTest()) {
          HoneycombMouseEvent mouse_event;
          const HoneycombRect& expected_rect = GetElementBounds("LI04");
          mouse_event.x = MiddleX(expected_rect);
          mouse_event.y = MiddleY(expected_rect);
          mouse_event.modifiers = 0;
      ...
      case OSR_TEST_QUICK_MENU:
        if (StartTest()) {
          HoneycombTouchEvent touch_event_pressed;
          touch_event_pressed.type = HONEYCOMB_TET_PRESSED;
          const HoneycombRect& expected_rect = GetElementBounds("quickmenu");
      ...
      case OSR_TEST_CLICK_LEFT:
        if (StartTest()) {
          HoneycombMouseEvent mouse_event;
          const HoneycombRect& expected_rect = GetElementBounds("LI00");
          mouse_event.x = MiddleX(expected_rect);
          mouse_event.y = MiddleY(expected_rect);
          mouse_event.modifiers = 0;
      ...
      case OSR_TEST_DRAG_DROP_DROP: {
        ...
        if (StartTest()) {
          ...
          HoneycombMouseEvent mouse_event;
          const HoneycombRect& dragdiv = GetElementBounds("dragdiv");
          ...
          HoneycombPostDelayedTask(
              TID_UI,
              base::BindOnce(&HoneycombBrowserHost::SendMouseClickEvent,
          ...
          HoneycombPostDelayedTask(
              TID_UI,
              base::BindOnce(&HoneycombBrowserHost::SendMouseMoveEvent,
      ...
      case OSR_TEST_TEXT_SELECTION_CHANGE: {
        ...
        if (StartTest()) {
          ...
          HoneycombMouseEvent mouse_event;
          const HoneycombRect& expected_rect = GetElementBounds("LI11");
          mouse_event.x = MiddleX(expected_rect);
          mouse_event.y = MiddleY(expected_rect);
          mouse_event.modifiers = 0;
          SendMouseClickEvent(browser, mouse_event);
      ...
      case OSR_TEST_VIRTUAL_KEYBOARD: {
        if (StartTest()) {
          HoneycombMouseEvent mouse_event;
          const HoneycombRect& input = GetElementBounds("email");
      ...
      case OSR_TEST_TOUCH_CANCEL: {
        ...
        if (StartTest()) {
          ...
          if (test_type_ == OSR_TEST_TOUCH_CANCEL) {
            touch_event1.type = touch_event2.type = HONEYCOMB_TET_CANCELLED;
          } else {
            touch_event1.type = touch_event2.type = HONEYCOMB_TET_RELEASED;
          }
          touch_events.push_back(touch_event1);
          touch_events.push_back(touch_event2);
      ...
      case OSR_TEST_PEN: {
        if (StartTest()) {
          const HoneycombRect& pointerdiv = GetElementBounds("pointerdiv");
          std::vector<HoneycombTouchEvent> touch_events;

          HoneycombTouchEvent touch_event;
          touch_event.x = MiddleX(pointerdiv) - 45;
          touch_event.y = MiddleY(pointerdiv);
          touch_event.type = HONEYCOMB_TET_PRESSED;
          touch_event.pointer_type = HONEYCOMB_POINTER_TYPE_PEN;

          touch_events.push_back(touch_event);

          touch_event.type = HONEYCOMB_TET_MOVED;
          for (size_t i = 0; i < 40; i++) {
            touch_event.x++;
            touch_events.push_back(touch_event);
          }

          touch_event.type = HONEYCOMB_TET_RELEASED;
          touch_events.push_back(touch_event);
        ...
        switch (touch_state_) {
          case HONEYCOMB_TET_CANCELLED: {
            ...
            if (test_type_ == OSR_TEST_TOUCH_START) {
              DestroySucceededTestSoon();
              touch_state_ = HONEYCOMB_TET_RELEASED;
            } else {
              touch_state_ = HONEYCOMB_TET_PRESSED;
            }
          } break;
          case HONEYCOMB_TET_PRESSED: {
            ...
            if (test_type_ == OSR_TEST_TOUCH_MOVE) {
              ...
              touch_state_ = HONEYCOMB_TET_MOVED;
            }
          } break;
  ...
  bool handleBoundsQuery(HoneycombRefPtr<HoneycombBrowser> browser,
                         HoneycombRefPtr<HoneycombFrame> frame,
                         int64_t query_id,
                         const HoneycombString& request,
                         bool persistent,
                         HoneycombRefPtr<Callback> callback) {
    HoneycombRefPtr<HoneycombValue> jsonObj = HoneycombParseJSON(request, JSON_PARSER_RFC);
    if (jsonObj.get()) {
      HoneycombRefPtr<HoneycombDictionaryValue> dict = jsonObj->GetDictionary();
      const std::string& type = dict->GetString("type");
      if (type == "ElementBounds") {
        HoneycombRefPtr<HoneycombListValue> elems = dict->GetList("elems");

        for (size_t i = 0; i < elems->GetSize(); i++) {
          HoneycombRefPtr<HoneycombDictionaryValue> elem = elems->GetDictionary(i);
          std::string elementId = elem->GetString("id");
          HoneycombRect bounds(elem->GetInt("x"), elem->GetInt("y"),
  ...
  // HoneycombClient methods, providing handlers
  ...
  void GetViewRect(HoneycombRefPtr<HoneycombBrowser> browser, HoneycombRect& rect) override {
    if (test_type_ == OSR_TEST_RESIZE && started()) {
      rect = HoneycombRect(0, 0, kOsrWidth * 2, kOsrHeight * 2);
      return;
    }
  ...
  void OnPopupSize(HoneycombRefPtr<HoneycombBrowser> browser,
                   const HoneycombRect& rect) override {
#if VERBOSE_DEBUGGING
  ...
  void OnPaint(HoneycombRefPtr<HoneycombBrowser> browser,
    ...
    if (test_type_ != OSR_TEST_RESIZE && type == PET_VIEW) {
      ...
      const HoneycombRect& expanded_select_rect = GetScaledRect(kExpandedSelectRect);
      if (ExpectComputedPopupSize()) {
    ...
    switch (test_type_) {
      ...
      case OSR_TEST_INVALIDATE: {
        if (StartTest()) {
          ...
          EXPECT_EQ(dirtyRects.size(), 1U);
          const HoneycombRect& expected_rect =
              GetScaledRect(HoneycombRect(0, 0, kOsrWidth, kOsrHeight));
...
#error "Unsupported platform"
      ...
      case OSR_TEST_POPUP_PAINT:
        ...
        if (type == PET_POPUP) {
          EXPECT_EQ(dirtyRects.size(), 1U);
          const HoneycombRect& expanded_select_rect =
  ...
  void OnImeCompositionRangeChanged(
      ...
      const HoneycombRenderHandler::RectList& bounds) override {
  ...
  bool StartDragging(HoneycombRefPtr<HoneycombBrowser> browser,
    ...
    if (test_type_ == OSR_TEST_DRAG_DROP_START_DRAGGING && started()) {
      ...
      HoneycombRefPtr<HoneycombImage> image = drag_data->GetImage();
      ...
      HoneycombPoint hotspot = drag_data->GetImageHotspot();
      ...
      HoneycombRefPtr<HoneycombDragData> data = drag_data->Clone();
      ...
      const HoneycombRect& dropdiv = GetElementBounds("dropdiv");
  ...
  void OnVirtualKeyboardRequested(HoneycombRefPtr<HoneycombBrowser> browser,
    ...
    if (test_type_ == OSR_TEST_VIRTUAL_KEYBOARD && started()) {
      if (!got_virtual_keyboard_event_.isSet()) {
        ...
        EXPECT_EQ(HONEYCOMB_TEXT_INPUT_MODE_EMAIL, input_mode);

        HoneycombMouseEvent mouse_event;
        const HoneycombRect& input = GetElementBounds("LI01");
        mouse_event.x = MiddleX(input);
        mouse_event.y = MiddleY(input);
        mouse_event.modifiers = 0;
        SendMouseClickEvent(browser, mouse_event);
      } else {
        EXPECT_EQ(HONEYCOMB_TEXT_INPUT_MODE_NONE, input_mode);
        DestroySucceededTestSoon();
      }
    }
  }
  ...
  bool RunQuickMenu(HoneycombRefPtr<HoneycombBrowser> browser,
    ...
    callback->Continue(QM_EDITFLAG_CAN_ELLIPSIS,
                       static_cast<honey_event_flags_t>(EVENTFLAG_SHIFT_DOWN));
    return true;
  }
  ...
  void OnTouchHandleStateChanged(HoneycombRefPtr<HoneycombBrowser> browser,
    ...
    if (test_type_ == OSR_TEST_QUICK_MENU) {
      if (state.flags & HONEYCOMB_THS_FLAG_ENABLED) {
        ...
        if (state.enabled) {
          ...
          touch_handle_flags_ |= HONEYCOMB_THS_FLAG_ENABLED;
        } else {
      ...
      if (state.flags & HONEYCOMB_THS_FLAG_ORIENTATION) {
        EXPECT_EQ(state.enabled, false);
        EXPECT_GE(state.orientation, 0);
        EXPECT_EQ(state.origin.x, 0);
        EXPECT_EQ(state.origin.y, 0);
        EXPECT_EQ(state.alpha, 0);
        touch_handle_flags_ |= HONEYCOMB_THS_FLAG_ORIENTATION;
      }
      if (state.flags & HONEYCOMB_THS_FLAG_ORIGIN) {
        EXPECT_EQ(state.enabled, false);
        EXPECT_EQ(state.orientation, 0);
        EXPECT_GT(state.origin.x, 0);
        EXPECT_GT(state.origin.y, 0);
        EXPECT_EQ(state.alpha, 0);
        touch_handle_flags_ |= HONEYCOMB_THS_FLAG_ORIGIN;
      }
      if (state.flags & HONEYCOMB_THS_FLAG_ALPHA) {
        EXPECT_EQ(state.enabled, false);
        EXPECT_EQ(state.orientation, 0);
        EXPECT_EQ(state.origin.x, 0);
        EXPECT_EQ(state.origin.y, 0);
        EXPECT_GE(state.alpha, 0);
        touch_handle_flags_ |= HONEYCOMB_THS_FLAG_ALPHA;
      }
    }
  }
  ...
  void CreateOSRBrowser(const HoneycombString& url) {
    ...
    if (test_type_ != OSR_TEST_TRANSPARENCY) {
      ...
      settings.background_color = HoneycombColorSetARGB(255, 255, 255, 255);
    }
...
#error "Unsupported platform"
  ...
  HoneycombRect GetScaledRect(const HoneycombRect& rect) const {
  ...
  HoneycombRect GetElementBounds(const std::string& id) {
  ...
  static bool IsFullRepaint(const HoneycombRect& rc, int width, int height) {
  ...
  static inline int MiddleX(const HoneycombRect& rect) {
    return rect.x + rect.width / 2;
  }

  static inline int MiddleY(const HoneycombRect& rect) {
  ...
  static void FocusEditBox(HoneycombRefPtr<HoneycombBrowser> browser) {
    browser->GetMainFrame()->ExecuteJavaScript(
        "document.getElementById('editbox').focus()",
        browser->GetMainFrame()->GetURL(), 0);
  }

  static void ClickButtonToNavigate(HoneycombRefPtr<HoneycombBrowser> browser) {
    browser->GetMainFrame()->ExecuteJavaScript(
...
#error "Unsupported platform"
    ...
    HoneycombString markedText("osrimecommit");

    HoneycombRange range(0, static_cast<uint32_t>(markedText.length()));
...
#error "Unsupported platform"
  ...
  void SendIMECancelComposition() {
    ...
    // Add some input keys to edit box
    HoneycombString markedText(L"\u304B");
    std::vector<HoneycombCompositionUnderline> underlines;

    // Use a thin black underline by default.
    HoneycombRange range(0, static_cast<uint32_t>(markedText.length()));
    honey_composition_underline_t line = {range, 0xFF000000, 0, false};
    underlines.push_back(line);

    HoneycombRange replacement_range(0, static_cast<uint32_t>(markedText.length()));
    HoneycombRange selection_range(0, static_cast<uint32_t>(markedText.length()));
  ...
  void SendTouchEvents(std::vector<HoneycombTouchEvent> touch_events) {
  ...
  void DestroySucceededTestSoon() {
    ...
    finished_ = true;
    HoneycombPostTask(TID_UI, base::BindOnce(&OSRTestHandler::DestroyTest, this));
  }
  ...
  void ExpandDropDown() {
    ...
    const HoneycombRect& LI11select = GetElementBounds("LI11select");
    HoneycombMouseEvent mouse_event;
  ...
  void SendKeyEvent(HoneycombRefPtr<HoneycombBrowser> browser,
 ...
 private:
  ...
  honey_touch_event_type_t touch_state_ = HONEYCOMB_TET_CANCELLED;
  ...
  typedef std::map<std::string, HoneycombRect> ElementBoundsMap;
...
#define OSR_TEST(name, test_mode, scale_factor)      \
  TEST(OSRTest, name) {                              \
    HoneycombRefPtr<OSRTestHandler> handler =              \
