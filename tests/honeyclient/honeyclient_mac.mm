...
#import "include/wrapper/honey_library_loader.h"
...
@interface ClientApplication : NSApplication <HoneycombAppProtocol> {
 @private
...
- (HoneycombRefPtr<HoneycombBrowser>)getActiveBrowser {
  auto root_window =
...
- (NSWindow*)getActiveBrowserNSWindow {
  if (auto browser = [self getActiveBrowser]) {
    if (auto view = CAST_HONEYCOMB_WINDOW_HANDLE_TO_NSVIEW(
...
// a tag value from include/honey_command_ids.h. For example, 37000 is IDC_FIND
