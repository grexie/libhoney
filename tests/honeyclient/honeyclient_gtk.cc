...
#include "tests/honeyclient/browser/main_message_loop_multithreaded_gtk.h"
...
int RunMain(int argc, char* argv[]) {
  ...
  HoneycombScopedArgArray scoped_arg_array(argc, argv);
  ...
  int exit_code = HoneycombExecuteProcess(main_args, app, nullptr);
  if (exit_code >= 0) {
    return exit_code;
  }
...
// use of the sandbox.
  ...
  if (settings.windowless_rendering_enabled) {
    ...
    // TODO(honeyclient): Rewrite OSRRenderer to use shaders instead of the
  ...
  // initialization. Therefore initialize GTK after Honeycomb.
