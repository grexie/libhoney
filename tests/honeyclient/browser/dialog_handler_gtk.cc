...
const char kPromptTextId[] = "honey_prompt_text";
...
void AddFilters(GtkFileChooser* chooser,
  ...
  for (size_t j = 0; j < accept_filters.size(); ++j) {
    ...
    if (sep_index != std::string::npos) {
      ...
      std::vector<HoneycombString> ext;
      HoneycombGetExtensionsForMimeType(filter, ext);
...
bool ClientDialogHandlerGtk::OnJSDialog(HoneycombRefPtr<HoneycombBrowser> browser,
...
bool ClientDialogHandlerGtk::OnBeforeUnloadDialog(
  ...
  return OnJSDialog(browser, HoneycombString(), JSDIALOGTYPE_CONFIRM,
                    new_message_text, HoneycombString(), callback, suppress_message);
}

void ClientDialogHandlerGtk::OnResetDialogState(HoneycombRefPtr<HoneycombBrowser> browser) {
  HONEYCOMB_REQUIRE_UI_THREAD();
...
void ClientDialogHandlerGtk::OnFileDialogContinue(
  ...
  std::vector<HoneycombString> files;
...
void ClientDialogHandlerGtk::OnJSDialogContinue(const OnJSDialogParams& params,
  ...
  if (!params.origin_url.empty()) {
    title += " - ";
    title += HoneycombFormatUrlForSecurityDisplay(params.origin_url).ToString();
  }
...
void ClientDialogHandlerGtk::OnDialogResponse(GtkDialog* dialog,
  ...
  switch (response_id) {
    ...
    case GTK_RESPONSE_DELETE_EVENT:
      handler->js_dialog_callback_->Continue(false, HoneycombString());
      break;
    default:
      NOTREACHED();
  }
