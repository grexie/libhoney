...
char16_t ToLower(char16_t c) {
  HoneycombStringUTF16 str16;
  honey_string_utf16_to_lower(&c, 1, str16.GetWritableStruct());
...
HoneycombRefPtr<HoneycombPanel> ViewsMenuBar::GetMenuPanel() {
  EnsureMenuPanel();
  return panel_;
}

HoneycombRefPtr<HoneycombMenuModel> ViewsMenuBar::CreateMenuModel(const HoneycombString& label,
                                                      int* menu_id) {
  EnsureMenuPanel();
  ...
  HoneycombRefPtr<HoneycombMenuButton> button =
      HoneycombMenuButton::CreateMenuButton(this, label);
...
HoneycombRefPtr<HoneycombMenuModel> ViewsMenuBar::GetMenuModel(int menu_id) const {
...
bool ViewsMenuBar::OnKeyEvent(const HoneycombKeyEvent& event) {
  if (!panel_) {
    return false;
  }
...
void ViewsMenuBar::OnMenuButtonPressed(
  ...
  HoneycombRefPtr<HoneycombMenuModel> menu_model = GetMenuModel(menu_button->GetID());

  const auto button_bounds = menu_button->GetBoundsInScreen();

  // Adjust menu position to align with the button.
  HoneycombPoint point = screen_point;
  if (HoneycombIsRTL()) {
  ...
  menu_button->ShowMenu(menu_model, point, HONEYCOMB_MENU_ANCHOR_TOPLEFT);

  last_nav_with_keyboard_ = cur_last_nav_with_keyboard;
}

void ViewsMenuBar::ExecuteCommand(HoneycombRefPtr<HoneycombMenuModel> menu_model,
                                  int command_id,
                                  honey_event_flags_t event_flags) {
  delegate_->MenuBarExecuteCommand(menu_model, command_id, event_flags);
}

void ViewsMenuBar::MouseOutsideMenu(HoneycombRefPtr<HoneycombMenuModel> menu_model,
                                    const HoneycombPoint& screen_point) {
  DCHECK(panel_);

  // Retrieve the Window hosting the Panel.
  HoneycombRefPtr<HoneycombWindow> window = panel_->GetWindow();
  DCHECK(window);

  // Convert the point from screen to window coordinates.
  HoneycombPoint window_point = screen_point;
  if (!window->ConvertPointFromScreen(window_point)) {
    return;
  }

  HoneycombRect panel_bounds = panel_->GetBounds();
  ...
  for (int id = id_start_; id < id_next_; ++id) {
    ...
    HoneycombRefPtr<HoneycombView> button = panel_->GetViewForID(id);
    HoneycombRect button_bounds = button->GetBounds();
...
void ViewsMenuBar::UnhandledOpenSubmenu(HoneycombRefPtr<HoneycombMenuModel> menu_model,
                                        bool is_rtl) {
  TriggerNextMenu(is_rtl ? 1 : -1);
}

void ViewsMenuBar::UnhandledCloseSubmenu(HoneycombRefPtr<HoneycombMenuModel> menu_model,
                                         bool is_rtl) {
  TriggerNextMenu(is_rtl ? -1 : 1);
}

void ViewsMenuBar::MenuClosed(HoneycombRefPtr<HoneycombMenuModel> menu_model) {
...
void ViewsMenuBar::EnsureMenuPanel() {
  ...
  HoneycombBoxLayoutSettings top_panel_layout_settings;
...
int ViewsMenuBar::GetActiveMenuId() {
  ...
  for (int id = id_start_; id < id_next_; ++id) {
    HoneycombRefPtr<HoneycombButton> button = panel_->GetViewForID(id)->AsButton();
    if (button->GetState() == HONEYCOMB_BUTTON_STATE_PRESSED) {
      return id;
    }
  }
...
void ViewsMenuBar::TriggerMenuButton(HoneycombRefPtr<HoneycombView> button) {
  HoneycombRefPtr<HoneycombMenuButton> menu_button =
