...
HoneycombString GetJSON(HoneycombRefPtr<HoneycombDictionaryValue> dictionary) {
  HoneycombRefPtr<HoneycombValue> value = HoneycombValue::Create();
  value->SetDictionary(dictionary);
  return HoneycombWriteJSON(value, JSON_WRITER_DEFAULT);
}

typedef HoneycombMessageRouterBrowserSide::Callback CallbackType;

void SendSuccess(HoneycombRefPtr<CallbackType> callback,
                 HoneycombRefPtr<HoneycombDictionaryValue> result) {
  callback->Success(GetJSON(result));
}

void SendFailure(HoneycombRefPtr<CallbackType> callback,
                 int error_code,
                 const std::string& error_message) {
  callback->Failure(error_code, error_message);
}

// Callback for HoneycombMediaRouter::CreateRoute.
class MediaRouteCreateCallback : public HoneycombMediaRouteCreateCallback {
 public:
  explicit MediaRouteCreateCallback(HoneycombRefPtr<CallbackType> create_callback)
      : create_callback_(create_callback) {}

  // HoneycombMediaRouteCreateCallback method:
 ...
 private:
  HoneycombRefPtr<CallbackType> create_callback_;
...
class MediaObserver : public HoneycombMediaObserver {
 public:
  typedef std::vector<HoneycombRefPtr<HoneycombMediaRoute>> MediaRouteVector;
  typedef std::vector<HoneycombRefPtr<HoneycombMediaSink>> MediaSinkVector;

  MediaObserver(HoneycombRefPtr<HoneycombMediaRouter> media_router,
                HoneycombRefPtr<CallbackType> subscription_callback)
  ...
  bool CreateRoute(const std::string& source_urn,
                   ...
                   HoneycombRefPtr<CallbackType> callback,
                   std::string& error) {
    HoneycombRefPtr<HoneycombMediaSource> source = GetSource(source_urn);
    if (!source) {
      error = "Invalid source: " + source_urn;
      return false;
    }

    HoneycombRefPtr<HoneycombMediaSink> sink = GetSink(sink_id);
    if (!sink) {
  ...
  bool TerminateRoute(const std::string& route_id, std::string& error) {
    HoneycombRefPtr<HoneycombMediaRoute> route = GetRoute(route_id);
    if (!route) {
 ...
 protected:
  class DeviceInfoCallback : public HoneycombMediaSinkDeviceInfoCallback {
   public:
  ...
  void OnSinks(const MediaSinkVector& sinks) override {
    ...
    for (size_t idx = 0; it != sinks.end(); ++it, ++idx) {
      HoneycombRefPtr<HoneycombMediaSink> sink = *it;
  ...
  void OnRoutes(const MediaRouteVector& routes) override {
    ...
    HoneycombRefPtr<HoneycombDictionaryValue> payload = HoneycombDictionaryValue::Create();
    HoneycombRefPtr<HoneycombListValue> routes_list = HoneycombListValue::Create();
    routes_list->SetSize(routes.size());

    MediaRouteVector::const_iterator it = routes.begin();
    for (size_t idx = 0; it != routes.end(); ++it, ++idx) {
      HoneycombRefPtr<HoneycombMediaRoute> route = *it;
      const std::string& route_id = route->GetId();
      route_map_.insert(std::make_pair(route_id, route));

      HoneycombRefPtr<HoneycombDictionaryValue> route_dict = HoneycombDictionaryValue::Create();
 ...
 private:
  HoneycombRefPtr<HoneycombMediaSource> GetSource(const std::string& source_urn) {
    HoneycombRefPtr<HoneycombMediaSource> source = media_router_->GetSource(source_urn);
    if (!source) {
      return nullptr;
    }
    return source;
  ...
  HoneycombRefPtr<HoneycombMediaSink> GetSink(const std::string& sink_id) {
  ...
  HoneycombRefPtr<HoneycombMediaRoute> GetRoute(const std::string& route_id) {
  ...
  void SendResponse(const std::string& name,
                    HoneycombRefPtr<HoneycombDictionaryValue> payload) {
    HoneycombRefPtr<HoneycombDictionaryValue> result = HoneycombDictionaryValue::Create();
  ...
  void SendSinksResponse() {
    HoneycombRefPtr<HoneycombDictionaryValue> payload = HoneycombDictionaryValue::Create();
    HoneycombRefPtr<HoneycombListValue> sinks_list = HoneycombListValue::Create();
    ...
    for (size_t idx = 0; it != sink_info_map_.end(); ++it, ++idx) {
      ...
      HoneycombRefPtr<HoneycombDictionaryValue> sink_dict = HoneycombDictionaryValue::Create();
      ...
      sink_dict->SetString("ip_address",
                           HoneycombString(&info->device_info.ip_address));
      sink_dict->SetInt("port", info->device_info.port);
      sink_dict->SetString("model_name",
                           HoneycombString(&info->device_info.model_name));
  ...
  HoneycombRefPtr<HoneycombMediaRouter> media_router_;
  HoneycombRefPtr<CallbackType> subscription_callback_;
  ...
  typedef std::map<std::string, HoneycombRefPtr<HoneycombMediaRoute>> RouteMap;
  RouteMap route_map_;
...
class Handler : public HoneycombMessageRouterBrowserSide::Handler {
 public:
  ...
  // Called due to honeyQuery execution in media_router.html.
  bool OnQuery(HoneycombRefPtr<HoneycombBrowser> browser,
    ...
    HoneycombRefPtr<MediaObserver> media_observer =
  ...
  void OnQueryCanceled(HoneycombRefPtr<HoneycombBrowser> browser,
                       HoneycombRefPtr<HoneycombFrame> frame,
 ...
 private:
  static void SendSuccessACK(HoneycombRefPtr<Callback> callback) {
    HoneycombRefPtr<HoneycombDictionaryValue> result = HoneycombDictionaryValue::Create();
  ...
  struct SubscriptionState {
    int64_t query_id;
    HoneycombRefPtr<MediaObserver> observer;
    HoneycombRefPtr<HoneycombRegistration> registration;
  };

  bool CreateSubscription(HoneycombRefPtr<HoneycombBrowser> browser,
                          int64_t query_id,
                          HoneycombRefPtr<Callback> callback) {
    ...
    HoneycombRefPtr<HoneycombMediaRouter> media_router =
  ...
  HoneycombRefPtr<MediaObserver> GetMediaObserver(int browser_id) {
