...
class ClientRequestContextHandler : public HoneycombRequestContextHandler,
                                    public HoneycombExtensionHandler {
 public:
...
scoped_refptr<RootWindow> RootWindowManager::CreateRootWindowAsPopup(
  ...
  if (MainContext::Get()->UseDefaultPopup()) {
    ...
    // still required by honeyclient architecture.
    client = new ClientHandlerStd(/*delegate=*/nullptr, with_controls,
                                  /*startup_url=*/HoneycombString());
    return nullptr;
  }
...
bool RootWindowManager::HasRootWindowAsExtension(
  ...
  for (auto root_window : root_windows_) {
    ...
    HoneycombRefPtr<HoneycombBrowser> browser = root_window->GetBrowser();
    if (!browser) {
      continue;
    }

    HoneycombRefPtr<HoneycombExtension> browser_extension =
...
HoneycombRefPtr<HoneycombBrowser> RootWindowManager::GetActiveBrowser() const {
...
void RootWindowManager::AddExtension(HoneycombRefPtr<HoneycombExtension> extension) {
  if (!CURRENTLY_ON_MAIN_THREAD()) {
    // Execute this method on the main thread.
...
HoneycombRefPtr<HoneycombRequestContext> RootWindowManager::GetRequestContext(
  ...
  if (request_context_per_browser_) {
    ...
    if (command_line->HasSwitch(switches::kCachePath)) {
      if (request_context_shared_cache_) {
        ...
        HoneycombString(&settings.cache_path) = ss.str();
      }
    }

    return HoneycombRequestContext::CreateContext(settings,
                                            new ClientRequestContextHandler);
  }

  // All browsers will share the global request context.
  if (!shared_request_context_.get()) {
    shared_request_context_ = HoneycombRequestContext::CreateContext(
        HoneycombRequestContext::GetGlobalContext(), new ClientRequestContextHandler);
  }
...
void RootWindowManager::OnRootWindowDestroyed(RootWindow* root_window) {
  ...
  if (terminate_when_all_windows_closed_ && root_windows_.empty()) {
    ...
    HoneycombPostTask(TID_UI, base::BindOnce(&RootWindowManager::CleanupOnUIThread,
