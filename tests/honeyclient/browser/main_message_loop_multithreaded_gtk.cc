...
int MainMessageLoopMultithreadedGtk::Run() {
  ...
  // TODO(honey): It might be more efficient to use input functions
...
void MainMessageLoopMultithreadedGtk::Quit() {
  PostTask(HoneycombCreateClosureTask(base::BindOnce(
      &MainMessageLoopMultithreadedGtk::DoQuit, base::Unretained(this))));
}

void MainMessageLoopMultithreadedGtk::PostTask(HoneycombRefPtr<HoneycombTask> task) {
...
void MainMessageLoopMultithreadedGtk::RunTasks() {
  ...
  std::queue<HoneycombRefPtr<HoneycombTask>> tasks;
  ...
  while (!tasks.empty()) {
    HoneycombRefPtr<HoneycombTask> task = tasks.front();
    tasks.pop();
    task->Execute();
  }
}
