...
HoneycombRefPtr<HoneycombBrowser> BrowserWindow::GetBrowser() const {
  REQUIRE_MAIN_THREAD();
  return browser_;
}
...
void BrowserWindow::OnBrowserCreated(HoneycombRefPtr<HoneycombBrowser> browser) {
  REQUIRE_MAIN_THREAD();
  DCHECK(!browser_);
  browser_ = browser;
...
void BrowserWindow::OnBrowserClosing(HoneycombRefPtr<HoneycombBrowser> browser) {
  REQUIRE_MAIN_THREAD();
...
void BrowserWindow::OnBrowserClosed(HoneycombRefPtr<HoneycombBrowser> browser) {
  REQUIRE_MAIN_THREAD();
  if (browser_.get()) {
    DCHECK_EQ(browser->GetIdentifier(), browser_->GetIdentifier());
    browser_ = nullptr;
...
void BrowserWindow::OnAutoResize(const HoneycombSize& new_size) {
  REQUIRE_MAIN_THREAD();
