...
honey_color_t ParseColor(const std::string& color) {
  const std::string& colorToLower = AsciiStrToLower(color);
  if (colorToLower == "black") {
    return HoneycombColorSetARGB(255, 0, 0, 0);
  } else if (colorToLower == "blue") {
    return HoneycombColorSetARGB(255, 0, 0, 255);
  } else if (colorToLower == "green") {
    return HoneycombColorSetARGB(255, 0, 255, 0);
  } else if (colorToLower == "red") {
    return HoneycombColorSetARGB(255, 255, 0, 0);
  } else if (colorToLower == "white") {
    return HoneycombColorSetARGB(255, 255, 255, 255);
  }
...
MainContextImpl::MainContextImpl(HoneycombRefPtr<HoneycombCommandLine> command_line,
...
// Choose a reasonable default rate based on the OSR mode.
  ...
  if (use_chrome_runtime_ && !use_views_) {
    ...
    // styled Chrome window with honeyclient menu customizations). In the mean
    // time this can be demo'd with "honeysimple --enable-chrome-runtime".
  ...
  if (background_color_ == 0 && !use_views_) {
    ...
    background_color_ = HoneycombColorSetARGB(255, 255, 255, 255);
  }
...
honey_color_t MainContextImpl::GetBackgroundColor() {
...
void MainContextImpl::PopulateSettings(HoneycombSettings* settings) {
  ...
  HoneycombString(&settings->cache_path) =
  ...
  if (command_line_->HasSwitch("lang")) {
    ...
    HoneycombString(&settings->accept_language_list) =
  ...
  if (command_line_->HasSwitch("enable-chrome-policy")) {
    ...
    HoneycombString(&settings->chrome_policy_id) =
#if defined(OS_WIN)
...
void MainContextImpl::PopulateBrowserSettings(HoneycombBrowserSettings* settings) {
...
bool MainContextImpl::Initialize(const HoneycombMainArgs& args,
  ...
  if (!HoneycombInitialize(args, settings, application, windows_sandbox_info)) {
    return false;
  }

  // Need to create the RootWindowManager after calling HoneycombInitialize because
  // TempWindowX11 uses honey_get_xdisplay().
