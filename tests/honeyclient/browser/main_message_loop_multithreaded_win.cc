...
int MainMessageLoopMultithreadedWin::Run() {
  ...
  HACCEL hAccelTable =
      LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_HONEYCOMBCLIENT));
...
void MainMessageLoopMultithreadedWin::PostTask(HoneycombRefPtr<HoneycombTask> task) {
  base::AutoLock lock_scope(lock_);
...
MainMessageLoopMultithreadedWin::MessageWndProc(HWND hWnd,
  ...
  if (self && message == self->task_message_id_) {
    // Execute the task.
    HoneycombTask* task = reinterpret_cast<HoneycombTask*>(wParam);
    task->Execute();
