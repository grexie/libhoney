...
void GetCompositionUnderlines(
  ...
  if (clause_length) {
    ...
    for (int i = 0; i < clause_length - 1; ++i) {
      honey_composition_underline_t underline = {};
...
void OsrImeHandlerWin::GetCompositionInfo(
  ...
  if (!underlines.size()) {
    HoneycombCompositionUnderline underline;
    underline.color = ColorUNDERLINE;
    underline.background_color = ColorBKCOLOR;
    if (target_start > 0) {
...
bool OsrImeHandlerWin::GetResult(LPARAM lparam, HoneycombString& result) {
  bool ret = false;
  HIMC imc = ::ImmGetContext(hwnd_);
  if (imc) {
...
void OsrImeHandlerWin::ChangeCompositionRange(
    ...
    const std::vector<HoneycombRect>& bounds) {
