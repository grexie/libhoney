...
inline int MiddleX(const HoneycombRect& rect) {
  return rect.x + rect.width / 2;
}

inline int MiddleY(const HoneycombRect& rect) {
...
@interface OsrAXNodeObject : NSObject {
  ...
  HoneycombNativeAccessible* parent_;
}
...
- (NSString*)axRole {
  // Get the Role from HoneycombAccessibilityHelper and Map to NSRole
...
- (void)doMouseClick:(honey_mouse_button_type_t)type {
...
- (NSMutableArray*)getKids {
  ...
  if (numChilds > 0) {
    ...
    for (int index = 0; index < numChilds; index++) {
      ...
      [kids addObject:child ? CAST_HONEYCOMB_NATIVE_ACCESSIBLE_TO_NSOBJECT(
...
- (NSPoint)position {
  HoneycombRect honey_rect = node_->AxLocation();
  NSPoint origin = NSMakePoint(honey_rect.x, honey_rect.y);
  NSSize size = NSMakeSize(honey_rect.width, honey_rect.height);

  NSView* view = CAST_HONEYCOMB_WINDOW_HANDLE_TO_NSVIEW(node_->GetWindowHandle());
...
- (NSSize)size {
  HoneycombRect honey_rect = node_->AxLocation();
  NSRect rect =
      NSMakeRect(honey_rect.x, honey_rect.y, honey_rect.width, honey_rect.height);
  NSView* view = CAST_HONEYCOMB_WINDOW_HANDLE_TO_NSVIEW(node_->GetWindowHandle());
...
- (id)accessibilityAttributeValue:(NSString*)attribute {
  NSObject* typed_parent = CAST_HONEYCOMB_NATIVE_ACCESSIBLE_TO_NSOBJECT(parent_);
  if (!node_) {
    return nil;
  }
...
void OsrAXNode::NotifyAccessibilityEvent(std::string event_type) const {
  NSView* view = CAST_HONEYCOMB_WINDOW_HANDLE_TO_NSVIEW(GetWindowHandle());
  if (event_type == "focus") {
...
void OsrAXNode::Destroy() {
  if (platform_accessibility_) {
    NSAccessibilityPostNotification(
        CAST_HONEYCOMB_NATIVE_ACCESSIBLE_TO_NSOBJECT(platform_accessibility_),
...
HoneycombNativeAccessible* OsrAXNode::GetNativeAccessibleObject(
    client::OsrAXNode* parent) {
  if (!platform_accessibility_) {
    platform_accessibility_ = CAST_NSOBJECT_TO_HONEYCOMB_NATIVE_ACCESSIBLE(
