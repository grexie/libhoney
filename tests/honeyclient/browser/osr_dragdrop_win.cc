...
DWORD DragOperationToDropEffect(HoneycombRenderHandler::DragOperation allowed_ops) {
...
HoneycombRenderHandler::DragOperationsMask DropEffectToDragOperation(DWORD effect) {
  ...
  return static_cast<HoneycombRenderHandler::DragOperationsMask>(operation);
}

HoneycombMouseEvent ToMouseEvent(POINTL p, DWORD key_state, HWND hWnd) {
  HoneycombMouseEvent ev;
  POINT screen_point = {p.x, p.y};
  ScreenToClient(hWnd, &screen_point);
  ev.x = screen_point.x;
  ev.y = screen_point.y;
  ev.modifiers = GetHoneycombMouseModifiers(key_state);
  return ev;
}
...
bool DragDataToDataObject(HoneycombRefPtr<HoneycombDragData> drag_data,
  ...
  int curr_index = 0;
  HoneycombString text = drag_data->GetFragmentText();
  if (!text.empty()) {
  ...
  HoneycombString html = drag_data->GetFragmentHtml();
  if (!html.empty()) {
    HoneycombString base_url = drag_data->GetFragmentBaseURL();
  ...
  if (bufferSize) {
    HoneycombRefPtr<BytesWriteHandler> handler = new BytesWriteHandler(bufferSize);
    ...
    HoneycombString fileName = drag_data->GetFileName();
...
HoneycombRefPtr<HoneycombDragData> DataObjectToDragData(IDataObject* data_object) {
  HoneycombRefPtr<HoneycombDragData> drag_data = HoneycombDragData::Create();
    ...
    for (unsigned i = 0; i < celtFetched; i++) {
      ...
      if (data_object->GetData(&rgelt[i], &medium) == S_OK) {
        ...
        if (format == CF_UNICODETEXT) {
          ...
          HoneycombString url(html.substr(0, pos));
          HoneycombString title(html.substr(pos + 1));
...
HRESULT DropTargetWin::DragEnter(IDataObject* data_object,
  ...
  HoneycombRefPtr<HoneycombDragData> drag_data = current_drag_data_;
  if (!drag_data) {
    drag_data = DataObjectToDragData(data_object);
  }
  HoneycombMouseEvent ev = ToMouseEvent(cursor_position, key_state, hWnd_);
  HoneycombBrowserHost::DragOperationsMask mask = DropEffectToDragOperation(*effect);
  mask = callback_->OnDragEnter(drag_data, ev, mask);
  *effect = DragOperationToDropEffect(mask);
  return S_OK;
}

HoneycombBrowserHost::DragOperationsMask DropTargetWin::StartDragging(
