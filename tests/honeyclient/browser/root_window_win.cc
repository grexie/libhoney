...
#include "tests/honeyclient/browser/root_window_win.h"
...
#include "tests/honeyclient/browser/browser_window_osr_win.h"
...
void RootWindowWin::Init(RootWindow::Delegate* delegate,
  ...
  if (HoneycombCurrentlyOn(TID_UI)) {
    ...
    HoneycombPostTask(TID_UI, base::BindOnce(&RootWindowWin::ContinueInitOnUIThread,
...
void RootWindowWin::ContinueInitOnUIThread(
  ...
  if (!config->bounds.IsEmpty()) {
    ...
    if (prefs::LoadWindowRestorePreferences(initial_show_state_, bounds) &&
        bounds) {
      initial_bounds_ = HoneycombDisplay::ConvertScreenRectToPixels(*bounds);
    }
  }
...
HoneycombRefPtr<HoneycombBrowser> RootWindowWin::GetBrowser() const {
  REQUIRE_MAIN_THREAD();
...
void RootWindowWin::CreateRootWindow(const HoneycombBrowserSettings& settings,
  ...
  const std::wstring& window_class = GetResourceString(IDC_HONEYCOMBCLIENT);
  ...
  if (!initially_hidden) {
    ...
    if (no_activate) {
      mode = ShowNoActivate;
    } else if (initial_show_state_ == HONEYCOMB_SHOW_STATE_MAXIMIZED) {
      mode = ShowMaximized;
    } else if (initial_show_state_ == HONEYCOMB_SHOW_STATE_MINIMIZED) {
      mode = ShowMinimized;
    }
...
void RootWindowWin::RegisterRootClass(HINSTANCE hInstance,
  ...
  wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_HONEYCOMBCLIENT));
  wcex.hCursor = LoadCursor(nullptr, IDC_ARROW);
  wcex.hbrBackground = background_brush;
  wcex.lpszMenuName = MAKEINTRESOURCE(IDC_HONEYCOMBCLIENT);
...
bool RootWindowWin::OnCommand(UINT id) {
  ...
  switch (id) {
    ...
    case IDC_NAV_BACK:  // Back button
      if (HoneycombRefPtr<HoneycombBrowser> browser = GetBrowser()) {
        browser->GoBack();
      }
      return true;
...
void RootWindowWin::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  if (!is_popup_) {
    ...
    HoneycombRect honey_rect(rect.left, rect.top, rect.right - rect.left,
                     rect.bottom - rect.top);
    browser_window_->CreateBrowser(hwnd_, honey_rect, browser_settings_, nullptr,
...
void RootWindowWin::OnBrowserCreated(HoneycombRefPtr<HoneycombBrowser> browser) {
  REQUIRE_MAIN_THREAD();
...
void RootWindowWin::OnSetAddress(const std::string& url) {
  ...
  if (edit_hwnd_) {
    SetWindowText(edit_hwnd_, HoneycombString(url).ToWString().c_str());
  }
}
...
void RootWindowWin::OnSetTitle(const std::string& title) {
  ...
  if (hwnd_) {
    SetWindowText(hwnd_, HoneycombString(title).ToWString().c_str());
  }
}
...
void RootWindowWin::OnAutoResize(const HoneycombSize& new_size) {
  REQUIRE_MAIN_THREAD();
...
LPCWSTR kParentWndProc = L"HoneycombParentWndProc";
LPCWSTR kDraggableRegion = L"HoneycombDraggableRegion";
...
void RootWindowWin::OnSetDraggableRegions(
  ...
  std::vector<HoneycombDraggableRegion>::const_iterator it = regions.begin();
...
void RootWindowWin::SaveWindowRestoreOnUIThread(
  ...
  HoneycombRect pixel_bounds(rect.left, rect.top, rect.right - rect.left,
                       rect.bottom - rect.top);
  const auto dip_bounds = HoneycombDisplay::ConvertScreenRectFromPixels(pixel_bounds);
