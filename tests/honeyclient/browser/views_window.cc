...
typedef std::vector<HoneycombRefPtr<HoneycombLabelButton>> LabelButtons;
...
void MakeButtonsSameSize(const LabelButtons& buttons) {
  ...
  for (size_t i = 0U; i < buttons.size(); ++i) {
    const HoneycombSize& button_size = buttons[i]->GetPreferredSize();
...
void AddTestMenuItems(HoneycombRefPtr<HoneycombMenuModel> test_menu) {
...
void AddFileMenuItems(HoneycombRefPtr<HoneycombMenuModel> file_menu) {
...
HoneycombBrowserViewDelegate::ChromeToolbarType CalculateChromeToolbarType(
  ...
  if (toolbar_type == "location") {
    return HONEYCOMB_CTT_LOCATION;
  }

  return with_overlay_controls ? HONEYCOMB_CTT_LOCATION : HONEYCOMB_CTT_NORMAL;
}

}  // namespace

// static
HoneycombRefPtr<ViewsWindow> ViewsWindow::Create(
    WindowType type,
    Delegate* delegate,
  ...
  HoneycombRefPtr<ViewsWindow> views_window =
  ...
  HoneycombWindow::CreateTopLevelWindow(views_window);
...
void ViewsWindow::SetBounds(const HoneycombRect& bounds) {
  HONEYCOMB_REQUIRE_UI_THREAD();
  if (window_) {
...
void ViewsWindow::SetBrowserSize(const HoneycombSize& size,
                                 bool has_position,
...
void ViewsWindow::Close(bool force) {
  ...
  if (browser) {
    // This will result in a call to HoneycombWindow::Close() which will then call
...
void ViewsWindow::SetFavicon(HoneycombRefPtr<HoneycombImage> image) {
  HONEYCOMB_REQUIRE_UI_THREAD();
...
void ViewsWindow::SetLoadingState(bool isLoading,
                                  ...
                                  bool canGoForward) {
  HONEYCOMB_REQUIRE_UI_THREAD();
  if (!window_ || chrome_toolbar_type_ == HONEYCOMB_CTT_NORMAL) {
    return;
  }
...
void ViewsWindow::SetDraggableRegions(
  ...
  std::vector<HoneycombDraggableRegion> window_regions = regions;
  for (auto& region : window_regions) {
    HoneycombPoint origin = HoneycombPoint(region.bounds.x, region.bounds.y);
...
void ViewsWindow::TakeFocus(bool next) {
  ...
  if (chrome_toolbar_type_ == HONEYCOMB_CTT_NORMAL) {
...
void ViewsWindow::OnBeforeContextMenu(HoneycombRefPtr<HoneycombMenuModel> model) {
  HONEYCOMB_REQUIRE_UI_THREAD();
...
void ViewsWindow::OnExtensionsChanged(const ExtensionSet& extensions) {
  ...
  for (; it != extensions.end(); ++it) {
    HoneycombRefPtr<HoneycombExtension> extension = *it;
    bool internal = false;
...
bool ViewsWindow::GetWindowRestorePreferences(
  ...
  show_state = HONEYCOMB_SHOW_STATE_NORMAL;
  if (window_->IsMinimized()) {
    show_state = HONEYCOMB_SHOW_STATE_MINIMIZED;
  } else if (window_->IsFullscreen()) {
    // On MacOS, IsMaximized() will also return true for fullscreen, so check
    // IsFullscreen() first.
    show_state = HONEYCOMB_SHOW_STATE_FULLSCREEN;
  } else if (window_->IsMaximized()) {
    show_state = HONEYCOMB_SHOW_STATE_MAXIMIZED;
  }

  if (show_state == HONEYCOMB_SHOW_STATE_NORMAL) {
...
HoneycombRefPtr<HoneycombBrowserViewDelegate> ViewsWindow::GetDelegateForPopupBrowserView(
    HoneycombRefPtr<HoneycombBrowserView> browser_view,
    const HoneycombBrowserSettings& settings,
    HoneycombRefPtr<HoneycombClient> client,
    bool is_devtools) {
  ...
  // The popup browser client is created in HoneycombLifeSpanHandler::OnBeforePopup()
...
bool ViewsWindow::OnPopupBrowserViewCreated(
  ...
  HoneycombRefPtr<ViewsWindow> popup_window =
      static_cast<ViewsWindow*>(static_cast<HoneycombBrowserViewDelegate*>(
  ...
  // creation.
  HoneycombWindow::CreateTopLevelWindow(popup_window);

  // We created the Window.
  return true;
}

HoneycombBrowserViewDelegate::ChromeToolbarType ViewsWindow::GetChromeToolbarType(
    HoneycombRefPtr<HoneycombBrowserView> browser_view) {
...
void ViewsWindow::OnButtonPressed(HoneycombRefPtr<HoneycombButton> button) {
  HONEYCOMB_REQUIRE_UI_THREAD();
...
void ViewsWindow::OnMenuButtonPressed(
  ...
  menu_button->ShowMenu(button_menu_model_, point,
                        with_overlay_controls_ ? HONEYCOMB_MENU_ANCHOR_TOPLEFT
                                               : HONEYCOMB_MENU_ANCHOR_TOPRIGHT);
}

void ViewsWindow::ExecuteCommand(HoneycombRefPtr<HoneycombMenuModel> menu_model,
                                 int command_id,
...
bool ViewsWindow::OnKeyEvent(HoneycombRefPtr<HoneycombTextfield> textfield,
  ...
  if (window_ && browser_view_ && event.type == KEYEVENT_RAWKEYDOWN &&
    ...
    HoneycombRefPtr<HoneycombBrowser> browser = browser_view_->GetBrowser();
    if (browser) {
      const HoneycombString& url = textfield->GetText();
      if (!url.empty()) {
...
void ViewsWindow::OnWindowFullscreenTransition(HoneycombRefPtr<HoneycombWindow> window,
                                               bool is_completed) {
#if defined(OS_MAC)
...
void ViewsWindow::OnWindowCreated(HoneycombRefPtr<HoneycombWindow> window) {
  HONEYCOMB_REQUIRE_UI_THREAD();
  ...
  if (type_ == WindowType::NORMAL) {
    const HoneycombRect bounds = delegate_->GetInitialBounds();
    if (bounds.IsEmpty()) {
      // Size the Window and center it at the default size.
      window_->CenterWindow(HoneycombSize(kDefaultWidth, kDefaultHeight));
    } else {
  ...
  if (with_controls_) {
    ...
    if (delegate_->GetInitialShowState() == HONEYCOMB_SHOW_STATE_FULLSCREEN) {
    ...
    if (type_ != WindowType::EXTENSION) {
      ...
      minimum_window_size_ = HoneycombSize(100, 100);
    }
  }
...
void ViewsWindow::OnWindowClosing(HoneycombRefPtr<HoneycombWindow> window) {
  HONEYCOMB_REQUIRE_UI_THREAD();
  DCHECK(window_);
...
void ViewsWindow::OnWindowDestroyed(HoneycombRefPtr<HoneycombWindow> window) {
  HONEYCOMB_REQUIRE_UI_THREAD();
  DCHECK(window_);
...
void ViewsWindow::OnWindowActivationChanged(HoneycombRefPtr<HoneycombWindow> window,
                                            bool active) {
  if (!active) {
    return;
  }
...
void ViewsWindow::OnWindowBoundsChanged(HoneycombRefPtr<HoneycombWindow> window,
...
bool ViewsWindow::CanClose(HoneycombRefPtr<HoneycombWindow> window) {
  HONEYCOMB_REQUIRE_UI_THREAD();
...
HoneycombRefPtr<HoneycombWindow> ViewsWindow::GetParentWindow(HoneycombRefPtr<HoneycombWindow> window,
                                                  bool* is_menu,
                                                  bool* can_activate_menu) {
  HONEYCOMB_REQUIRE_UI_THREAD();
  HoneycombRefPtr<HoneycombWindow> parent_window = delegate_->GetParentWindow();
  if (parent_window) {
...
bool ViewsWindow::IsWindowModalDialog(HoneycombRefPtr<HoneycombWindow> window) {
  HONEYCOMB_REQUIRE_UI_THREAD();
  DCHECK(delegate_->GetParentWindow());
  return use_window_modal_dialog_;
}

HoneycombRect ViewsWindow::GetInitialBounds(HoneycombRefPtr<HoneycombWindow> window) {
  HONEYCOMB_REQUIRE_UI_THREAD();
  const HoneycombRect bounds = delegate_->GetInitialBounds();
  if (frameless_ && bounds.IsEmpty()) {
    // Need to provide a size for frameless windows that will be centered.
    return HoneycombRect(0, 0, kDefaultWidth, kDefaultHeight);
  }
  return bounds;
}

honey_show_state_t ViewsWindow::GetInitialShowState(HoneycombRefPtr<HoneycombWindow> window) {
  HONEYCOMB_REQUIRE_UI_THREAD();
  return delegate_->GetInitialShowState();
}

bool ViewsWindow::IsFrameless(HoneycombRefPtr<HoneycombWindow> window) {
  HONEYCOMB_REQUIRE_UI_THREAD();
  return frameless_;
}

bool ViewsWindow::WithStandardWindowButtons(HoneycombRefPtr<HoneycombWindow> window) {
  HONEYCOMB_REQUIRE_UI_THREAD();
  return with_standard_buttons_;
}

bool ViewsWindow::GetTitlebarHeight(HoneycombRefPtr<HoneycombWindow> window,
...
bool ViewsWindow::CanResize(HoneycombRefPtr<HoneycombWindow> window) {
  HONEYCOMB_REQUIRE_UI_THREAD();
  // Only allow resize of normal windows.
  return type_ == WindowType::NORMAL;
}

bool ViewsWindow::CanMaximize(HoneycombRefPtr<HoneycombWindow> window) {
  HONEYCOMB_REQUIRE_UI_THREAD();
  // Only allow maximize of normal windows.
  return type_ == WindowType::NORMAL;
}

bool ViewsWindow::CanMinimize(HoneycombRefPtr<HoneycombWindow> window) {
  HONEYCOMB_REQUIRE_UI_THREAD();
  // Only allow minimize of normal windows.
  return type_ == WindowType::NORMAL;
}

bool ViewsWindow::OnAccelerator(HoneycombRefPtr<HoneycombWindow> window, int command_id) {
  HONEYCOMB_REQUIRE_UI_THREAD();
...
bool ViewsWindow::OnKeyEvent(HoneycombRefPtr<HoneycombWindow> window,
                             const HoneycombKeyEvent& event) {
  HONEYCOMB_REQUIRE_UI_THREAD();
...
HoneycombSize ViewsWindow::GetPreferredSize(HoneycombRefPtr<HoneycombView> view) {
  HONEYCOMB_REQUIRE_UI_THREAD();
  ...
  if (view->GetID() == ID_WINDOW && type_ == WindowType::DIALOG) {
    ...
    return HoneycombSize(kDefaultWidth, kDefaultHeight);
  }

  return HoneycombSize();
}

HoneycombSize ViewsWindow::GetMinimumSize(HoneycombRefPtr<HoneycombView> view) {
  HONEYCOMB_REQUIRE_UI_THREAD();
...
void ViewsWindow::OnFocus(HoneycombRefPtr<HoneycombView> view) {
  HONEYCOMB_REQUIRE_UI_THREAD();
...
void ViewsWindow::OnBlur(HoneycombRefPtr<HoneycombView> view) {
  HONEYCOMB_REQUIRE_UI_THREAD();
...
void ViewsWindow::OnWindowChanged(HoneycombRefPtr<HoneycombView> view, bool added) {
  const int view_id = view->GetID();
  ...
  if (added) {
    ...
    if (with_overlay_controls_) {
      ...
      overlay_controls_->Initialize(window_, CreateMenuButton(),
                                    CreateLocationBar(),
                                    chrome_toolbar_type_ != HONEYCOMB_CTT_NONE);
    }
  } else {
...
void ViewsWindow::OnLayoutChanged(HoneycombRefPtr<HoneycombView> view,
                                  const HoneycombRect& new_bounds) {
  const int view_id = view->GetID();
  if (view_id != ID_BROWSER_VIEW) {
    return;
...
void ViewsWindow::MenuBarExecuteCommand(HoneycombRefPtr<HoneycombMenuModel> menu_model,
                                        int command_id,
                                        honey_event_flags_t event_flags) {
...
void ViewsWindow::SetBrowserView(HoneycombRefPtr<HoneycombBrowserView> browser_view) {
  DCHECK(!browser_view_);
  DCHECK(browser_view);
...
void ViewsWindow::CreateMenuModel() {
  ...
  button_menu_model_ = HoneycombMenuModel::CreateMenuModel(this);
  HoneycombRefPtr<HoneycombMenuModel> test_menu =
...
HoneycombRefPtr<HoneycombLabelButton> ViewsWindow::CreateBrowseButton(
    const std::string& label,
    int id) {
  HoneycombRefPtr<HoneycombLabelButton> button =
      HoneycombLabelButton::CreateLabelButton(this, label);
  button->SetID(id);
...
HoneycombRefPtr<HoneycombMenuButton> ViewsWindow::CreateMenuButton() {
  // Create the menu button.
  DCHECK(!menu_button_);
  menu_button_ = HoneycombMenuButton::CreateMenuButton(this, HoneycombString());
  ...
  menu_button_->SetMinimumSize(HoneycombSize(0, 0));
  return menu_button_;
}

HoneycombRefPtr<HoneycombView> ViewsWindow::CreateLocationBar() {
  DCHECK(!location_bar_);
  if (chrome_toolbar_type_ == HONEYCOMB_CTT_LOCATION) {
  ...
  if (!location_bar_) {
    ...
    HoneycombRefPtr<HoneycombTextfield> url_textfield = HoneycombTextfield::CreateTextfield(this);
...
void ViewsWindow::AddBrowserView() {
  ...
  HoneycombBoxLayoutSettings window_layout_settings;
  window_layout_settings.horizontal = false;
  window_layout_settings.between_child_spacing = 2;
  HoneycombRefPtr<HoneycombBoxLayout> window_layout =
  ...
  // is added to the HoneycombWindow. This is necessary because Chrome toolbars are
...
void ViewsWindow::AddControls() {
  ...
  // the HoneycombWindow. This is a requirement to use Chrome toolbars.

  HoneycombRefPtr<HoneycombPanel> menu_panel;
  if (menu_bar_) {
  ...
  if (!toolbar_) {
    ...
    HoneycombRefPtr<HoneycombBoxLayout> panel_layout =
  ...
  minimum_window_size_ = HoneycombSize(min_width, min_height);
}
...
void ViewsWindow::UpdateExtensionControls() {
  ...
  if (!extensions_panel_) {
    extensions_panel_ = HoneycombPanel::CreatePanel(nullptr);
  ...
  for (int id = ID_EXTENSION_BUTTON_FIRST;
    ...
    HoneycombRefPtr<HoneycombMenuButton> button =
        HoneycombMenuButton::CreateMenuButton(this, HoneycombString());
    button->SetID(id);
    button->SetImage(HONEYCOMB_BUTTON_STATE_NORMAL, (*it).image_);
    views_style::ApplyTo(button.get());
    button->SetInkDropEnabled(true);
    // Override the default minimum size.
    button->SetMinimumSize(HoneycombSize(0, 0));

    extensions_panel_->AddChildView(button);
  }

  HoneycombRefPtr<HoneycombView> parent_view = extensions_panel_->GetParentView();
  if (parent_view) {
...
void ViewsWindow::OnExtensionIconsLoaded(const ExtensionSet& extensions,
  ...
  if (!HoneycombCurrentlyOn(TID_UI)) {
    ...
    HoneycombPostTask(TID_UI, base::BindOnce(&ViewsWindow::OnExtensionIconsLoaded,
  ...
  for (; it1 != extensions.end() && it2 != images.end(); ++it1, ++it2) {
    HoneycombRefPtr<HoneycombImage> icon = *it2;
    if (!icon) {
