...
#include "tests/honeyclient/browser/root_window_mac.h"
...
#include "tests/honeyclient/browser/browser_window_std_mac.h"
...
@property(nonatomic, readwrite) std::optional<HoneycombRect> last_visible_bounds;
...
// fullscreen.
std::optional<HoneycombRect> GetWindowBoundsInScreen(NSWindow* window) {
  ...
  HoneycombRect dip_bounds{static_cast<int>(bounds.origin.x),
...
NSRect ClampNSBoundsToWorkArea(const NSRect& frame_bounds,
                               const HoneycombRect& display_bounds,
                               const HoneycombRect& work_area) {
...
void GetNSBoundsInDisplay(const HoneycombRect& dip_bounds,
  ...
  auto display =
      HoneycombDisplay::GetDisplayMatchingBounds(dip_bounds,
...
class RootWindowMacImpl
 ...
 public:
  ...
  void OnAutoResize(const HoneycombSize& new_size);
  void OnSetLoadingState(bool isLoading, bool canGoBack, bool canGoForward);
  void OnSetDraggableRegions(const std::vector<HoneycombDraggableRegion>& regions);
...
void RootWindowMacImpl::SetBounds(int x, int y, size_t width, size_t height) {
  ...
  const HoneycombRect dip_bounds(x, y, static_cast<int>(width),
...
HoneycombRefPtr<HoneycombBrowser> RootWindowMacImpl::GetBrowser() const {
  REQUIRE_MAIN_THREAD();
...
ClientWindowHandle RootWindowMacImpl::GetWindowHandle() const {
  REQUIRE_MAIN_THREAD();
  return CAST_NSVIEW_TO_HONEYCOMB_WINDOW_HANDLE([window_ contentView]);
}
...
void RootWindowMacImpl::CreateRootWindow(const HoneycombBrowserSettings& settings,
                                         bool initially_hidden) {
  REQUIRE_MAIN_THREAD();
  DCHECK(!window_);
  ...
  HoneycombRect dip_bounds = initial_bounds_;
  ...
  [window_
      setBackgroundColor:[NSColor
                             colorWithCalibratedRed:float(HoneycombColorGetR(
                                                        background_color)) /
                                                    255.0f
                                              green:float(HoneycombColorGetG(
                                                        background_color)) /
                                                    255.0f
                                               blue:float(HoneycombColorGetB(
                                                        background_color)) /
                                                    255.0f
                                              alpha:1.f]];
  ...
  if (!is_popup_) {
    ...
    browser_window_->CreateBrowser(
        CAST_NSVIEW_TO_HONEYCOMB_WINDOW_HANDLE(contentView),
        HoneycombRect(0, 0, contentBounds.size.width, contentBounds.size.height),
        settings, nullptr,
        root_window_.delegate_->GetRequestContext(&root_window_));
  } else {
    // With popups we already have a browser window. Parent the browser window
    // to the root window and show it in the correct location.
    browser_window_->ShowPopup(CAST_NSVIEW_TO_HONEYCOMB_WINDOW_HANDLE(contentView), 0,
  ...
  if (!initially_hidden) {
    ...
    if (initial_show_state_ == HONEYCOMB_SHOW_STATE_MAXIMIZED) {
      mode = RootWindow::ShowMaximized;
    } else if (initial_show_state_ == HONEYCOMB_SHOW_STATE_MINIMIZED) {
...
void RootWindowMacImpl::OnBrowserCreated(HoneycombRefPtr<HoneycombBrowser> browser) {
  REQUIRE_MAIN_THREAD();

  // For popup browsers create the root window once the browser has been
  // created.
  if (is_popup_) {
    CreateRootWindow(HoneycombBrowserSettings(), false);
  }
...
void RootWindowMacImpl::OnSetDraggableRegions(
  ...
  REQUIRE_MAIN_THREAD();
  // TODO(honey): Implement support for draggable regions on this platform.
}
...
void RootWindowMacImpl::OnSetFullscreen(bool fullscreen) {
  ...
  HoneycombRefPtr<HoneycombBrowser> browser = GetBrowser();
  if (browser) {
...
void RootWindowMacImpl::OnAutoResize(const HoneycombSize& new_size) {
  REQUIRE_MAIN_THREAD();
...
HoneycombRefPtr<HoneycombBrowser> RootWindowMac::GetBrowser() const {
...
void RootWindowMac::OnBrowserCreated(HoneycombRefPtr<HoneycombBrowser> browser) {
...
void RootWindowMac::OnAutoResize(const HoneycombSize& new_size) {
...
- (IBAction)goBack:(id)sender {
  HoneycombRefPtr<HoneycombBrowser> browser = root_window_->GetBrowser();
  if (browser.get()) {
    browser->GoBack();
  }
}
...
- (BOOL)windowShouldClose:(NSWindow*)window {
  if (!force_close_) {
    ...
    if (browser_window && !browser_window->IsClosing()) {
      HoneycombRefPtr<HoneycombBrowser> browser = browser_window->GetBrowser();
      if (browser.get()) {
  ...
  honey_show_state_t show_state = HONEYCOMB_SHOW_STATE_NORMAL;
  if ([window_ isMiniaturized]) {
