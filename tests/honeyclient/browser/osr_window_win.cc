...
#include "tests/honeyclient/browser/osr_ime_handler_win.h"
#include "tests/honeyclient/browser/osr_render_handler_win_d3d11.h"
#include "tests/honeyclient/browser/osr_render_handler_win_gl.h"
...
class CreateBrowserHelper {
 public:
  ...
  HWND hwnd_;
  RECT rect_;
  HoneycombRefPtr<HoneycombClient> handler_;
  std::string url_;
...
void OsrWindowWin::CreateBrowser(HWND parent_hwnd,
  ...
  if (!HoneycombCurrentlyOn(TID_UI)) {
    ...
    HoneycombPostTask(TID_UI, base::BindOnce(CreateBrowserWithHelper, helper));
    return;
  }
  ...
  HoneycombBrowserHost::CreateBrowser(window_info, handler, startup_url, settings,
...
void OsrWindowWin::ShowPopup(HWND parent_hwnd,
  ...
  if (!HoneycombCurrentlyOn(TID_UI)) {
    // Execute this method on the UI thread.
    HoneycombPostTask(TID_UI, base::BindOnce(&OsrWindowWin::ShowPopup, this,
...
void OsrWindowWin::Show() {
  if (!HoneycombCurrentlyOn(TID_UI)) {
    // Execute this method on the UI thread.
    HoneycombPostTask(TID_UI, base::BindOnce(&OsrWindowWin::Show, this));
    return;
  }
...
void OsrWindowWin::Hide() {
  if (!HoneycombCurrentlyOn(TID_UI)) {
    // Execute this method on the UI thread.
    HoneycombPostTask(TID_UI, base::BindOnce(&OsrWindowWin::Hide, this));
    return;
  }
...
void OsrWindowWin::SetBounds(int x, int y, size_t width, size_t height) {
  if (!HoneycombCurrentlyOn(TID_UI)) {
    // Execute this method on the UI thread.
    HoneycombPostTask(TID_UI, base::BindOnce(&OsrWindowWin::SetBounds, this, x, y,
                                       width, height));
    return;
  }
...
void OsrWindowWin::SetFocus() {
  if (!HoneycombCurrentlyOn(TID_UI)) {
    // Execute this method on the UI thread.
    HoneycombPostTask(TID_UI, base::BindOnce(&OsrWindowWin::SetFocus, this));
    return;
  }
...
void OsrWindowWin::SetDeviceScaleFactor(float device_scale_factor) {
  if (!HoneycombCurrentlyOn(TID_UI)) {
    // Execute this method on the UI thread.
    HoneycombPostTask(TID_UI, base::BindOnce(&OsrWindowWin::SetDeviceScaleFactor,
...
void OsrWindowWin::Create(HWND parent_hwnd, const RECT& rect) {
  ...
  const honey_color_t background_color = MainContext::Get()->GetBackgroundColor();
  const HBRUSH background_brush = CreateSolidBrush(
      RGB(HoneycombColorGetR(background_color), HoneycombColorGetG(background_color),
          HoneycombColorGetB(background_color)));
...
void OsrWindowWin::OnIMEComposition(UINT message,
  ...
  if (browser_ && ime_handler_) {
    ...
    if (ime_handler_->GetResult(lParam, cTextStr)) {
      ...
      browser_->GetHost()->ImeCommitText(cTextStr, HoneycombRange::InvalidRange(), 0);
      ime_handler_->ResetComposition();
    ...
    if (ime_handler_->GetComposition(lParam, cTextStr, underlines,
      ...
      browser_->GetHost()->ImeSetComposition(
          cTextStr, underlines, HoneycombRange::InvalidRange(),
          HoneycombRange(composition_start,
...
void OsrWindowWin::OnMouseEvent(UINT message, WPARAM wParam, LPARAM lParam) {
  ...
  HoneycombRefPtr<HoneycombBrowserHost> browser_host;
  if (browser_) {
  ...
  switch (message) {
    ...
    case WM_MBUTTONDOWN: {
      ...
      if (wParam & MK_SHIFT) {
        ...
        HoneycombBrowserHost::MouseButtonType btnType =
        ...
        if (browser_host) {
          ...
          mouse_event.modifiers = GetHoneycombMouseModifiers(wParam);
...
void OsrWindowWin::OnKeyEvent(UINT message, WPARAM wParam, LPARAM lParam) {
  ...
  event.modifiers = GetHoneycombKeyboardModifiers(wParam, lParam);
...
void OsrWindowWin::OnAfterCreated(HoneycombRefPtr<HoneycombBrowser> browser) {
  HONEYCOMB_REQUIRE_UI_THREAD();
  DCHECK(!browser_);
  browser_ = browser;
...
void OsrWindowWin::OnBeforeClose(HoneycombRefPtr<HoneycombBrowser> browser) {
  HONEYCOMB_REQUIRE_UI_THREAD();
...
bool OsrWindowWin::GetRootScreenRect(HoneycombRefPtr<HoneycombBrowser> browser,
                                     HoneycombRect& rect) {
  HONEYCOMB_REQUIRE_UI_THREAD();
  return false;
}

void OsrWindowWin::GetViewRect(HoneycombRefPtr<HoneycombBrowser> browser, HoneycombRect& rect) {
  HONEYCOMB_REQUIRE_UI_THREAD();
...
bool OsrWindowWin::GetScreenPoint(HoneycombRefPtr<HoneycombBrowser> browser,
                                  int viewX,
                                  int viewY,
                                  int& screenX,
                                  int& screenY) {
...
bool OsrWindowWin::GetScreenInfo(HoneycombRefPtr<HoneycombBrowser> browser,
...
void OsrWindowWin::OnPopupShow(HoneycombRefPtr<HoneycombBrowser> browser, bool show) {
  render_handler_->OnPopupShow(browser, show);
}

void OsrWindowWin::OnPopupSize(HoneycombRefPtr<HoneycombBrowser> browser,
                               const HoneycombRect& rect) {
  render_handler_->OnPopupSize(browser,
                               LogicalToDevice(rect, device_scale_factor_));
}

void OsrWindowWin::OnPaint(HoneycombRefPtr<HoneycombBrowser> browser,
...
void OsrWindowWin::OnCursorChange(HoneycombRefPtr<HoneycombBrowser> browser,
...
bool OsrWindowWin::StartDragging(
  ...
  HoneycombBrowserHost::DragOperationsMask result =
...
void OsrWindowWin::UpdateDragCursor(HoneycombRefPtr<HoneycombBrowser> browser,
...
void OsrWindowWin::OnImeCompositionRangeChanged(
  ...
  if (ime_handler_) {
    ...
    HoneycombRenderHandler::RectList device_bounds;
    HoneycombRenderHandler::RectList::const_iterator it = character_bounds.begin();
...
void OsrWindowWin::UpdateAccessibilityTree(HoneycombRefPtr<HoneycombValue> value) {
  HONEYCOMB_REQUIRE_UI_THREAD();
...
void OsrWindowWin::UpdateAccessibilityLocation(HoneycombRefPtr<HoneycombValue> value) {
  HONEYCOMB_REQUIRE_UI_THREAD();
...
HoneycombBrowserHost::DragOperationsMask OsrWindowWin::OnDragEnter(
    HoneycombRefPtr<HoneycombDragData> drag_data,
    HoneycombMouseEvent ev,
    HoneycombBrowserHost::DragOperationsMask effect) {
  if (browser_) {
...
HoneycombBrowserHost::DragOperationsMask OsrWindowWin::OnDragOver(
    HoneycombMouseEvent ev,
    HoneycombBrowserHost::DragOperationsMask effect) {
  if (browser_) {
    DeviceToLogical(ev, device_scale_factor_);
...
HoneycombBrowserHost::DragOperationsMask OsrWindowWin::OnDrop(
    HoneycombMouseEvent ev,
    HoneycombBrowserHost::DragOperationsMask effect) {
  if (browser_) {
    DeviceToLogical(ev, device_scale_factor_);
