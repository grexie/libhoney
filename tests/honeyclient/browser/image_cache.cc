...
void ImageCache::LoadImages(const ImageInfoSet& image_info,
  ...
  if (!HoneycombCurrentlyOn(TID_UI)) {
    HoneycombPostTask(TID_UI, base::BindOnce(&ImageCache::LoadImages, this,
...
HoneycombRefPtr<HoneycombImage> ImageCache::GetCachedImage(const std::string& image_id) {
  HONEYCOMB_REQUIRE_UI_THREAD();
...
void ImageCache::LoadMissing(const ImageInfoSet& image_info,
  ...
  HoneycombPostTask(TID_UI, base::BindOnce(&ImageCache::UpdateCache, this, image_info,
...
void ImageCache::UpdateCache(const ImageInfoSet& image_info,
  ...
  for (; it1 != image_info.end() && it2 != contents.end(); ++it1, ++it2) {
    ...
    if (content.image_ || info.id_ == kEmptyId) {
      ...
      HoneycombRefPtr<HoneycombImage> image = CreateImage(info.id_, content);
...
// static
HoneycombRefPtr<HoneycombImage> ImageCache::CreateImage(const std::string& image_id,
