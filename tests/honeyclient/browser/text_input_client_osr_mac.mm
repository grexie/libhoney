...
honey_color_t HoneycombColorFromNSColor(NSColor* color) {
  CGFloat r, g, b, a;
...
void ExtractUnderlines(NSAttributedString* string,
  ...
  while (i < length) {
    ...
    if (style) {
      honey_color_t color = ColorBLACK;
      if (NSColor* colorAttr =
              [attrs objectForKey:NSUnderlineColorAttributeName]) {
        color = HoneycombColorFromNSColor(
            [colorAttr colorUsingColorSpace:NSColorSpace.deviceRGBColorSpace]);
      }
      honey_composition_underline_t line = {
...
@implementation HoneycombTextInputClientOSRMac
...
- (void)insertText:(id)aString replacementRange:(NSRange)replacementRange {
  ...
  if (handlingKeyDown_) {
    ...
    honey_range_t range = {static_cast<uint32_t>(replacementRange.location),
...
- (void)setMarkedText:(id)aString
  ...
  if (isAttributedString) {
    ...
    honey_composition_underline_t line = {{0, length}, ColorBLACK, 0, false};
  ...
  if (handlingKeyDown_) {
    ...
    HoneycombRange replacement_range(
        static_cast<uint32_t>(replacementRange.location),
        static_cast<uint32_t>(NSMaxRange(replacementRange)));
    HoneycombRange selection_range(static_cast<uint32_t>(newSelRange.location),
...
- (NSRect)firstViewRectForCharacterRange:(NSRange)theRange
  ...
  if (location < composition_bounds_.size()) {
    const HoneycombRect& rc = composition_bounds_[location];
...
- (void)HandleKeyEventBeforeTextInputClient:(NSEvent*)keyEvent {
  ...
  markedText_.clear();
  underlines_.clear();
  setMarkedTextReplacementRange_ = HoneycombRange::InvalidRange();
...
- (void)HandleKeyEventAfterTextInputClient:(HoneycombKeyEvent)keyEvent {
  ...
  if (hasMarkedText_ && markedText_.length()) {
    ...
    browser_->GetHost()->ImeSetComposition(
        ...
        HoneycombRange(static_cast<uint32_t>(selectedRange_.location),
