...
void GetColorModelForMode(HoneycombPrintSettings::ColorModel color_mode,
...
void InitPrintSettings(GtkPrintSettings* settings,
                       ...
                       HoneycombRefPtr<HoneycombPrintSettings> print_settings) {
  DCHECK(settings);
  DCHECK(page_setup);
  ...
  HoneycombSize physical_size_device_units;
  HoneycombRect printable_area_device_units;
...
// Views framework.
GtkWindow* GetWindow(HoneycombRefPtr<HoneycombBrowser> browser) {
...
void GetWindowAndContinue(HoneycombRefPtr<HoneycombBrowser> browser,
  ...
  if (window) {
    HoneycombPostTask(TID_UI, base::BindOnce(std::move(callback), window));
  }
}
...
struct ClientPrintHandlerGtk::PrintHandler {
  PrintHandler(HoneycombRefPtr<HoneycombBrowser> browser)
      : browser_(browser),
        dialog_(nullptr),
  ...
  void OnPrintSettings(HoneycombRefPtr<HoneycombPrintSettings> settings,
                       bool get_defaults) {
    ScopedGdkThreadsEnter scoped_gdk_threads;
  ...
  bool OnPrintJob(const HoneycombString& document_name,
 ...
 private:
  void OnDialogResponse(GtkDialog* dialog, gint response_id) {
    ...
    switch (response_id) {
      case GTK_RESPONSE_OK: {
        ...
        // Handle page ranges.
        HoneycombPrintSettings::PageRangeList ranges_vector;
        gint num_ranges;
            ...
            if (gtk_range) {
              for (int i = 0; i < num_ranges; ++i) {
                ranges_vector.push_back(
                    HoneycombRange(gtk_range[i].start, gtk_range[i].end));
              }
              g_free(gtk_range);
            }
            break;
        ...
        HoneycombRefPtr<HoneycombPrintSettings> settings = HoneycombPrintSettings::Create();
  ...
  void OnJobCompleted(GtkPrintJob* print_job, const GError* error) {
    ...
    // chance to unwind.
    HoneycombPostTask(TID_UI, base::BindOnce(&HoneycombPrintJobCallback::Continue,
                                       job_callback_.get()));
  ...
  HoneycombRefPtr<HoneycombPrintDialogCallback> dialog_callback_;
  HoneycombRefPtr<HoneycombPrintJobCallback> job_callback_;
};
...
void ClientPrintHandlerGtk::OnPrintStart(HoneycombRefPtr<HoneycombBrowser> browser) {
...
void ClientPrintHandlerGtk::OnPrintReset(HoneycombRefPtr<HoneycombBrowser> browser) {
  HONEYCOMB_REQUIRE_UI_THREAD();
...
HoneycombSize ClientPrintHandlerGtk::GetPdfPaperSize(HoneycombRefPtr<HoneycombBrowser> browser,
  ...
  return HoneycombSize(width * device_units_per_inch, height * device_units_per_inch);
}
