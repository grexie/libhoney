...
// data back and forth between MSAA clients and HoneycombClient renderers.
...
struct HoneycombIAccessible : public IAccessible {
 public:
  // Implement IUnknown
  ...
  HoneycombIAccessible(OsrAXNode* node) : ref_count_(0), node_(node) {}
...
STDMETHODIMP HoneycombIAccessible::QueryInterface(REFIID riid, void** ppvObject) {
...
STDMETHODIMP_(ULONG) HoneycombIAccessible::AddRef() {
  return InterlockedIncrement((LONG volatile*)&ref_count_);
}

STDMETHODIMP_(ULONG) HoneycombIAccessible::Release() {
...
STDMETHODIMP HoneycombIAccessible::get_accParent(IDispatch** ppdispParent) {
  HRESULT retCode = DATACHECK(node_);
  if (SUCCEEDED(retCode)) {
    if (ppdispParent) {
      HoneycombNativeAccessible* parent = node_->GetParentAccessibleObject();
      if (!parent) {
...
STDMETHODIMP HoneycombIAccessible::get_accChildCount(long* pcountChildren) {
  HRESULT retCode = DATACHECK(node_);
...
STDMETHODIMP HoneycombIAccessible::get_accChild(VARIANT varChild,
...
STDMETHODIMP HoneycombIAccessible::get_accName(VARIANT varChild, BSTR* pszName) {
  HRESULT retCode = DATACHECK(node_);
  if (SUCCEEDED(retCode)) {
...
STDMETHODIMP HoneycombIAccessible::get_accValue(VARIANT varChild, BSTR* pszValue) {
  HRESULT retCode = DATACHECK(node_);
  if (SUCCEEDED(retCode)) {
...
STDMETHODIMP HoneycombIAccessible::get_accDescription(VARIANT varChild,
                                                BSTR* pszDescription) {
  HRESULT retCode = DATACHECK(node_);
  if (SUCCEEDED(retCode)) {
...
STDMETHODIMP HoneycombIAccessible::get_accRole(VARIANT varChild, VARIANT* pvarRole) {
  HRESULT retCode = DATACHECK(node_);
  if (SUCCEEDED(retCode)) {
...
STDMETHODIMP HoneycombIAccessible::get_accState(VARIANT varChild,
                                          VARIANT* pvarState) {
  HRESULT retCode = DATACHECK(node_);
  if (SUCCEEDED(retCode)) {
    if (pvarState) {
...
STDMETHODIMP HoneycombIAccessible::get_accKeyboardShortcut(
    VARIANT varChild,
...
STDMETHODIMP HoneycombIAccessible::get_accFocus(VARIANT* pFocusChild) {
  HRESULT retCode = DATACHECK(node_);
  if (SUCCEEDED(retCode)) {
    OsrAXNode* focusedNode = node_->GetAccessibilityHelper()->GetFocusedNode();
    HoneycombNativeAccessible* nativeObj = nullptr;
    if (focusedNode) {
...
STDMETHODIMP HoneycombIAccessible::get_accSelection(VARIANT* pvarChildren) {
  HRESULT retCode = DATACHECK(node_);
  if (SUCCEEDED(retCode)) {
    if (pvarChildren) {
...
STDMETHODIMP HoneycombIAccessible::get_accDefaultAction(VARIANT varChild,
...
STDMETHODIMP HoneycombIAccessible::accSelect(long flagsSelect, VARIANT varChild) {
  HRESULT retCode = DATACHECK(node_);
  if (SUCCEEDED(retCode)) {
...
STDMETHODIMP HoneycombIAccessible::accLocation(long* pxLeft,
                                         long* pyTop,
                                         long* pcxWidth,
                                         long* pcyHeight,
                                         VARIANT varChild) {
  ...
  if (SUCCEEDED(retCode)) {
    if (pxLeft && pyTop && pcxWidth && pcyHeight && VALID_CHILDID(varChild)) {
      HoneycombRect loc = node_->AxLocation();
...
// Deprecated
STDMETHODIMP HoneycombIAccessible::accNavigate(long navDir,
                                         VARIANT varStart,
                                         VARIANT* pvarEndUpAt) {
  return E_NOTIMPL;
}
...
STDMETHODIMP HoneycombIAccessible::accHitTest(long xLeft,
                                        long yTop,
                                        VARIANT* pvarChild) {
  HRESULT retCode = DATACHECK(node_);
  if (SUCCEEDED(retCode)) {
...
STDMETHODIMP HoneycombIAccessible::accDoDefaultAction(VARIANT varChild) {
  HRESULT retCode = DATACHECK(node_);
  if (SUCCEEDED(retCode) && VALID_CHILDID(varChild)) {
    // doing our default action for out button is to simply click the button.
    HoneycombRefPtr<HoneycombBrowser> browser = node_->GetBrowser();
    if (browser) {
      HoneycombMouseEvent mouse_event;
      const HoneycombRect& rect = node_->AxLocation();
...
STDMETHODIMP HoneycombIAccessible::put_accName(VARIANT varChild, BSTR szName) {
  return E_NOTIMPL;
}

// Set the value for an element in the accessibility tree
STDMETHODIMP HoneycombIAccessible::put_accValue(VARIANT varChild, BSTR szValue) {
  return E_NOTIMPL;
}

// Return E_NOTIMPL as no help file/ topic
STDMETHODIMP HoneycombIAccessible::get_accHelp(VARIANT varChild, BSTR* pszHelp) {
  return E_NOTIMPL;
}

STDMETHODIMP HoneycombIAccessible::get_accHelpTopic(BSTR* pszHelpFile,
                                              VARIANT varChild,
                                              long* pidTopic) {
  return E_NOTIMPL;
}
...
STDMETHODIMP HoneycombIAccessible::GetTypeInfoCount(unsigned int FAR* pctinfo) {
  return E_NOTIMPL;
}

STDMETHODIMP HoneycombIAccessible::GetTypeInfo(unsigned int iTInfo,
                                         LCID lcid,
                                         ITypeInfo FAR* FAR* ppTInfo) {
  return E_NOTIMPL;
}

STDMETHODIMP HoneycombIAccessible::GetIDsOfNames(REFIID riid,
                                           OLECHAR FAR* FAR* rgszNames,
                                           unsigned int cNames,
                                           LCID lcid,
                                           DISPID FAR* rgDispId) {
...
STDMETHODIMP HoneycombIAccessible::Invoke(DISPID dispIdMember,
                                    REFIID riid,
                                    LCID lcid,
                                    WORD wFlags,
                                    DISPPARAMS FAR* pDispParams,
...
void OsrAXNode::Destroy() {
  HoneycombIAccessible* ptr = static_cast<HoneycombIAccessible*>(platform_accessibility_);
  if (ptr) {
    ptr->MarkDestroyed();
  }
...
HoneycombNativeAccessible* OsrAXNode::GetNativeAccessibleObject(OsrAXNode* parent) {
  if (!platform_accessibility_) {
    platform_accessibility_ = new HoneycombIAccessible(this);
...
#else  // !defined(HONEYCOMB_USE_ATL)
