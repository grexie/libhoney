...
void OsrRenderer::Initialize() {
  ...
  if (IsTransparent()) {
    ...
    glClearColor(float(HoneycombColorGetR(settings_.background_color)) / 255.0f,
                 float(HoneycombColorGetG(settings_.background_color)) / 255.0f,
                 float(HoneycombColorGetB(settings_.background_color)) / 255.0f,
                 1.0f);
    VERIFY_NO_ERROR;
  }
...
void OsrRenderer::OnPopupShow(HoneycombRefPtr<HoneycombBrowser> browser, bool show) {
  if (!show) {
...
void OsrRenderer::OnPopupSize(HoneycombRefPtr<HoneycombBrowser> browser,
...
HoneycombRect OsrRenderer::GetPopupRectInWebView(const HoneycombRect& original_rect) {
  HoneycombRect rc(original_rect);
...
void OsrRenderer::OnPaint(HoneycombRefPtr<HoneycombBrowser> browser,
  ...
  if (type == PET_VIEW) {
    ...
    if (old_width != view_width_ || old_height != view_height_ ||
        (dirtyRects.size() == 1 &&
         dirtyRects[0] == HoneycombRect(0, 0, view_width_, view_height_))) {
      ...
      HoneycombRenderHandler::RectList::const_iterator i = dirtyRects.begin();
      for (; i != dirtyRects.end(); ++i) {
        const HoneycombRect& rect = *i;
