...
// static
void WindowTestRunner::ModifyBounds(const HoneycombRect& display, HoneycombRect& window) {
  window.x += display.x;
  window.y += display.y;
...
void WindowTestRunner::Fullscreen(HoneycombRefPtr<HoneycombBrowser> browser) {
  NOTIMPLEMENTED();
}

void WindowTestRunner::SetTitleBarHeight(HoneycombRefPtr<HoneycombBrowser> browser,
