...
#include "tests/honeyclient/browser/root_window_gtk.h"

#include <gdk/gdk.h>
#include <gdk/gdkx.h>

#include <X11/Xlib.h>
#undef Success    // Definition conflicts with honey_message_router.h
...
#include "tests/honeyclient/browser/browser_window_std_gtk.h"
...
namespace client
{
  ...
  namespace
  {
    ...
    void UseDefaultX11VisualForGtk(GtkWidget *widget)
    {
      ...
      // However, it breaks Honeycomb: https://github.com/cztomczak/honeycapi/issues/9
      // Let's use the default X11 visual instead of the GTK's blessed one.
      // Copied from: https://github.com/cztomczak/honeycapi.
  ...
  HoneycombRefPtr<HoneycombBrowser> RootWindowGtk::GetBrowser() const
  {
    REQUIRE_MAIN_THREAD();
  ...
  void RootWindowGtk::CreateRootWindow(const HoneycombBrowserSettings &settings,
                                       bool initially_hidden)
  {
    ...
    GdkRGBA rgba = {0};
    rgba.red = HoneycombColorGetR(background_color) * 65535 / 255;
    rgba.green = HoneycombColorGetG(background_color) * 65535 / 255;
    rgba.blue = HoneycombColorGetB(background_color) * 65535 / 255;
    rgba.alpha = 1;
  ...
  void RootWindowGtk::OnBrowserCreated(HoneycombRefPtr<HoneycombBrowser> browser)
  {
    REQUIRE_MAIN_THREAD();
  ...
  void RootWindowGtk::OnAutoResize(const HoneycombSize &new_size)
  {
    REQUIRE_MAIN_THREAD();
  ...
  void RootWindowGtk::NotifyContentBounds(int x, int y, int width, int height)
  {
    ...
    browser_bounds_ =
        HoneycombRect(browser_x, browser_y, browser_width, browser_height);
    if (browser_window_.get())
    {
  ...
  void RootWindowGtk::NotifyForceClose()
  {
    if (!HoneycombCurrentlyOn(TID_UI))
    {
      HoneycombPostTask(TID_UI, base::BindOnce(&RootWindowGtk::NotifyForceClose, this));
      return;
    }
