...
#include "libhoneycomb/common/alloy/alloy_main_delegate.h"
...
#include "libhoneycomb/common/resource_util.h"
...
AlloyMainDelegate::AlloyMainDelegate(HoneycombMainRunnerHandler* runner,
...
absl::optional<int> AlloyMainDelegate::BasicStartupComplete() {
  ...
  if (is_browser) {
    ...
    if (settings_->browser_subprocess_path.length > 0) {
      base::FilePath file_path =
          base::FilePath(HoneycombString(&settings_->browser_subprocess_path));
      if (!file_path.empty()) {
    ...
    if (settings_->framework_dir_path.length > 0) {
      base::FilePath file_path =
          base::FilePath(HoneycombString(&settings_->framework_dir_path));
      if (!file_path.empty()) {
    ...
    if (settings_->main_bundle_path.length > 0) {
      base::FilePath file_path =
          base::FilePath(HoneycombString(&settings_->main_bundle_path));
      if (!file_path.empty()) {
    ...
    if (settings_->log_file.length > 0) {
      log_file = base::FilePath(HoneycombString(&settings_->log_file));
    }
    ...
    if (settings_->pack_loading_disabled) {
      ...
      if (settings_->resources_dir_path.length > 0) {
        base::FilePath file_path =
            base::FilePath(HoneycombString(&settings_->resources_dir_path));
        if (!file_path.empty()) {
      ...
      if (settings_->locales_dir_path.length > 0) {
        base::FilePath file_path =
            base::FilePath(HoneycombString(&settings_->locales_dir_path));
        if (!file_path.empty()) {
  ...
  if (application_) {
    ...
    application_->OnBeforeCommandLineProcessing(HoneycombString(process_type),
                                                commandLinePtr.get());
...
HoneycombRefPtr<HoneycombRequestContext> AlloyMainDelegate::GetGlobalRequestContext() {
  if (!browser_client_) {
    return nullptr;
  }
...
HoneycombBrowserContext* AlloyMainDelegate::CreateNewBrowserContext(
...
void AlloyMainDelegate::InitializeResourceBundle() {
  ...
  auto module_handle =
      ::GetModuleHandle(HoneycombAppManager::Get()->GetResourceDllName());
  if (!module_handle) {
