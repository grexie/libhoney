...
// we use "~/.config/honey_user_data".
bool GetDefaultUserDataDirectory(base::FilePath* result) {
  std::unique_ptr<base::Environment> env(base::Environment::Create());
  base::FilePath config_dir(base::nix::GetXDGDirectory(
      env.get(), base::nix::kXdgConfigHomeEnvVar, base::nix::kDotConfigDir));
  *result = config_dir.Append(FILE_PATH_LITERAL("honey_user_data"));
  return true;
}
...
bool GetDefaultUserDataDirectory(base::FilePath* result) {
  ...
  *result = result->Append(FILE_PATH_LITERAL("Honeycomb"));
...
base::FilePath GetUserDataPath(HoneycombSettings* settings,
                               const base::CommandLine* command_line) {
  // |settings| will be non-nullptr in the main process only.
  if (settings) {
    HoneycombString root_cache_path;
    if (settings->root_cache_path.length > 0) {
      root_cache_path = HoneycombString(&settings->root_cache_path);
    }
