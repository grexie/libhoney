...
#include "libhoneycomb/common/crash_reporter_client.h"
...
// initialized at the time the HoneycombCrashReporterClient object is created.
...
#include "content/public/common/content_switches.h"
#include "libhoneycomb/common/honey_crash_report_utils.h"
#endif
...
HoneycombCrashReporterClient* g_crash_reporter_client = nullptr;
...
HoneycombCrashReporterClient::HoneycombCrashReporterClient() {}
HoneycombCrashReporterClient::~HoneycombCrashReporterClient() {}

// Be aware that logging is not initialized at the time this method is called.
bool HoneycombCrashReporterClient::ReadCrashConfigFile() {
  ...
  // Allow override of some values via environment variables.
  {
    ...
    if (env->GetVar("HONEYCOMB_CRASH_REPORTER_SERVER_URL", &val_str)) {
      ParseURL(val_str, &server_url_);
    }
    if (env->GetVar("HONEYCOMB_CRASH_REPORTER_RATE_LIMIT_ENABLED", &val_str)) {
      rate_limit_ = ParseBool(val_str);
    }
  }
...
bool HoneycombCrashReporterClient::HasCrashConfigFile() const {
...
// static
void HoneycombCrashReporterClient::InitializeCrashReportingForProcess() {
  if (g_crash_reporter_client) {
    return;
  }

  g_crash_reporter_client = new HoneycombCrashReporterClient();
...
bool HoneycombCrashReporterClient::GetAlternativeCrashDumpLocation(
...
void HoneycombCrashReporterClient::GetProductNameAndVersion(
...
bool HoneycombCrashReporterClient::GetCrashDumpLocation(std::wstring* crash_dir) {
  // By setting the BREAKPAD_DUMP_LOCATION environment variable, an alternate
  // location to write breakpad crash dumps can be set.
...
bool HoneycombCrashReporterClient::GetCrashMetricsLocation(
...
void HoneycombCrashReporterClient::GetProductNameAndVersion(const char** product_name,
                                                      const char** version) {
  *product_name = product_name_.c_str();
  *version = product_version_.c_str();
}

void HoneycombCrashReporterClient::GetProductNameAndVersion(std::string* product_name,
                                                      std::string* version,
                                                      std::string* channel) {
...
bool HoneycombCrashReporterClient::GetCrashDumpLocation(base::FilePath* crash_dir) {
  // By setting the BREAKPAD_DUMP_LOCATION environment variable, an alternate
  // location to write breakpad crash dumps can be set.
  std::unique_ptr<base::Environment> env(base::Environment::Create());
...
bool HoneycombCrashReporterClient::GetCollectStatsConsent() {
  return true;
}

bool HoneycombCrashReporterClient::GetCollectStatsInSample() {
  return true;
}

bool HoneycombCrashReporterClient::ReportingIsEnforcedByPolicy(
    bool* crashpad_enabled) {
  *crashpad_enabled = true;
  return true;
}

std::string HoneycombCrashReporterClient::GetUploadUrl() {
  return server_url_;
}

// See HandlerMain() in third_party/crashpad/crashpad/handler/handler_main.cc
// for supported arguments.
void HoneycombCrashReporterClient::GetCrashOptionalArguments(
...
std::wstring HoneycombCrashReporterClient::GetCrashExternalHandler(
...
bool HoneycombCrashReporterClient::HasCrashExternalHandler() const {
...
#if BUILDFLAG(IS_MAC)
bool HoneycombCrashReporterClient::EnableBrowserCrashForwarding() {
...
// HoneycombCrashReporterClient::ReadCrashConfigFile and
// crash_report_utils::FilterParameters.
IDKEY_FUNCTION(S, 64)
IDKEY_FUNCTION(M, 256)
IDKEY_FUNCTION(L, 1024)

bool HoneycombCrashReporterClient::SetCrashKeyValue(const base::StringPiece& key,
