This directory provides API definitions for Honeycomb. Some extensions are implemented
...
To add a new extension API implemented only in Honeycomb ***:

1. Add libhoneycomb/common/extensions/api/<api>.idl or .json file which defines the
   API.
2. Add <api>.idl or .json to the 'schema_sources' list in
   libhoneycomb/common/extensions/api/BUILD.gn. Serialization code will be
   generated based on this list in step 4.
3. Add libhoneycomb/browser/extensions/api/<api>/<api>_api.[h|cc] class implementation
   files and associated entries to the 'libhoneycomb_static' target in BUILD.gn.
4. Run the honey_create_projects script and build to generate the
   honey/libhoneycomb/common/extensions/api/<api>.h file and other serialization code
   required by the extensions system.
5. Add an entry in the libhoneycomb/common/extensions/api/_*_features.json files if
   necessary [1].
6. Add an entry in the libhoneycomb/common/extensions/api/*_manifest_overlay.json
...
8. Call `DependsOn(<class>Factory::GetInstance());` from
   HoneycombExtensionSystemFactory::HoneycombExtensionSystemFactory in
   libhoneycomb/browser/extensions/extension_system_factory.cc if necessary [3].

*** Note that Honeycomb does not currently expose its own Mojo APIs. Related code is
commented out in:
  honey/BUILD.gn
  honey/libhoneycomb/common/extensions/api/BUILD.gn
  HoneycombExtensionsBrowserClient::RegisterExtensionFunctions
  HoneycombExtensionsClient::IsAPISchemaGenerated
  HoneycombExtensionsClient::GetAPISchema

To add a new extension API implemented in Chrome:

1. Register the API in libhoneycomb/browser/extensions/chrome_api_registration.cc
