...
#include "libhoneycomb/common/chrome/chrome_main_delegate_honey.h"
...
ChromeMainRunnerDelegate::GetContentMainDelegate() {
  if (!main_delegate_) {
    main_delegate_ = std::make_unique<ChromeMainDelegateHoneycomb>(runner_, settings_,
                                                             application_);
  }
...
void ChromeMainRunnerDelegate::BeforeMainMessageLoopRun(
  ...
  // UI thread before terminating the message loop (e.g. from HoneycombQuitMessageLoop
...
bool ChromeMainRunnerDelegate::HandleMainMessageLoopQuit() {
  ...
  // HoneycombMainRunner::QuitMessageLoop. We instead wait for all Chrome browser
  // windows to exit.
  return true;
}
...
void ChromeMainRunnerDelegate::AfterUIThreadShutdown() {
  static_cast<ChromeContentBrowserClient*>(
      HoneycombAppManager::Get()->GetContentClient()->browser())
      ->CleanupOnUIThread();
...
void ChromeMainRunnerDelegate::BeforeExecuteProcess(const HoneycombMainArgs& args) {
