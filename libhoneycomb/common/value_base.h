...
#ifndef HONEYCOMB_LIBHONEYCOMB_COMMON_VALUE_BASE_H_
#define HONEYCOMB_LIBHONEYCOMB_COMMON_VALUE_BASE_H_
#pragma once
...
class HoneycombValueController
    : public base::RefCountedThreadSafe<HoneycombValueController> {
 public:
  ...
  class AutoLock {
   public:
    explicit AutoLock(HoneycombValueController* impl)
   ...
   private:
    scoped_refptr<HoneycombValueController> impl_;
    bool verified_;
  };

  HoneycombValueController();

  HoneycombValueController(const HoneycombValueController&) = delete;
  HoneycombValueController& operator=(const HoneycombValueController&) = delete;
  ...
  void TakeFrom(HoneycombValueController* other);
 ...
 protected:
  friend class base::RefCountedThreadSafe<HoneycombValueController>;

  virtual ~HoneycombValueController();
...
class HoneycombValueControllerThreadSafe : public HoneycombValueController {
 public:
  explicit HoneycombValueControllerThreadSafe() : locked_thread_id_(0) {}

  HoneycombValueControllerThreadSafe(const HoneycombValueControllerThreadSafe&) = delete;
  HoneycombValueControllerThreadSafe& operator=(const HoneycombValueControllerThreadSafe&) =
      delete;

  // HoneycombValueController methods.
...
class HoneycombValueControllerNonThreadSafe : public HoneycombValueController {
 public:
  explicit HoneycombValueControllerNonThreadSafe()
      : thread_id_(base::PlatformThread::CurrentId()) {}

  HoneycombValueControllerNonThreadSafe(const HoneycombValueControllerNonThreadSafe&) =
      delete;
  HoneycombValueControllerNonThreadSafe& operator=(
      const HoneycombValueControllerNonThreadSafe&) = delete;
...
#define HONEYCOMB_VALUE_VERIFY_RETURN_VOID_EX(object, modify) \
  if (!VerifyAttached())                                \
    return;                                             \
...
#define HONEYCOMB_VALUE_VERIFY_RETURN_VOID(modify) \
  HONEYCOMB_VALUE_VERIFY_RETURN_VOID_EX(this, modify)

#define HONEYCOMB_VALUE_VERIFY_RETURN_EX(object, modify, error_val) \
  if (!VerifyAttached())                                      \
    return error_val;                                         \
  AutoLock auto_lock(object, modify);                         \
  if (!auto_lock.verified())                                  \
...
#define HONEYCOMB_VALUE_VERIFY_RETURN(modify, error_val) \
  HONEYCOMB_VALUE_VERIFY_RETURN_EX(this, modify, error_val)

// Template class for implementing Honeycomb wrappers of other types.
template <class HoneycombType, class ValueType>
class HoneycombValueBase : public HoneycombType, public HoneycombValueController::Object {
 public:
  ...
  // will be added.
  HoneycombValueBase(ValueType* value,
               void* parent_value,
    ...
    if (!reference() && !controller_.get()) {
      ...
      controller_ = new HoneycombValueControllerThreadSafe();
      SetOwnsController();
    }
  ...
  HoneycombValueBase(const HoneycombValueBase&) = delete;
  HoneycombValueBase& operator=(const HoneycombValueBase&) = delete;

  ~HoneycombValueBase() override {
  ...
  inline HoneycombValueController* controller() const { return controller_.get(); }
  ...
  [[nodiscard]] ValueType* Detach(HoneycombValueController* new_controller) {
 ...
 protected:
  // HoneycombValueController::Object methods.
  ...
  inline void SetOwnsController() {
    HoneycombValueController::AutoLock lock_scope(controller_.get());
  ...
  class AutoLock {
   public:
    explicit AutoLock(HoneycombValueBase* impl, bool modify)
   ...
   private:
    HoneycombValueController::AutoLock auto_lock_;
    bool verified_;
  };
 ...
 private:
  ...
  bool read_only_;
  scoped_refptr<HoneycombValueController> controller_;

  IMPLEMENT_REFCOUNTING(HoneycombValueBase);
};

#endif  // HONEYCOMB_LIBHONEYCOMB_COMMON_VALUE_BASE_H_
