...
#include "libhoneycomb/common/honey_crash_report_upload_thread.h"
...
HoneycombCrashReportUploadThread::HoneycombCrashReportUploadThread(
...
void HoneycombCrashReportUploadThread::ProcessPendingReports() {
  if (BackoffPending()) {
    // Try again later.
    return;
  }
...
void HoneycombCrashReportUploadThread::ProcessPendingReport(
...
HoneycombCrashReportUploadThread::FilterParameters(const ParameterMap& parameters) {
  return crash_report_utils::FilterParameters(parameters);
}

bool HoneycombCrashReportUploadThread::UploadsEnabled() const {
...
bool HoneycombCrashReportUploadThread::MaxUploadsEnabled() const {
  return options_.rate_limit && max_uploads_ > 0;
}

bool HoneycombCrashReportUploadThread::MaxUploadsExceeded() const {
  return MaxUploadsEnabled() && recent_upload_ct_ >= max_uploads_;
}

bool HoneycombCrashReportUploadThread::BackoffPending() const {
...
void HoneycombCrashReportUploadThread::IncreaseBackoff() {
  if (!options_.rate_limit) {
    return;
  }
...
void HoneycombCrashReportUploadThread::ResetBackoff() {
  if (!options_.rate_limit) {
    return;
  }
