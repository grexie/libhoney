...
#ifndef HONEYCOMB_LIBHONEYCOMB_BROWSER_BROWSER_CONTENTS_DELEGATE_H_
#define HONEYCOMB_LIBHONEYCOMB_BROWSER_BROWSER_CONTENTS_DELEGATE_H_
#pragma once
...
enum class HoneycombBrowserContentsState : uint8_t {
  kNone = 0,
  kNavigation = (1 << 0),
  kDocument = (1 << 1),
  kFullscreen = (1 << 2),
...
constexpr inline HoneycombBrowserContentsState operator&(
    HoneycombBrowserContentsState lhs,
    HoneycombBrowserContentsState rhs) {
  return static_cast<HoneycombBrowserContentsState>(static_cast<int>(lhs) &
                                              static_cast<int>(rhs));
}
...
class HoneycombBrowserContentsDelegate : public content::WebContentsDelegate,
                                   public content::WebContentsObserver,
                                   public content::NotificationObserver {
 public:
  using State = HoneycombBrowserContentsState;

  // Interface to implement for observers that wish to be informed of changes
  // to the delegate. All methods will be called on the UI thread.
  class Observer : public base::CheckedObserver {
   public:
    // Called after state has changed and before the associated HoneycombClient
  ...
  explicit HoneycombBrowserContentsDelegate(
      scoped_refptr<HoneycombBrowserInfo> browser_info);

  HoneycombBrowserContentsDelegate(const HoneycombBrowserContentsDelegate&) = delete;
  HoneycombBrowserContentsDelegate& operator=(const HoneycombBrowserContentsDelegate&) =
      delete;
  ...
  HoneycombRefPtr<HoneycombFrameHostImpl> focused_frame() const { return focused_frame_; }

  // Helpers for executing client callbacks.
  // TODO(honey): Make this private if/when possible.
  bool OnSetFocus(honey_focus_source_t source);

 private:
  HoneycombRefPtr<HoneycombClient> client() const;
  ...
  void OnLoadStart(HoneycombRefPtr<HoneycombFrame> frame,
                   ui::PageTransition transition_type);
  void OnLoadEnd(HoneycombRefPtr<HoneycombFrame> frame,
                 const GURL& url,
                 int http_status_code);
  void OnLoadError(HoneycombRefPtr<HoneycombFrame> frame, const GURL& url, int error_code);
...
#endif  // HONEYCOMB_LIBHONEYCOMB_BROWSER_BROWSER_CONTENTS_DELEGATE_H_
