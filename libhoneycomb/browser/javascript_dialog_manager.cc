...
class HoneycombJSDialogCallbackImpl : public HoneycombJSDialogCallback {
 public:
  using CallbackType = content::JavaScriptDialogManager::DialogClosedCallback;

  HoneycombJSDialogCallbackImpl(CallbackType callback)
      : callback_(std::move(callback)) {}
  ~HoneycombJSDialogCallbackImpl() override {
    if (!callback_.is_null()) {
      ...
      if (HONEYCOMB_CURRENTLY_ON_UIT()) {
        ...
        HONEYCOMB_POST_TASK(HONEYCOMB_UIT,
                      base::BindOnce(&HoneycombJSDialogCallbackImpl::CancelNow,
                                     std::move(callback_)));
      }
    }
  }
  ...
  void Continue(bool success, const HoneycombString& user_input) override {
    if (HONEYCOMB_CURRENTLY_ON_UIT()) {
      ...
      HONEYCOMB_POST_TASK(HONEYCOMB_UIT, base::BindOnce(&HoneycombJSDialogCallbackImpl::Continue,
 ...
 private:
  ...
  IMPLEMENT_REFCOUNTING(HoneycombJSDialogCallbackImpl);
};
...
HoneycombJavaScriptDialogManager::HoneycombJavaScriptDialogManager(
    HoneycombBrowserHostBase* browser)
    : browser_(browser), weak_ptr_factory_(this) {}

HoneycombJavaScriptDialogManager::~HoneycombJavaScriptDialogManager() {}

void HoneycombJavaScriptDialogManager::Destroy() {
  if (handler_) {
...
void HoneycombJavaScriptDialogManager::RunJavaScriptDialog(
    content::WebContents* web_contents,
  ...
  *did_suppress_message = false;
  ...
  callback =
      base::BindOnce(&HoneycombJavaScriptDialogManager::DialogClosed,
  ...
  if (auto client = browser_->GetClient()) {
    if (auto handler = client->GetJSDialogHandler()) {
      ...
      HoneycombRefPtr<HoneycombJSDialogCallbackImpl> callbackPtr(
          new HoneycombJSDialogCallbackImpl(std::move(callback)));

      // Execute the user callback.
      bool handled = handler->OnJSDialog(
          browser_, origin_url.spec(),
          static_cast<honey_jsdialog_type_t>(message_type), message_text,
...
void HoneycombJavaScriptDialogManager::RunBeforeUnloadDialog(
    content::WebContents* web_contents,
    content::RenderFrameHost* render_frame_host,
    bool is_reload,
    DialogClosedCallback callback) {
...
bool HoneycombJavaScriptDialogManager::HandleJavaScriptDialog(
    content::WebContents* web_contents,
    bool accept,
...
void HoneycombJavaScriptDialogManager::CancelDialogs(
    content::WebContents* web_contents,
    bool reset_state) {
  if (handler_) {
    if (reset_state) {
...
void HoneycombJavaScriptDialogManager::DialogClosed(
    DialogClosedCallback callback,
    bool success,
...
bool HoneycombJavaScriptDialogManager::InitializeRunner() {
...
bool HoneycombJavaScriptDialogManager::CanUseChromeDialogs() const {
