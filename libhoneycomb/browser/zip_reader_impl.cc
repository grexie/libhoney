...
#include "libhoneycomb/browser/zip_reader_impl.h"
#include <time.h>
...
HoneycombRefPtr<HoneycombZipReader> HoneycombZipReader::Create(
  ...
  HoneycombRefPtr<HoneycombZipReaderImpl> impl(new HoneycombZipReaderImpl());
...
OF((voidpf opaque, voidpf stream, void* buf, uLong size)) {
  HoneycombRefPtr<HoneycombStreamReader> reader(static_cast<HoneycombStreamReader*>(opaque));
...
int ZCALLBACK zlib_close_callback OF((voidpf opaque, voidpf stream)) {
  HoneycombRefPtr<HoneycombStreamReader> reader(static_cast<HoneycombStreamReader*>(opaque));
  // Release the reference added by HoneycombZipReaderImpl::Initialize().
  reader->Release();
  return 0;
}
...
HoneycombZipReaderImpl::HoneycombZipReaderImpl()
...
bool HoneycombZipReaderImpl::Initialize(HoneycombRefPtr<HoneycombStreamReader> stream) {
...
bool HoneycombZipReaderImpl::MoveToFirstFile() {
  if (!VerifyContext()) {
    return false;
  }
...
bool HoneycombZipReaderImpl::MoveToNextFile() {
  if (!VerifyContext()) {
    return false;
  }
...
bool HoneycombZipReaderImpl::MoveToFile(const HoneycombString& fileName,
                                  bool caseSensitive) {
  if (!VerifyContext()) {
    return false;
  }
...
bool HoneycombZipReaderImpl::Close() {
  if (!VerifyContext()) {
    return false;
  }
...
HoneycombString HoneycombZipReaderImpl::GetFileName() {
...
int64_t HoneycombZipReaderImpl::GetFileSize() {
  if (!VerifyContext() || !GetFileInfo()) {
    return -1;
  }
...
HoneycombBaseTime HoneycombZipReaderImpl::GetFileLastModified() {
  if (!VerifyContext() || !GetFileInfo()) {
...
bool HoneycombZipReaderImpl::OpenFile(const HoneycombString& password) {
  if (!VerifyContext()) {
    return false;
  }
...
bool HoneycombZipReaderImpl::CloseFile() {
...
int HoneycombZipReaderImpl::ReadFile(void* buffer, size_t bufferSize) {
  if (!VerifyContext() || !has_fileopen_) {
    return -1;
  }
...
int64_t HoneycombZipReaderImpl::Tell() {
  if (!VerifyContext() || !has_fileopen_) {
    return -1;
  }
...
bool HoneycombZipReaderImpl::Eof() {
  if (!VerifyContext() || !has_fileopen_) {
    return true;
  }
...
bool HoneycombZipReaderImpl::GetFileInfo() {
  if (has_fileinfo_) {
    return true;
  }
...
bool HoneycombZipReaderImpl::VerifyContext() {
