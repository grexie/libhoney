// Copyright 2019 The Honeycomb Authors. Portions copyright
// 2013 The Chromium Authors and 2016 the CEF Authors. All rights reserved. Use of this source code is
// governed by a BSD-style license that can be found in the LICENSE file.

#include "libhoneycomb/browser/devtools/devtools_file_manager.h"
...
HoneycombDevToolsFileManager::HoneycombDevToolsFileManager(
...
void HoneycombDevToolsFileManager::SaveToFile(const std::string& url,
                                        const std::string& content,
                                        bool save_as) {
  Save(url, content, save_as,
       base::BindOnce(&HoneycombDevToolsFileManager::FileSavedAs,
                      weak_factory_.GetWeakPtr(), url),
       base::BindOnce(&HoneycombDevToolsFileManager::CanceledFileSaveAs,
                      weak_factory_.GetWeakPtr(), url));
}

void HoneycombDevToolsFileManager::AppendToFile(const std::string& url,
                                          const std::string& content) {
  Append(url, content,
         base::BindOnce(&HoneycombDevToolsFileManager::AppendedTo,
                        weak_factory_.GetWeakPtr(), url));
}

void HoneycombDevToolsFileManager::Save(const std::string& url,
                                  const std::string& content,
                                  bool save_as,
  ...
  if (!initial_path.empty()) {
    ...
    if (!initial_path.Extension().empty()) {
      params.accept_types.push_back(HoneycombString(initial_path.Extension()));
    }
  }

  browser_impl_->RunFileChooserForBrowser(
      params,
      base::BindOnce(&HoneycombDevToolsFileManager::SaveAsDialogDismissed,
                     weak_factory_.GetWeakPtr(), url, content,
                     std::move(saveCallback), std::move(cancelCallback)));
}

void HoneycombDevToolsFileManager::SaveAsDialogDismissed(
...
void HoneycombDevToolsFileManager::SaveAsFileSelected(const std::string& url,
                                                const std::string& content,
...
void HoneycombDevToolsFileManager::FileSavedAs(const std::string& url,
...
void HoneycombDevToolsFileManager::CanceledFileSaveAs(const std::string& url) {
  base::Value url_value(url);
  CallClientFunction("DevToolsAPI.canceledSaveURL", &url_value, nullptr,
                     nullptr);
}

void HoneycombDevToolsFileManager::Append(const std::string& url,
                                    const std::string& content,
...
void HoneycombDevToolsFileManager::AppendedTo(const std::string& url) {
  base::Value url_value(url);
  CallClientFunction("DevToolsAPI.appendedToURL", &url_value, nullptr, nullptr);
}

void HoneycombDevToolsFileManager::CallClientFunction(
