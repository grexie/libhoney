...
#include "libhoneycomb/browser/devtools/devtools_frontend.h"
...
class HoneycombDevToolsFrontend::NetworkResourceLoader
    : public network::SimpleURLLoaderStreamConsumer {
 public:
  NetworkResourceLoader(int stream_id,
                        HoneycombDevToolsFrontend* bindings,
 ...
 private:
  ...
  HoneycombDevToolsFrontend* const bindings_;
...
// static
HoneycombDevToolsFrontend* HoneycombDevToolsFrontend::Show(
  ...
  HoneycombBrowserSettings new_settings = settings;
  if (!windowInfo.windowless_rendering_enabled &&
      HoneycombColorGetA(new_settings.background_color) != SK_AlphaOPAQUE) {
    // Use white as the default background color for windowed DevTools instead
    // of the HoneycombSettings.background_color value.
  ...
  if (inspected_browser->is_views_hosted()) {
    ...
    create_params.window_info.reset(new HoneycombWindowInfo(windowInfo));
  }
  ...
  HoneycombRefPtr<AlloyBrowserHostImpl> frontend_browser =
  ...
  // destroyed.
  HoneycombDevToolsFrontend* devtools_frontend = new HoneycombDevToolsFrontend(
...
void HoneycombDevToolsFrontend::Activate() {
  frontend_browser_->ActivateContents(web_contents());
}

void HoneycombDevToolsFrontend::Focus() {
  frontend_browser_->SetFocus(true);
}

void HoneycombDevToolsFrontend::InspectElementAt(int x, int y) {
...
void HoneycombDevToolsFrontend::ReadyToCommitNavigation(
    content::NavigationHandle* navigation_handle) {
  content::RenderFrameHost* frame = navigation_handle->GetRenderFrameHost();
  if (navigation_handle->IsInMainFrame()) {
    frontend_host_ = content::DevToolsFrontendHost::Create(
        frame, base::BindRepeating(
                   &HoneycombDevToolsFrontend::HandleMessageFromDevToolsFrontend,
...
void HoneycombDevToolsFrontend::PrimaryMainDocumentElementAvailable() {
...
void HoneycombDevToolsFrontend::WebContentsDestroyed() {
  if (agent_host_) {
    agent_host_->DetachClient(this);
    agent_host_ = nullptr;
  }
...
void HoneycombDevToolsFrontend::HandleMessageFromDevToolsFrontend(
...
void HoneycombDevToolsFrontend::DispatchProtocolMessage(
...
void HoneycombDevToolsFrontend::CallClientFunction(
...
void HoneycombDevToolsFrontend::SendMessageAck(int request_id,
                                         base::Value::Dict arg) {
  CallClientFunction("DevToolsAPI", "embedderMessageAck",
                     base::Value(request_id), base::Value(std::move(arg)));
}

bool HoneycombDevToolsFrontend::ProtocolLoggingEnabled() const {
  return !protocol_log_file_.empty();
}

void HoneycombDevToolsFrontend::LogProtocolMessage(ProtocolMessageType type,
  ...
  auto task_runner = HoneycombTaskRunnerManager::Get()->GetBackgroundTaskRunner();
  task_runner->PostTask(
      FROM_HERE, base::BindOnce(::LogProtocolMessage, protocol_log_file_, type,
                                std::move(to_log)));
}

void HoneycombDevToolsFrontend::AgentHostClosed(
    content::DevToolsAgentHost* agent_host) {
...
PrefService* HoneycombDevToolsFrontend::GetPrefs() const {
