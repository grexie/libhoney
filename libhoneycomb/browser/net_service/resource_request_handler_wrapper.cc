...
#include "libhoneycomb/browser/net_service/resource_request_handler_wrapper.h"
...
#include "libhoneycomb/browser/net_service/cookie_helper.h"
#include "libhoneycomb/browser/net_service/proxy_url_loader_factory.h"
#include "libhoneycomb/browser/net_service/resource_handler_wrapper.h"
#include "libhoneycomb/browser/net_service/response_filter_wrapper.h"
#include "libhoneycomb/browser/prefs/browser_prefs.h"
...
class RequestCallbackWrapper : public HoneycombCallback {
 public:
...
class InterceptedRequestHandlerWrapper : public InterceptedRequestHandler {
 public:
  struct RequestState {
    ...
    void Reset(HoneycombRefPtr<HoneycombResourceRequestHandler> handler,
               HoneycombRefPtr<HoneycombSchemeHandlerFactory> scheme_factory,
               HoneycombRefPtr<HoneycombRequestImpl> request,
    ...
    HoneycombRefPtr<HoneycombResourceRequestHandler> handler_;
    HoneycombRefPtr<HoneycombSchemeHandlerFactory> scheme_factory_;
    HoneycombRefPtr<HoneycombCookieAccessFilter> cookie_filter_;
    HoneycombRefPtr<HoneycombRequestImpl> pending_request_;
    HoneycombRefPtr<HoneycombResponseImpl> pending_response_;
  ...
  // Observer to receive notification of Honeycomb context or associated browser
  // destruction. Only one of the *Destroyed() methods will be called.
  class DestructionObserver : public HoneycombBrowserHostBase::Observer,
                              public HoneycombContext::Observer {
   public:
    explicit DestructionObserver(HoneycombBrowserHostBase* browser) {
      if (browser) {
        browser_info_ = browser->browser_info();
        browser->AddObserver(this);
      } else {
        HoneycombContext::Get()->AddObserver(this);
      }
    }
  ...
  struct InitState {
    ...
    void Initialize(content::BrowserContext* browser_context,
      ...
      auto honey_browser_context = HoneycombBrowserContext::FromProfile(profile);
      browser_context_getter_ = honey_browser_context->getter();
      iothread_state_ = honey_browser_context->iothread_state();
      CHECK(iothread_state_);
      cookieable_schemes_ = honey_browser_context->GetCookieableSchemes();

      // We register to be notified of Honeycomb context or browser destruction so
      ...
      // leaked objects during Honeycomb shutdown).
      ...
      user_agent_ =
          HoneycombAppManager::Get()->GetContentClient()->browser()->GetUserAgent();
    ...
    HoneycombBrowserContext::CookieableSchemes cookieable_schemes_;
    content::GlobalRenderFrameHostId global_id_;
    bool is_navigation_ = true;
    bool is_download_ = false;
    HoneycombString request_initiator_;
  ...
  struct InitHelper : base::RefCountedThreadSafe<InitHelper> {
   public:
    ...
    void MaybeSetInitialized(std::unique_ptr<InitState> init_state) {
      HONEYCOMB_POST_TASK(HONEYCOMB_IOT, base::BindOnce(&InitHelper::SetInitialized, this,
                                            std::move(init_state)));
    }
  ...
  void SetInitialized(std::unique_ptr<InitState> init_state) {
    ...
    // Check that the Honeycomb context or associated browser was not destroyed
    ...
    if (frame) {
      ...
      content::RenderFrameHostImpl* rfh =
          static_cast<content::RenderFrameHostImpl*>(
              static_cast<HoneycombFrameHostImpl*>(frame.get())
                  ->GetRenderFrameHost());
      if (rfh) {
        if (rfh->frame_tree_node() &&
          ...
          // TODO(honey): This approach for retrieving the NavigationRequest is
      ...
      auto honey_browser_context = browser_context_getter.Run();
      auto browser_context = honey_browser_context
                                 ? honey_browser_context->AsBrowserContext()
                                 : nullptr;
      if (browser_context) {
        url_loader_network_observer =
  ...
  void OnBeforeRequest(int32_t request_id,
    ...
    HoneycombRefPtr<HoneycombRequestImpl> requestPtr;
    HoneycombRefPtr<HoneycombResourceRequestHandler> handler =
  ...
  void AllowCookieLoad(int32_t request_id,
    ...
    HoneycombCookie honey_cookie;
    if (net_service::MakeHoneycombCookie(cookie, honey_cookie)) {
      *allow = state->cookie_filter_->CanSendCookie(
          init_state_->browser_, init_state_->frame_,
          state->pending_request_.get(), honey_cookie);
    }
  }
  ...
  void ShouldInterceptRequest(
    ...
    if (state->handler_) {
      ...
      HoneycombRefPtr<RequestCallbackWrapper> callbackPtr =
      ...
      honey_return_value_t retval = state->handler_->OnBeforeResourceLoad(
          init_state_->browser_, init_state_->frame_,
  ...
  void ContinueShouldInterceptRequest(
    ...
    if (state->handler_) {
      ...
      const bool redirect =
          (state->pending_request_->GetChanges() & HoneycombRequestImpl::kChangedUrl);
      if (redirect) {
    ...
    HoneycombRefPtr<HoneycombResourceHandler> resource_handler;
  ...
  void ProcessResponseHeaders(int32_t request_id,
    ...
    if (!state->pending_response_) {
      state->pending_response_ = new HoneycombResponseImpl();
    } else {
  ...
  void HandleRedirect(int32_t request_id,
    ...
    HoneycombString newUrl = redirect_info.new_url.spec();
    HoneycombString oldUrl = newUrl;
  ...
  void HandleResponse(int32_t request_id,
    ...
    if (state->handler_->OnResourceResponse(
      ...
      if (changes) {
        ...
        if (changes & HoneycombRequestImpl::kChangedUrl) {
 ...
 private:
  void CallHandlerOnComplete(RequestState* state,
    ...
    state->pending_response_->SetError(
        static_cast<honey_errorcode_t>(status.error_code));
    state->pending_response_->SetReadOnly(true);
  ...
  HoneycombRefPtr<HoneycombResourceRequestHandler> GetHandler(
      int32_t request_id,
      network::ResourceRequest* request,
      bool* intercept_only,
      HoneycombRefPtr<HoneycombRequestImpl>& requestPtr) const {
    HoneycombRefPtr<HoneycombResourceRequestHandler> handler;
    ...
    if (init_state_->browser_) {
      ...
      if (client) {
        HoneycombRefPtr<HoneycombRequestHandler> request_handler =
    ...
    if (!handler) {
      ...
      HoneycombRefPtr<HoneycombRequestContextHandler> context_handler =
  ...
  // Honeycomb context or associated browser is destroyed.
  void OnDestroyed() {
    HONEYCOMB_REQUIRE_IOT();
    DCHECK(init_state_);
  ...
  static HoneycombRefPtr<HoneycombRequestImpl> MakeRequest(
      const network::ResourceRequest* request,
      int64_t request_id,
      bool read_only) {
    HoneycombRefPtr<HoneycombRequestImpl> requestPtr = new HoneycombRequestImpl();
...
std::unique_ptr<InterceptedRequestHandler> CreateInterceptedRequestHandler(
  ...
  HoneycombRefPtr<HoneycombBrowserHostBase> browserPtr;
  HoneycombRefPtr<HoneycombFrame> framePtr;

  // Default to handlers for the same process in case |frame| doesn't have an
  // associated HoneycombBrowserHost.
  ...
  if (frame) {
    ...
    browserPtr = HoneycombBrowserHostBase::GetBrowserForHost(frame);
    if (browserPtr) {
...
std::unique_ptr<InterceptedRequestHandler> CreateInterceptedRequestHandler(
  ...
  if (request.is_outermost_main_frame ||
    ...
    if (!frame) {
      // Use the main frame for the HoneycombBrowserHost.
      frame = web_contents->GetPrimaryMainFrame();
      CHECK(frame);
    }
  }
