...
#include "libhoneycomb/browser/prefs/pref_registrar.h"
#include "libhoneycomb/browser/prefs/pref_store.h"
...
std::string GetAcceptLanguageListSetting(Profile* profile) {
  if (auto* browser_context = HoneycombBrowserContext::FromProfile(profile)) {
    const auto& settings = browser_context->settings();
    if (settings.accept_language_list.length > 0) {
      return HoneycombString(&settings.accept_language_list);
    }
  }

  const auto& settings = HoneycombContext::Get()->settings();
  if (settings.accept_language_list.length > 0) {
    return HoneycombString(&settings.accept_language_list);
  }
...
void RegisterLocalStatePrefs(PrefRegistrySimple* registry) {
  pref_registrar::RegisterCustomPrefs(HONEYCOMB_PREFERENCES_TYPE_GLOBAL, registry);
}

void RegisterProfilePrefs(PrefRegistrySimple* registry) {
  pref_registrar::RegisterCustomPrefs(HONEYCOMB_PREFERENCES_TYPE_REQUEST_CONTEXT,
                                      registry);
}
...
std::unique_ptr<PrefService> CreatePrefService(Profile* profile,
  ...
  if (store_on_disk) {
    ...
    scoped_refptr<HoneycombPrefStore> honey_pref_store = new HoneycombPrefStore();
    honey_pref_store->SetInitializationCompleted();
    factory.set_user_prefs(honey_pref_store.get());
  }
  ...
  if (profile) {
    ...
    if (store_on_disk) {
      ...
      scoped_refptr<HoneycombPrefStore> honey_pref_store = new HoneycombPrefStore();
      honey_pref_store->SetInitializationCompleted();
      supervised_user_settings->Init(honey_pref_store);
    }
  ...
  // Some preferences are specific to Honeycomb and others are defined in Chromium.
  ...
  //    otherwise inappropiate logic (e.g. calls to objects that Honeycomb doesn't
  //    use) then register the preferences directly instead of calling the
  //    existing registration method.

  // Default preferences.
  HoneycombMediaCaptureDevicesDispatcher::RegisterPrefs(registry.get());
...
std::string GetAcceptLanguageList(Profile* profile) {
  // Always prefer to the Honeycomb settings configuration, if specified.
