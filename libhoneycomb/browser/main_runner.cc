...
#include "libhoneycomb/browser/main_runner.h"

#include "libhoneycomb/browser/browser_message_loop.h"
#include "libhoneycomb/browser/thread_util.h"
#include "libhoneycomb/common/alloy/alloy_main_runner_delegate.h"
#include "libhoneycomb/common/honey_switches.h"
#include "libhoneycomb/common/chrome/chrome_main_runner_delegate.h"
...
std::unique_ptr<HoneycombMainRunnerDelegate> MakeDelegate(
    RuntimeType type,
    HoneycombMainRunnerHandler* runner,
...
// components/crash/core/app/BUILD.gn. Honeycomb uses an embedded handler on all
...
class HoneycombUIThread : public base::PlatformThread::Delegate {
 public:
  HoneycombUIThread(HoneycombMainRunner* runner, base::OnceClosure setup_callback)
      : runner_(runner), setup_callback_(std::move(setup_callback)) {}
  ~HoneycombUIThread() override { Stop(); }
  ...
  void Stop() {
    ...
    if (!stopping_) {
      stopping_ = true;
      HONEYCOMB_POST_TASK(HONEYCOMB_UIT, base::BindOnce(&HoneycombMainRunner::QuitMessageLoop,
 ...
 protected:
  void ThreadMain() override {
    base::PlatformThread::SetName("HoneycombUIThread");
  ...
  HoneycombMainRunner* const runner_;
...
HoneycombMainRunner::HoneycombMainRunner(bool multi_threaded_message_loop,
...
HoneycombMainRunner::~HoneycombMainRunner() = default;

bool HoneycombMainRunner::Initialize(HoneycombSettings* settings,
                               HoneycombRefPtr<HoneycombApp> application,
...
void HoneycombMainRunner::Shutdown(base::OnceClosure shutdown_on_ui_thread,
  ...
  if (multi_threaded_message_loop_) {
    ...
    HONEYCOMB_POST_TASK(
        HONEYCOMB_UIT,
        base::BindOnce(&HoneycombMainRunner::FinishShutdownOnUIThread,
...
void HoneycombMainRunner::RunMessageLoop() {
...
void HoneycombMainRunner::QuitMessageLoop() {
...
// static
int HoneycombMainRunner::RunAsHelperProcess(const HoneycombMainArgs& args,
                                      HoneycombRefPtr<HoneycombApp> application,
                                      void* windows_sandbox_info) {
...
int HoneycombMainRunner::ContentMainInitialize(const HoneycombMainArgs& args,
                                         void* windows_sandbox_info,
                                         int* no_sandbox) {
...
bool HoneycombMainRunner::ContentMainRun(bool* initialized,
                                   base::OnceClosure context_initialized) {
  ...
  if (multi_threaded_message_loop_) {
    ...
    if (!CreateUIThread(base::BindOnce(
            [](HoneycombMainRunner* runner, base::WaitableEvent* event) {
  ...
  if (HONEYCOMB_CURRENTLY_ON_UIT()) {
    ...
    HONEYCOMB_POST_TASK(HONEYCOMB_UIT, base::BindOnce(&HoneycombMainRunner::OnContextInitialized,
                                          base::Unretained(this),
...
void HoneycombMainRunner::PreBrowserMain() {
  if (external_message_pump_) {
    InitExternalMessagePumpFactoryForUI();
  }
}

int HoneycombMainRunner::RunMainProcess(
    content::MainFunctionParams main_function_params) {
  if (!multi_threaded_message_loop_) {
    // Use our own browser process runner.
    browser_runner_ = content::BrowserMainRunner::Create();
...
bool HoneycombMainRunner::CreateUIThread(base::OnceClosure setup_callback) {
  DCHECK(!ui_thread_);

  ui_thread_.reset(new HoneycombUIThread(this, std::move(setup_callback)));
  ui_thread_->Start();
...
void HoneycombMainRunner::OnContextInitialized(
    base::OnceClosure context_initialized) {
...
void HoneycombMainRunner::FinishShutdownOnUIThread(
...
void HoneycombMainRunner::FinalizeShutdown(base::OnceClosure finalize_shutdown) {
...
// From libhoneycomb/features/runtime.h:
namespace honey {
