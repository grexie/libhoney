...
HONEYCOMB_EXPORT XDisplay* honey_get_xdisplay() {
...
HoneycombWindowX11::HoneycombWindowX11(HoneycombRefPtr<HoneycombBrowserHostBase> browser,
...
HoneycombWindowX11::~HoneycombWindowX11() {
...
void HoneycombWindowX11::SetBounds(const gfx::Rect& bounds) {
  if (xwindow_ == x11::Window::None) {
    return;
  }
...
gfx::Rect HoneycombWindowX11::GetBoundsInScreen() {
  if (auto coords =
          connection_
...
views::DesktopWindowTreeHostLinux* HoneycombWindowX11::GetHost() {
  if (browser_.get()) {
    auto child = FindChild(xwindow_);
...
bool HoneycombWindowX11::CanDispatchEvent(const ui::PlatformEvent& event) {
  auto* dispatching_event = connection_->dispatching_event();
  return dispatching_event && dispatching_event->window() == xwindow_;
}

uint32_t HoneycombWindowX11::DispatchEvent(const ui::PlatformEvent& event) {
...
void HoneycombWindowX11::OnEvent(const x11::Event& event) {
...
void HoneycombWindowX11::ContinueFocus() {
  if (!focus_pending_) {
    return;
  }
  if (browser_.get()) {
...
bool HoneycombWindowX11::TopLevelAlwaysOnTop() const {
  auto toplevel_window = FindToplevelParent(xwindow_);
...
void HoneycombWindowX11::ProcessXEvent(const x11::Event& event) {
  if (auto* configure = event.As<x11::ConfigureNotifyEvent>()) {
    ...
    if (focus->opcode == x11::FocusEvent::In) {
      ...
      if (!focus_pending_) {
        focus_pending_ = true;
        HONEYCOMB_POST_DELAYED_TASK(HONEYCOMB_UIT,
                              base::BindOnce(&HoneycombWindowX11::ContinueFocus,
