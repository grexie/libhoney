...
#include "libhoneycomb/browser/native/browser_platform_delegate_native_linux.h"
...
#include "libhoneycomb/browser/native/window_x11.h"
#include "ui/events/keycodes/keyboard_code_conversion_x.h"
#include "ui/events/keycodes/keyboard_code_conversion_xkb.h"
#include "ui/views/widget/desktop_aura/desktop_window_tree_host_linux.h"
#endif

HoneycombBrowserPlatformDelegateNativeLinux::HoneycombBrowserPlatformDelegateNativeLinux(
    const HoneycombWindowInfo &window_info,
    SkColor background_color)
    : HoneycombBrowserPlatformDelegateNativeAura(window_info, background_color) {}

void HoneycombBrowserPlatformDelegateNativeLinux::BrowserDestroyed(
...
bool HoneycombBrowserPlatformDelegateNativeLinux::CreateHostWindow()
{
  ...
  // window is destroyed.
  window_x11_ =
      new HoneycombWindowX11(browser_, parent_window, rect,
                             HoneycombString(&window_info_.window_name).ToString());
  DCHECK_NE(window_x11_->xwindow(), x11::Window::None);
  window_info_.window =
      static_cast<honey_window_handle_t>(window_x11_->xwindow());
...
void HoneycombBrowserPlatformDelegateNativeLinux::CloseHostWindow()
{
#if BUILDFLAG(OZONE_PLATFORM_X11)
  if (window_x11_)
  {
...
HoneycombWindowHandle HoneycombBrowserPlatformDelegateNativeLinux::GetHostWindowHandle()
    const
{
...
views::Widget *HoneycombBrowserPlatformDelegateNativeLinux::GetWindowWidget() const
{
  return window_widget_;
}

void HoneycombBrowserPlatformDelegateNativeLinux::SetFocus(bool setFocus)
{
  if (!setFocus)
  {
    return;
...
void HoneycombBrowserPlatformDelegateNativeLinux::NotifyMoveOrResizeStarted()
{
...
void HoneycombBrowserPlatformDelegateNativeLinux::SizeTo(int width, int height)
{
#if BUILDFLAG(OZONE_PLATFORM_X11)
  if (window_x11_)
  {
...
void HoneycombBrowserPlatformDelegateNativeLinux::ViewText(const std::string &text)
{
  char buff[] = "/tmp/HoneycombSourceXXXXXX";
  int fd = mkstemp(buff);
...
bool HoneycombBrowserPlatformDelegateNativeLinux::HandleKeyboardEvent(
    const content::NativeWebKeyboardEvent &event)
{
  // TODO(honey): Is something required here to handle shortcut keys?
  return false;
}

HoneycombEventHandle HoneycombBrowserPlatformDelegateNativeLinux::GetEventHandle(
    const content::NativeWebKeyboardEvent &event) const
{
  // TODO(honey): We need to return an XEvent* from this method, but
  // |event.os_event->native_event()| now returns a ui::Event* instead.
  // See https://crbug.com/965991.
  return nullptr;
}

ui::KeyEvent HoneycombBrowserPlatformDelegateNativeLinux::TranslateUiKeyEvent(
...
HoneycombBrowserPlatformDelegateNativeLinux::TranslateWebKeyEvent(
    const HoneycombKeyEvent &key_event) const
{
