...
#include "libhoneycomb/browser/native/browser_platform_delegate_native_mac.h"
...
#include "libhoneycomb/browser/native/javascript_dialog_runner_mac.h"
#include "libhoneycomb/browser/native/menu_runner_mac.h"
...
// the view is deleted.
@interface HoneycombBrowserHostView : NSView {
 @private
  HoneycombBrowserHostBase* browser_;  // weak
}

@property(nonatomic, assign) HoneycombBrowserHostBase* browser;

@end

@implementation HoneycombBrowserHostView
...
@interface HoneycombWindowDelegate : NSObject <NSWindowDelegate> {
 @private
  HoneycombBrowserHostBase* browser_;  // weak
  NSWindow* __strong window_;
}
- (id)initWithWindow:(NSWindow*)window andBrowser:(HoneycombBrowserHostBase*)browser;
@end

@implementation HoneycombWindowDelegate
...
NSTimeInterval currentEventTimestamp() {
  ...
  if (currentEvent) {
    ...
    // Another option is to have the timestamp as a field in HoneycombEvent structures
    // and let the client provide it.
    return 0;
  }
}

NSUInteger NativeModifiers(int honey_modifiers) {
...
HoneycombBrowserPlatformDelegateNativeMac::HoneycombBrowserPlatformDelegateNativeMac(
    const HoneycombWindowInfo& window_info,
    SkColor background_color)
    : HoneycombBrowserPlatformDelegateNative(window_info, background_color) {}

void HoneycombBrowserPlatformDelegateNativeMac::BrowserDestroyed(
    HoneycombBrowserHostBase* browser) {
  HoneycombBrowserPlatformDelegateNative::BrowserDestroyed(browser);
...
bool HoneycombBrowserPlatformDelegateNativeMac::CreateHostWindow() {
  base::apple::ScopedNSAutoreleasePool autorelease_pool;

  NSWindow* new_window = nil;

  NSView* parent_view =
      CAST_HONEYCOMB_WINDOW_HANDLE_TO_NSVIEW(window_info_.parent_view);
  ...
  if (parent_view == nil) {
    ...
    window_delegate_ = [[HoneycombWindowDelegate alloc] initWithWindow:new_window
                                                      andBrowser:browser_];
    ...
    window_info_.parent_view = CAST_NSVIEW_TO_HONEYCOMB_WINDOW_HANDLE(parent_view);
  ...
  HoneycombBrowserHostView* browser_view =
      [[HoneycombBrowserHostView alloc] initWithFrame:browser_view_rect];
  ...
  window_info_.view = CAST_NSVIEW_TO_HONEYCOMB_WINDOW_HANDLE(browser_view);
...
void HoneycombBrowserPlatformDelegateNativeMac::CloseHostWindow() {
  NSView* nsview = CAST_HONEYCOMB_WINDOW_HANDLE_TO_NSVIEW(window_info_.view);
  if (nsview != nil) {
...
HoneycombWindowHandle HoneycombBrowserPlatformDelegateNativeMac::GetHostWindowHandle()
    const {
...
void HoneycombBrowserPlatformDelegateNativeMac::SendKeyEvent(
...
void HoneycombBrowserPlatformDelegateNativeMac::SendMouseClickEvent(
...
void HoneycombBrowserPlatformDelegateNativeMac::SendMouseMoveEvent(
    const HoneycombMouseEvent& event,
    bool mouseLeave) {
  auto view = GetHostView();
  if (!view) {
...
void HoneycombBrowserPlatformDelegateNativeMac::SendMouseWheelEvent(
    const HoneycombMouseEvent& event,
    int deltaX,
    int deltaY) {
  auto view = GetHostView();
...
void HoneycombBrowserPlatformDelegateNativeMac::SendTouchEvent(
    const HoneycombTouchEvent& event) {
  NOTIMPLEMENTED();
}

void HoneycombBrowserPlatformDelegateNativeMac::SetFocus(bool setFocus) {
  auto view = GetHostView();
  if (view) {
...
gfx::Point HoneycombBrowserPlatformDelegateNativeMac::GetScreenPoint(
  ...
  NSView* nsview = CAST_HONEYCOMB_WINDOW_HANDLE_TO_NSVIEW(window_info_.parent_view);
  if (nsview) {
...
void HoneycombBrowserPlatformDelegateNativeMac::ViewText(const std::string& text) {
  // TODO(honey): Implement this functionality.
  NOTIMPLEMENTED();
}

bool HoneycombBrowserPlatformDelegateNativeMac::HandleKeyboardEvent(
...
HoneycombEventHandle HoneycombBrowserPlatformDelegateNativeMac::GetEventHandle(
    const content::NativeWebKeyboardEvent& event) const {
  return CAST_NSEVENT_TO_HONEYCOMB_EVENT_HANDLE(event.os_event.Get());
}

std::unique_ptr<HoneycombJavaScriptDialogRunner>
HoneycombBrowserPlatformDelegateNativeMac::CreateJavaScriptDialogRunner() {
  return base::WrapUnique(new HoneycombJavaScriptDialogRunnerMac);
}

std::unique_ptr<HoneycombMenuRunner>
HoneycombBrowserPlatformDelegateNativeMac::CreateMenuRunner() {
  return base::WrapUnique(new HoneycombMenuRunnerMac);
}

content::NativeWebKeyboardEvent
HoneycombBrowserPlatformDelegateNativeMac::TranslateWebKeyEvent(
...
blink::WebMouseEvent
HoneycombBrowserPlatformDelegateNativeMac::TranslateWebClickEvent(
...
blink::WebMouseEvent HoneycombBrowserPlatformDelegateNativeMac::TranslateWebMoveEvent(
    const HoneycombMouseEvent& mouse_event,
...
HoneycombBrowserPlatformDelegateNativeMac::TranslateWebWheelEvent(
    const HoneycombMouseEvent& mouse_event,
    int deltaX,
    int deltaY) const {
...
void HoneycombBrowserPlatformDelegateNativeMac::TranslateWebMouseEvent(
    blink::WebMouseEvent& result,
...
HoneycombBrowserPlatformDelegateNativeMac::GetHostView() const {
  if (!web_contents_) {
    return nullptr;
  }
