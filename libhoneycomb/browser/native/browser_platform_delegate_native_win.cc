...
#include "libhoneycomb/browser/native/browser_platform_delegate_native_win.h"
...
#include "libhoneycomb/browser/native/window_delegate_view.h"
...
HoneycombRect GetScreenFrameRectFromDIPContentRect(HWND window,
                                             gfx::Rect dip_rect,
                                             DWORD style,
                                             DWORD ex_style,
                                             bool has_menu) {
  ...
  return HoneycombRect(screen_rect.x(), screen_rect.y(), rect.right - rect.left,
                 rect.bottom - rect.top);
}

HoneycombRect GetAdjustedScreenFrameRect(HoneycombRect screen_rect,
                                   DWORD style,
                                   DWORD ex_style,
                                   bool has_menu) {
  ...
  if (screen_rect.width == CW_USEDEFAULT ||
    ...
    return HoneycombRect(CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT);
  }
...
HoneycombBrowserPlatformDelegateNativeWin::HoneycombBrowserPlatformDelegateNativeWin(
    const HoneycombWindowInfo& window_info,
    SkColor background_color)
    : HoneycombBrowserPlatformDelegateNativeAura(window_info, background_color) {}

void HoneycombBrowserPlatformDelegateNativeWin::set_widget(
    views::Widget* widget,
    HoneycombWindowHandle widget_handle) {
...
void HoneycombBrowserPlatformDelegateNativeWin::BrowserDestroyed(
    HoneycombBrowserHostBase* browser) {
  HoneycombBrowserPlatformDelegateNativeAura::BrowserDestroyed(browser);
...
bool HoneycombBrowserPlatformDelegateNativeWin::CreateHostWindow() {
  RegisterWindowClass();

  if (window_info_.style == 0) {
    // Client didn't intialize the HoneycombWindowInfo. Provide reasonable defaults.
    window_info_.SetAsPopup(nullptr, HoneycombString());
  }

  has_frame_ = !(window_info_.style & WS_CHILD);

  const std::wstring windowName(HoneycombString(&window_info_.window_name));

  HoneycombRect window_rect = window_info_.bounds;
  ...
  HoneycombWindowDelegateView* delegate_view = new HoneycombWindowDelegateView(
...
void HoneycombBrowserPlatformDelegateNativeWin::CloseHostWindow() {
...
HoneycombWindowHandle HoneycombBrowserPlatformDelegateNativeWin::GetHostWindowHandle()
    const {
...
views::Widget* HoneycombBrowserPlatformDelegateNativeWin::GetWindowWidget() const {
  return window_widget_;
}

void HoneycombBrowserPlatformDelegateNativeWin::SetFocus(bool setFocus) {
  if (!setFocus) {
    return;
  }
  ...
  if (window_widget_) {
    ...
    // "HoneycombBrowserWindow" if we're called in response to our WndProc receiving
    // the WM_SETFOCUS event (possibly due to "HoneycombBrowserWindow" recieving the
    // top-level WM_ACTIVATE event), or some other HWND if the client calls
    // HoneycombBrowserHost::SetFocus(true) directly. DesktopWindowTreeHostWin may
    ...
    //    "HoneycombBrowserWindow") instead of the expected "Chrome_WidgetWin_0" HWND,
    ...
    // code path doesn't work for Honeycomb because IsTopLevelWindow in
...
void HoneycombBrowserPlatformDelegateNativeWin::NotifyMoveOrResizeStarted() {
  // Call the parent method to dismiss any existing popups.
  HoneycombBrowserPlatformDelegateNativeAura::NotifyMoveOrResizeStarted();
...
void HoneycombBrowserPlatformDelegateNativeWin::SizeTo(int width, int height) {
...
void HoneycombBrowserPlatformDelegateNativeWin::ViewText(const std::string& text) {
  HONEYCOMB_POST_USER_VISIBLE_TASK(base::BindOnce(WriteTempFileAndView, text));
}

bool HoneycombBrowserPlatformDelegateNativeWin::HandleKeyboardEvent(
...
HoneycombEventHandle HoneycombBrowserPlatformDelegateNativeWin::GetEventHandle(
...
ui::KeyEvent HoneycombBrowserPlatformDelegateNativeWin::TranslateUiKeyEvent(
...
gfx::Vector2d HoneycombBrowserPlatformDelegateNativeWin::GetUiWheelEventOffset(
    int deltaX,
    int deltaY) const {
...
// static
void HoneycombBrowserPlatformDelegateNativeWin::RegisterWindowClass() {
  ...
  WNDCLASSEX wcex = {
      ...
      /* lpfnWndProc = */ HoneycombBrowserPlatformDelegateNativeWin::WndProc,
      /* cbClsExtra = */ 0,
      /* cbWndExtra = */ 0,
      ...
      /* hbrBackground = */ 0,
      /* lpszMenuName = */ NULL,
      /* lpszClassName = */ HoneycombBrowserPlatformDelegateNativeWin::GetWndClass(),
      /* hIconSm = */ NULL,
  };
...
// static
LPCTSTR HoneycombBrowserPlatformDelegateNativeWin::GetWndClass() {
  return L"HoneycombBrowserWindow";
}

// static
LRESULT CALLBACK HoneycombBrowserPlatformDelegateNativeWin::WndProc(HWND hwnd,
                                                              UINT message,
                                                              WPARAM wParam,
                                                              LPARAM lParam) {
  HoneycombBrowserPlatformDelegateNativeWin* platform_delegate = nullptr;
  HoneycombBrowserHostBase* browser = nullptr;

  if (message != WM_NCCREATE) {
    platform_delegate = static_cast<HoneycombBrowserPlatformDelegateNativeWin*>(
  ...
  switch (message) {
    ...
    case WM_NCCREATE: {
      ...
      platform_delegate =
          reinterpret_cast<HoneycombBrowserPlatformDelegateNativeWin*>(
              cs->lpCreateParams);
