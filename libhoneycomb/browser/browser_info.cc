...
HoneycombBrowserInfo::FrameInfo::~FrameInfo() {
#if DCHECK_IS_ON()
...
HoneycombBrowserInfo::HoneycombBrowserInfo(int browser_id,
                               bool is_popup,
                               bool is_windowless,
...
HoneycombBrowserInfo::~HoneycombBrowserInfo() {
  DCHECK(frame_info_set_.empty());
}

HoneycombRefPtr<HoneycombBrowserHostBase> HoneycombBrowserInfo::browser() const {
...
void HoneycombBrowserInfo::SetBrowser(HoneycombRefPtr<HoneycombBrowserHostBase> browser) {
...
void HoneycombBrowserInfo::SetClosing() {
  base::AutoLock lock_scope(lock_);
  DCHECK(!is_closing_);
  is_closing_ = true;
}

void HoneycombBrowserInfo::MaybeCreateFrame(content::RenderFrameHost* host,
                                      bool is_guest_view) {
  ...
  // Guest views don't get their own HoneycombBrowser or HoneycombFrame objects.
  if (!is_guest_view) {
    // Create a new frame object.
    frame_info->frame_ = new HoneycombFrameHostImpl(this, host);
...
void HoneycombBrowserInfo::FrameHostStateChanged(
...
void HoneycombBrowserInfo::RemoveFrame(content::RenderFrameHost* host) {
  HONEYCOMB_REQUIRE_UIT();
  ...
  // And finally delete the frame info.
  {
    ...
    if (other_frame_info->frame_ && !other_frame_info->IsCurrentMainFrame()) {
      if (other_frame_info->frame_->Detach(
              HoneycombFrameHostImpl::DetachReason::RENDER_FRAME_DELETED)) {
...
HoneycombRefPtr<HoneycombFrameHostImpl> HoneycombBrowserInfo::GetMainFrame() {
  base::AutoLock lock_scope(lock_);
...
HoneycombRefPtr<HoneycombFrameHostImpl> HoneycombBrowserInfo::CreateTempSubFrame(
    const content::GlobalRenderFrameHostId& parent_global_id) {
  HoneycombRefPtr<HoneycombFrameHostImpl> parent = GetFrameForGlobalId(parent_global_id);
  if (!parent) {
    parent = GetMainFrame();
  }
  // Intentionally not notifying for temporary frames.
  return new HoneycombFrameHostImpl(this, parent->GetIdentifier());
}

HoneycombRefPtr<HoneycombFrameHostImpl> HoneycombBrowserInfo::GetFrameForHost(
...
HoneycombRefPtr<HoneycombFrameHostImpl> HoneycombBrowserInfo::GetFrameForGlobalId(
...
HoneycombBrowserInfo::FrameHostList HoneycombBrowserInfo::GetAllFrames() const {
  base::AutoLock lock_scope(lock_);
...
HoneycombBrowserInfo::NavigationLock::NavigationLock() : weak_ptr_factory_(this) {}

HoneycombBrowserInfo::NavigationLock::~NavigationLock() {
  HONEYCOMB_REQUIRE_UIT();
  if (pending_action_) {
    HONEYCOMB_POST_TASK(HONEYCOMB_UIT, std::move(pending_action_));
  }
}

scoped_refptr<HoneycombBrowserInfo::NavigationLock>
HoneycombBrowserInfo::CreateNavigationLock() {
  HONEYCOMB_REQUIRE_UIT();
...
bool HoneycombBrowserInfo::IsNavigationLocked(base::OnceClosure pending_action) {
  HONEYCOMB_REQUIRE_UIT();
  if (navigation_lock_) {
...
void HoneycombBrowserInfo::MaybeExecuteFrameNotification(
    FrameNotifyOnceAction pending_action) {
  HoneycombRefPtr<HoneycombFrameHandler> frame_handler;
...
void HoneycombBrowserInfo::MaybeNotifyDraggableRegionsChanged(
...
void HoneycombBrowserInfo::SetMainFrame(HoneycombRefPtr<HoneycombBrowserHostBase> browser,
                                  HoneycombRefPtr<HoneycombFrameHostImpl> frame) {
  ...
  if (main_frame_) {
    ...
    if (old_frame->Detach(HoneycombFrameHostImpl::DetachReason::NEW_MAIN_FRAME)) {
...
void HoneycombBrowserInfo::MaybeNotifyFrameCreated(
    HoneycombRefPtr<HoneycombFrameHostImpl> frame) {
  HONEYCOMB_REQUIRE_UIT();
  ...
  MaybeExecuteFrameNotification(base::BindOnce(
      [](scoped_refptr<HoneycombBrowserInfo> self, HoneycombRefPtr<HoneycombFrameHostImpl> frame,
         HoneycombRefPtr<HoneycombFrameHandler> handler) {
        if (auto browser = self->browser()) {
          handler->OnFrameCreated(browser, frame);
        }
      },
      scoped_refptr<HoneycombBrowserInfo>(this), frame));
}

// Passing in |browser| here because |browser_| may already be cleared.
void HoneycombBrowserInfo::MaybeNotifyFrameDetached(
    HoneycombRefPtr<HoneycombBrowserHostBase> browser,
    HoneycombRefPtr<HoneycombFrameHostImpl> frame) {
  HONEYCOMB_REQUIRE_UIT();
...
// Passing in |browser| here because |browser_| may already be cleared.
void HoneycombBrowserInfo::MaybeNotifyMainFrameChanged(
    HoneycombRefPtr<HoneycombBrowserHostBase> browser,
    HoneycombRefPtr<HoneycombFrameHostImpl> old_frame,
...
void HoneycombBrowserInfo::RemoveAllFrames(
    HoneycombRefPtr<HoneycombBrowserHostBase> old_browser) {
  lock_.AssertAcquired();

  // Make sure any callbacks will see the correct state (e.g. like
  // HoneycombBrowser::GetMainFrame returning nullptr and HoneycombBrowser::IsValid
  // returning false).
  DCHECK(!browser_);
  DCHECK(old_browser);
  ...
  for (auto& info : frame_info_set_) {
    if (info->frame_ && !info->IsCurrentMainFrame()) {
      if (info->frame_->Detach(
              HoneycombFrameHostImpl::DetachReason::BROWSER_DESTROYED)) {
...
HoneycombBrowserInfo::NotificationStateLock::NotificationStateLock(
    HoneycombBrowserInfo* browser_info)
