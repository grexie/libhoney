...
#include "libhoneycomb/browser/alloy/dialogs/alloy_javascript_dialog_manager_delegate.h"
#include "libhoneycomb/browser/extensions/browser_extensions_util.h"
#include "libhoneycomb/browser/extensions/extension_background_host.h"
...
HoneycombBrowserPlatformDelegateAlloy::HoneycombBrowserPlatformDelegateAlloy()
    : weak_ptr_factory_(this) {}

content::WebContents* HoneycombBrowserPlatformDelegateAlloy::CreateWebContents(
  ...
  auto browser_context = honey_browser_context->AsBrowserContext();
...
void HoneycombBrowserPlatformDelegateAlloy::WebContentsCreated(
...
void HoneycombBrowserPlatformDelegateAlloy::AddNewContents(
  ...
  HoneycombRefPtr<AlloyBrowserHostImpl> owner =
      AlloyBrowserHostImpl::GetBrowserForContents(new_contents.get());
  if (owner) {
    // Taking ownership of |new_contents|.
    static_cast<HoneycombBrowserPlatformDelegateAlloy*>(
...
bool HoneycombBrowserPlatformDelegateAlloy::
...
void HoneycombBrowserPlatformDelegateAlloy::RenderViewReady() {
  ConfigureAutoResize();
}

void HoneycombBrowserPlatformDelegateAlloy::BrowserCreated(
...
void HoneycombBrowserPlatformDelegateAlloy::CreateExtensionHost(
  ...
  if (host_type == extensions::mojom::ViewType::kExtensionDialog ||
    ...
    extension_host_ = new extensions::HoneycombExtensionViewHost(
    ...
    extension_host_ = new extensions::HoneycombExtensionBackgroundHost(
        alloy_browser,
        base::BindOnce(&HoneycombBrowserPlatformDelegateAlloy::OnExtensionHostDeleted,
...
extensions::ExtensionHost* HoneycombBrowserPlatformDelegateAlloy::GetExtensionHost()
    const {
  return extension_host_;
}

void HoneycombBrowserPlatformDelegateAlloy::BrowserDestroyed(
    HoneycombBrowserHostBase* browser) {
  if (primary_) {
...
HoneycombBrowserPlatformDelegateAlloy::GetWebContentsModalDialogHost() const {
  return web_contents_dialog_helper_.get();
}

void HoneycombBrowserPlatformDelegateAlloy::SendCaptureLostEvent() {
  if (!web_contents_) {
    return;
  }
...
void HoneycombBrowserPlatformDelegateAlloy::NotifyMoveOrResizeStarted() {
  if (!browser_) {
    return;
  }
  ...
  if (frame && frame->IsValid()) {
    static_cast<HoneycombFrameHostImpl*>(frame.get())->NotifyMoveOrResizeStarted();
  }
}
#endif

bool HoneycombBrowserPlatformDelegateAlloy::PreHandleGestureEvent(
    content::WebContents* source,
...
bool HoneycombBrowserPlatformDelegateAlloy::IsNeverComposited(
    content::WebContents* web_contents) {
  if (extension_host_) {
...
void HoneycombBrowserPlatformDelegateAlloy::SetAutoResizeEnabled(
    bool enabled,
...
void HoneycombBrowserPlatformDelegateAlloy::ConfigureAutoResize() {
...
void HoneycombBrowserPlatformDelegateAlloy::SetAccessibilityState(
...
bool HoneycombBrowserPlatformDelegateAlloy::IsPrintPreviewSupported() const {
  REQUIRE_ALLOY_RUNTIME();
  ...
  auto honey_browser_context =
      HoneycombBrowserContext::FromBrowserContext(web_contents_->GetBrowserContext());
  return honey_browser_context->IsPrintPreviewSupported();
}

void HoneycombBrowserPlatformDelegateAlloy::Find(const HoneycombString& searchText,
                                           bool forward,
                                           bool matchCase,
                                           bool findNext) {
  if (!web_contents_) {
...
void HoneycombBrowserPlatformDelegateAlloy::StopFinding(bool clearSelection) {
  if (!web_contents_) {
    return;
  }
...
bool HoneycombBrowserPlatformDelegateAlloy::HandleFindReply(
    int request_id,
...
HoneycombBrowserPlatformDelegateAlloy::GetBoundsChangedCallback() {
...
void HoneycombBrowserPlatformDelegateAlloy::SetOwnedWebContents(
...
void HoneycombBrowserPlatformDelegateAlloy::DestroyExtensionHost() {
  if (!extension_host_) {
    return;
  }
...
void HoneycombBrowserPlatformDelegateAlloy::OnExtensionHostDeleted() {
  DCHECK(is_background_host_);
  DCHECK(extension_host_);
  extension_host_ = nullptr;
}
