...
#include "libhoneycomb/browser/alloy/alloy_content_browser_client.h"
...
#include "libhoneycomb/browser/alloy/alloy_browser_main.h"
#include "libhoneycomb/browser/alloy/alloy_web_contents_view_delegate.h"
#include "libhoneycomb/browser/browser_context.h"
#include "libhoneycomb/browser/browser_frame.h"
#include "libhoneycomb/browser/browser_info.h"
#include "libhoneycomb/browser/browser_info_manager.h"
#include "libhoneycomb/browser/browser_manager.h"
#include "libhoneycomb/browser/browser_platform_delegate.h"
#include "libhoneycomb/browser/certificate_query.h"
#include "libhoneycomb/browser/context.h"
#include "libhoneycomb/browser/devtools/devtools_manager_delegate.h"
#include "libhoneycomb/browser/extensions/extension_system.h"
#include "libhoneycomb/browser/extensions/extension_web_contents_observer.h"
#include "libhoneycomb/browser/media_capture_devices_dispatcher.h"
#include "libhoneycomb/browser/net/chrome_scheme_handler.h"
#include "libhoneycomb/browser/net/throttle_handler.h"
#include "libhoneycomb/browser/net_service/cookie_manager_impl.h"
#include "libhoneycomb/browser/net_service/login_delegate.h"
...
#include "libhoneycomb/browser/prefs/renderer_prefs.h"
#include "libhoneycomb/browser/speech_recognition_manager_delegate.h"
...
#include "libhoneycomb/common/crash_reporting.h"
#endif
...
class HoneycombSelectClientCertificateCallbackImpl
    : public HoneycombSelectClientCertificateCallback {
 public:
  explicit HoneycombSelectClientCertificateCallbackImpl(
      std::unique_ptr<content::ClientCertificateDelegate> delegate)
      : delegate_(std::move(delegate)) {}

  HoneycombSelectClientCertificateCallbackImpl(
      const HoneycombSelectClientCertificateCallbackImpl&) = delete;
  HoneycombSelectClientCertificateCallbackImpl& operator=(
      const HoneycombSelectClientCertificateCallbackImpl&) = delete;
 ...
 private:
  void DoSelect(HoneycombRefPtr<HoneycombX509Certificate> cert) {
    if (HONEYCOMB_CURRENTLY_ON_UIT()) {
      RunNow(std::move(delegate_), cert);
    } else {
      HONEYCOMB_POST_TASK(
          HONEYCOMB_UIT,
          base::BindOnce(&HoneycombSelectClientCertificateCallbackImpl::RunNow,
  ...
  static void RunNow(
    ...
    if (cert) {
      HoneycombX509CertificateImpl* certImpl =
          static_cast<HoneycombX509CertificateImpl*>(cert.get());
      certImpl->AcquirePrivateKey(base::BindOnce(
          &HoneycombSelectClientCertificateCallbackImpl::RunWithPrivateKey,
  ...
  IMPLEMENT_REFCOUNTING(HoneycombSelectClientCertificateCallbackImpl);
};
...
void AlloyContentBrowserClient::RenderProcessWillLaunch(
  ...
  // Forwards dynamic parameters to HoneycombRenderThreadObserver.
...
bool AlloyContentBrowserClient::IsHandledURL(const GURL& url) {
  ...
  return HoneycombAppManager::Get()->HasCustomScheme(scheme);
}
...
void AlloyContentBrowserClient::AppendExtraCommandLineSwitches(
  ...
  if (process_type == switches::kRendererProcess) {
    ...
    if (extensions::ExtensionsEnabled()) {
      ...
      HoneycombBrowserContext* honey_browser_context =
          process ? HoneycombBrowserContext::FromBrowserContext(browser_context)
                  : nullptr;
      if (honey_browser_context) {
        if (honey_browser_context->IsPrintPreviewSupported()) {
...
content::MediaObserver* AlloyContentBrowserClient::GetMediaObserver() {
  return HoneycombMediaCaptureDevicesDispatcher::GetInstance();
}
...
AlloyContentBrowserClient::CreateSpeechRecognitionManagerDelegate() {
  ...
  if (command_line->HasSwitch(switches::kEnableSpeechInput)) {
    return new HoneycombSpeechRecognitionManagerDelegate();
  }
...
base::OnceClosure AlloyContentBrowserClient::SelectClientCertificate(
  ...
  HoneycombRefPtr<HoneycombRequestHandler> handler;
  ...
  HoneycombRequestHandler::X509CertificateList certs;
  for (net::ClientCertIdentityList::iterator iter = client_certs.begin();
       iter != client_certs.end(); iter++) {
    certs.push_back(new HoneycombX509CertificateImpl(std::move(*iter)));
  }

  HoneycombRefPtr<HoneycombSelectClientCertificateCallbackImpl> callbackImpl(
      new HoneycombSelectClientCertificateCallbackImpl(std::move(delegate)));
...
AlloyContentBrowserClient::CreateDevToolsManagerDelegate() {
  return std::make_unique<HoneycombDevToolsManagerDelegate>();
}
...
void AlloyContentBrowserClient::RegisterNonNetworkSubresourceURLLoaderFactories(
  ...
  extensions::HoneycombExtensionWebContentsObserver* web_observer =
      extensions::HoneycombExtensionWebContentsObserver::FromWebContents(
          web_contents);

  // There is nothing to do if no HoneycombExtensionWebContentsObserver is attached
  // to the |web_contents|.
  if (!web_observer) {
    return;
  }
...
bool AlloyContentBrowserClient::ConfigureNetworkContextParams(
  ...
  Profile* profile = honey_context->AsProfile();
  ...
  network_context_params->cookieable_schemes =
      honey_context->GetCookieableSchemes();

  // TODO(honey): Remove this and add required NetworkAnonymizationKey,
...
bool AlloyContentBrowserClient::HandleExternalProtocol(
  ...
  // HoneycombBrowserPlatformDelegate::HandleExternalProtocol may be called if
  // nothing handles the request.
  auto request_handler = net_service::CreateInterceptedRequestHandler(
      web_contents_getter, frame_tree_node_id, resource_request,
      base::BindRepeating(HoneycombBrowserPlatformDelegate::HandleExternalProtocol,
...
void AlloyContentBrowserClient::OnWebContentsCreated(
  ...
  // cases HoneycombBrowserPlatformDelegateAlloy::BrowserCreated and/or
  // HoneycombExtensionsAPIClient::AttachWebContentsHelpers should be used instead.

  if (extensions::ExtensionsEnabled()) {
    extensions::HoneycombExtensionWebContentsObserver::CreateForWebContents(
        web_contents);
  }
}
...
HoneycombRefPtr<HoneycombRequestContextImpl> AlloyContentBrowserClient::request_context()
    const {
  return browser_main_parts_->request_context();
}

HoneycombDevToolsDelegate* AlloyContentBrowserClient::devtools_delegate() const {
