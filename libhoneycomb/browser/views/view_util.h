...
#ifndef HONEYCOMB_LIBHONEYCOMB_BROWSER_VIEWS_VIEW_UTIL_H_
#define HONEYCOMB_LIBHONEYCOMB_BROWSER_VIEWS_VIEW_UTIL_H_
#pragma once
...
#define HONEYCOMB_REQUIRE_VALID_RETURN(ret) \
  if (!ParentClass::IsValid())        \
    return ret;

#define HONEYCOMB_REQUIRE_VALID_RETURN_VOID() \
  if (!ParentClass::IsValid())          \
    return;

// The below functions manage the relationship between HoneycombView and views::View
...
// Called when a HoneycombView is initialized to create the initial association
// between the underlying views::View and |view|. The HoneycombView owns the
// views::View at this stage.
void Register(HoneycombRefPtr<HoneycombView> view);

// Returns the HoneycombView object associated with the specified |view|. If no
// HoneycombView is associated with |view| and |find_known_parent| is true then this
...
// Called from HoneycombOverlayViewHost::Init to associate |host_view| with |widget|.
// This is necessary for GetWindowFor() to correctly return the HoneycombWindow
...
#endif  // HONEYCOMB_LIBHONEYCOMB_BROWSER_VIEWS_VIEW_UTIL_H_
