...
#include "libhoneycomb/browser/geometry_util.h"
...
class ClientViewEx : public views::ClientView {
 public:
  ClientViewEx(views::Widget* widget,
               ...
               HoneycombWindowView::Delegate* window_delegate)
 ...
 private:
  HoneycombWindowView::Delegate* window_delegate_;  // Not owned by this object.
};

// Extend NativeFrameView with draggable region handling.
class NativeFrameViewEx : public views::NativeFrameView {
 public:
  NativeFrameViewEx(views::Widget* widget, HoneycombWindowView* view)
...
class CaptionlessFrameView : public views::NonClientFrameView {
 public:
  CaptionlessFrameView(views::Widget* widget, HoneycombWindowView* view)
...
HoneycombWindowView::HoneycombWindowView(HoneycombWindowDelegate* honey_delegate,
                             Delegate* window_delegate)
    : ParentClass(honey_delegate), window_delegate_(window_delegate) {
  DCHECK(window_delegate_);
}

void HoneycombWindowView::CreateWidget(gfx::AcceleratedWidget parent_widget) {
  DCHECK(!GetWidget());

  // |widget| is owned by the NativeWidget and will be destroyed in response to
  // a native destruction message.
  views::Widget* widget = honey::IsChromeRuntimeEnabled() ? new ChromeBrowserFrame
                                                        : new views::Widget;
  ...
  RegisterDeleteDelegateCallback(
      base::BindOnce(&HoneycombWindowView::DeleteDelegate, base::Unretained(this)));

  if (honey_delegate()) {
    HoneycombRefPtr<HoneycombWindow> honey_window = GetHoneycombWindow();

    auto bounds = honey_delegate()->GetInitialBounds(honey_window);
    params.bounds = gfx::Rect(bounds.x, bounds.y, bounds.width, bounds.height);

    if (has_native_parent) {
      DCHECK(!params.bounds.IsEmpty());
    } else {
      is_frameless_ = honey_delegate()->IsFrameless(honey_window);

      params.native_widget =
          view_util::CreateNativeWidget(widget, honey_window, honey_delegate());

      can_resize = honey_delegate()->CanResize(honey_window);

      const auto show_state = honey_delegate()->GetInitialShowState(honey_window);
      switch (show_state) {
      ...
      HoneycombRefPtr<HoneycombWindow> parent_window = honey_delegate()->GetParentWindow(
          honey_window, &is_menu, &can_activate_menu);
      if (parent_window && !parent_window->IsSame(honey_window)) {
        HoneycombWindowImpl* parent_window_impl =
            static_cast<HoneycombWindowImpl*>(parent_window.get());
        ...
        if (is_menu) {
          ...
          if (honey_delegate()->IsWindowModalDialog(honey_window)) {
  ...
  if (host_widget) {
    ...
    if (honey_delegate() && honey_delegate()->IsWindowModalDialog(GetHoneycombWindow())) {
...
HoneycombRefPtr<HoneycombWindow> HoneycombWindowView::GetHoneycombWindow() const {
  HoneycombRefPtr<HoneycombWindow> window = GetHoneycombPanel()->AsWindow();
  DCHECK(window);
  return window;
}

void HoneycombWindowView::DeleteDelegate() {
...
bool HoneycombWindowView::CanMinimize() const {
  if (!honey_delegate()) {
    return true;
  }
  return honey_delegate()->CanMinimize(GetHoneycombWindow());
}

bool HoneycombWindowView::CanMaximize() const {
  if (!honey_delegate()) {
    return true;
  }
  return honey_delegate()->CanMaximize(GetHoneycombWindow());
}

std::u16string HoneycombWindowView::GetWindowTitle() const {
  return title_;
}

ui::ImageModel HoneycombWindowView::GetWindowIcon() {
  if (!window_icon_) {
    return ParentClass::GetWindowIcon();
  }
  auto image_skia =
      static_cast<HoneycombImageImpl*>(window_icon_.get())
          ->GetForced1xScaleRepresentation(GetDisplay().device_scale_factor());
  return ui::ImageModel::FromImageSkia(image_skia);
}

ui::ImageModel HoneycombWindowView::GetWindowAppIcon() {
  if (!window_app_icon_) {
    return ParentClass::GetWindowAppIcon();
  }
  auto image_skia =
      static_cast<HoneycombImageImpl*>(window_app_icon_.get())
          ->GetForced1xScaleRepresentation(GetDisplay().device_scale_factor());
  return ui::ImageModel::FromImageSkia(image_skia);
}

void HoneycombWindowView::WindowClosing() {
#if BUILDFLAG(IS_LINUX)
#if BUILDFLAG(OZONE_PLATFORM_X11)
  if (host_widget()) {
    auto parent = static_cast<gfx::AcceleratedWidget>(
...
views::View* HoneycombWindowView::GetContentsView() {
  // |this| will be the "Contents View" hosted by the Widget via ClientView and
  // RootView.
  return this;
}

views::ClientView* HoneycombWindowView::CreateClientView(views::Widget* widget) {
  return new ClientViewEx(widget, GetContentsView(), window_delegate_);
}

std::unique_ptr<views::NonClientFrameView>
HoneycombWindowView::CreateNonClientFrameView(views::Widget* widget) {
  if (is_frameless_) {
...
bool HoneycombWindowView::ShouldDescendIntoChildForEventHandling(
...
bool HoneycombWindowView::MaybeGetMinimumSize(gfx::Size* size) const {
#if BUILDFLAG(IS_LINUX)
...
bool HoneycombWindowView::MaybeGetMaximumSize(gfx::Size* size) const {
#if BUILDFLAG(IS_LINUX)
  // Resize is disabled on Linux by returning the preferred size as the min/max
  // size.
  if (!CanResize()) {
...
void HoneycombWindowView::ViewHierarchyChanged(
    const views::ViewHierarchyChangedDetails& details) {
  if (details.child == this) {
    // This View's parent types (RootView, ClientView) are not exposed via the
    // Honeycomb API. Therefore don't send notifications about this View's parent
    // changes.
    return;
  }
...
void HoneycombWindowView::OnWidgetActivationChanged(views::Widget* widget,
                                              bool active) {
  if (honey_delegate()) {
    honey_delegate()->OnWindowActivationChanged(GetHoneycombWindow(), active);
  }
}

void HoneycombWindowView::OnWidgetBoundsChanged(views::Widget* widget,
                                          const gfx::Rect& new_bounds) {
  MoveOverlaysIfNecessary();

  if (honey_delegate()) {
    honey_delegate()->OnWindowBoundsChanged(
        GetHoneycombWindow(), {new_bounds.x(), new_bounds.y(), new_bounds.width(),
                         new_bounds.height()});
  }
}

display::Display HoneycombWindowView::GetDisplay() const {
...
void HoneycombWindowView::SetTitle(const std::u16string& title) {
  title_ = title;
...
void HoneycombWindowView::SetWindowIcon(HoneycombRefPtr<HoneycombImage> window_icon) {
...
void HoneycombWindowView::SetWindowAppIcon(HoneycombRefPtr<HoneycombImage> window_app_icon) {
...
HoneycombRefPtr<HoneycombOverlayController> HoneycombWindowView::AddOverlayView(
  ...
  if (widget) {
    ...
    overlay_hosts_.push_back(
        std::make_unique<HoneycombOverlayViewHost>(this, docking_mode));
...
void HoneycombWindowView::MoveOverlaysIfNecessary() {
...
void HoneycombWindowView::SetDraggableRegions(
  ...
  for (const HoneycombDraggableRegion& region : regions) {
...
views::NonClientFrameView* HoneycombWindowView::GetNonClientFrameView() const {
  const views::Widget* widget = GetWidget();
  if (!widget) {
    return nullptr;
  }
...
void HoneycombWindowView::UpdateFindBarBoundingBox(gfx::Rect* bounds) const {
...
views::Widget* HoneycombWindowView::host_widget() const {
...
absl::optional<float> HoneycombWindowView::GetTitlebarHeight() const {
  if (honey_delegate()) {
    float title_bar_height = 0;
    const bool has_title_bar_height =
        honey_delegate()->GetTitlebarHeight(GetHoneycombWindow(), &title_bar_height);
