...
#include "libhoneycomb/browser/chrome/chrome_browser_context.h"
...
ChromeBrowserContext::ChromeBrowserContext(
    ...
    : HoneycombBrowserContext(settings), weak_ptr_factory_(this) {}
                                          ...
                                          Profile *profile)
{
      ...
      status != Profile::CreateStatus::CREATE_STATUS_INITIALIZED)
  {
    ...
    const auto &profile_id = Profile::OTRProfileID::CreateUniqueForHoneycomb();
  ...
  if (status == Profile::CreateStatus::CREATE_STATUS_INITIALIZED)
  {
    ...
    // ChromeContentBrowserClientHoneycomb::ConfigureNetworkContextParams so that
    // HoneycombBrowserContext::FromBrowserContext can find us.
    if (otr_profile)
    {
      otr_profile->Init();
